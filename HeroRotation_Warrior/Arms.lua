...
local function APL()
  ...
  Execute = function()
    ...
    if S.Execute:IsCastableP("Melee") and (Player:BuffP(S.MemoryofLucidDreams) or Player:BuffP(S.DeadlyCalmBuff)) then
  ...
  FiveTarget = function()
    ...
    if S.Execute:IsCastableP("Melee") and ((not S.Cleave:IsAvailable() and Target:DebuffRemainsP(S.DeepWoundsDebuff) < 2) or (Player:BuffP(S.SuddenDeathBuff) or Player:BuffP(S.StoneHeartBuff)) and (Player:BuffP(S.SweepingStrikesBuff) or S.SweepingStrikes:CooldownRemainsP() > 8)) then
  ...
  Hac = function()
    ...
    if S.Execute:IsCastableP("Melee") and ((not S.Cleave:IsAvailable() and Target:DebuffRemainsP(S.DeepWoundsDebuff) < 2) or Player:BuffStackP(S.SuddenDeathBuff)) then
  ...
  SingleTarget = function()
    ...
    if S.Execute:IsCastableP("Melee") and (Player:BuffP(S.SuddenDeathBuff)) then
