...
local function Trinkets()
  if Settings.Commons.Enabled.Trinkets then
    ...
    if I.TreacherousTransmitter:IsEquippedAndReady() and (VarAddsRemain or VarSTPlanning) then
      if not Settings.Commons.SyncTrinketsToCDs or CDsON() then
    ...
    if Trinket1:IsReady() and not VarTrinket1BL and (not Settings.Commons.SyncTrinketsToCDs or CDsON()) then
      local shouldUseTrinket = (VarTrinket1Buffs and not VarTrinket1Manual
        and (Player:BuffDown(S.AvatarBuff) and VarTrinket1CastTime > 0 or VarTrinket1CastTime == 0)
        and ((S.TitansTorment:IsAvailable() and S.Avatar:CooldownUp()) or (Player:BuffUp(S.AvatarBuff) and not S.TitansTorment:IsAvailable()))
        and (VarTrinket2Exclude or not Trinket2:HasCooldown() or Trinket2:CooldownDown() or VarTrinketPriority == 1))
        or Trinket1:BuffDuration() >= BossFightRemains
      if shouldUseTrinket then
    ...
    if Trinket2:IsReady() and not VarTrinket2BL and (not Settings.Commons.SyncTrinketsToCDs or CDsON()) then
      local shouldUseTrinket = (VarTrinket2Buffs and not VarTrinket2Manual
        and (Player:BuffDown(S.AvatarBuff) and VarTrinket2CastTime > 0 or VarTrinket2CastTime == 0)
        and ((S.TitansTorment:IsAvailable() and S.Avatar:CooldownUp()) or (Player:BuffUp(S.AvatarBuff) and not S.TitansTorment:IsAvailable()))
        and (VarTrinket1Exclude or not Trinket1:HasCooldown() or Trinket1:CooldownDown() or VarTrinketPriority == 2))
    ...
    if Trinket1:IsReady() and not VarTrinket1BL and (not Settings.Commons.SyncTrinketsToCDs or CDsON()) then
      local shouldUseTrinket = not VarTrinket1Buffs
        and (VarTrinket1CastTime > 0 and Player:BuffDown(S.AvatarBuff) or VarTrinket1CastTime == 0)
        and not VarTrinket1Manual
        and (not VarTrinket1Buffs and (Trinket2:CooldownDown() or not VarTrinket2Buffs)
          or (VarTrinket1CastTime > 0 and Player:BuffDown(S.AvatarBuff) or VarTrinket1CastTime == 0)
          or S.Avatar:CooldownRemains() > 20)
      if shouldUseTrinket then
    ...
    if Trinket2:IsReady() and not VarTrinket2BL and (not Settings.Commons.SyncTrinketsToCDs or CDsON()) then
      local shouldUseTrinket = not VarTrinket2Buffs
        and (VarTrinket2CastTime > 0 and Player:BuffDown(S.AvatarBuff) or VarTrinket2CastTime == 0)
        and not VarTrinket2Manual
        and (not VarTrinket2Buffs and (Trinket1:CooldownDown() or not VarTrinket1Buffs)
          or (VarTrinket2CastTime > 0 and Player:BuffDown(S.AvatarBuff) or VarTrinket2CastTime == 0)
          or S.Avatar:CooldownRemains() > 20)
      if shouldUseTrinket then
  ...
  if Settings.Commons.Enabled.Items then
    ...
    if ItemToUse and (not Settings.Commons.SyncTrinketsToCDs or CDsON()) then
...
local function Init()
  HR.Print("Fury Warrior rotation has been updated for patch 11.0.2 with new custom settings.")
end
