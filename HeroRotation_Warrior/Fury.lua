...
local function Precombat()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Charge:IsCastable() and not Target:IsSpellInRange(S.Bloodthirst) then
      if HR.Cast(S.Charge, nil, nil, not Target:IsSpellInRange(S.Charge)) then return "charge"; end
    end
...
local function SingleTarget()
  ...
  if S.Siegebreaker:IsCastable() then
    if HR.Cast(S.Siegebreaker, Settings.Fury.GCDasOffGCD.Siegebreaker, nil, not Target:IsSpellInRange(S.Siegebreaker)) then return "siegebreaker "; end
  end
  ...
  if S.DragonRoar:IsCastable() and HR.CDsON() and (Player:BuffUp(S.EnrageBuff)) then
    if HR.Cast(S.DragonRoar, Settings.Fury.GCDasOffGCD.DragonRoar, nil, not Target:IsInRange(12)) then return "dragon_roar"; end
  end
  ...
  if S.CrushingBlow:IsCastable() and (S.CrushingBlow:Charges() == 2) then
    if HR.Cast(S.CrushingBlow, nil, nil, not Target:IsSpellInRange(S.CrushingBlow)) then return "crushing_blow"; end
  end
  -- bloodthirst
  if S.Bloodthirst:IsCastable() then
    if HR.Cast(S.Bloodthirst, nil, nil, not Target:IsSpellInRange(S.Bloodthirst)) then return "bloodthirst"; end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Charge:IsCastable() and (not Target:IsInMeleeRange(5)) then
      if HR.Cast(S.Charge, Settings.Fury.GCDasOffGCD.Charge, nil, not Target:IsSpellInRange(S.Charge)) then return "charge"; end
    end
    ...
    if (Player:BuffUp(S.RecklessnessBuff)) then
      ...
      if S.BloodFury:IsCastable() then
        if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return "bloodfury"; end
      end
