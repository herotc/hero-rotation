...
--- APL Action Lists (and Variables)
  ...
  local function AoE ()
    ...
    if HR.CDsON() and S.Bladestorm:IsCastable() and Player:BuffRemainsP(S.Enrage) > 2 and Cache.EnemiesCount[8] > 1 then
      if HR.Cast(S.Bladestorm) then return ""; end
    end
    -- actions.aoe+=/whirlwind,if=buff.meat_cleaver.down
    if HR.AoEON() and S.Whirlwind:IsCastable() and not Player:Buff(S.MeatCleaver) then
    ...
    if HR.AoEON() and S.Whirlwind:IsCastable() then
      if HR.Cast(S.Whirlwind) then return ""; end
    end
    return false;
  end
  ...
  local function CDs ()
    ...
    if HR.CDsON() and S.OdynsFury:IsCastable() and Player:BuffP(S.Enrage) and (S.RagingBlow:CooldownRemainsP() > 0 or not S.InnerRage:IsAvailable()) then
  ...
  local function execute ()
    ...
    if HR.CDsON() and S.OdynsFury:IsCastable() then
      if HR.Cast(S.OdynsFury) then return ""; end
    end
...
-- Unit Update
  ...
  if HR.CDsON() and S.DragonRoar:IsCastable() 
  ...
  if HR.CDsON() and S.Rampage:IsReady() and S.BattleCry:CooldownRemainsP() < 1 and S.Bloodbath:CooldownRemainsP() < 1 and Target:HealthPercentage() > 20 then
    if HR.CastQueue(S.Rampage, S.BattleCry) then return ""; end
  end
  ...
  if I.UmbralMoonglaives:IsReady() and I.UmbralMoonglaives:IsEquipped() 
    ...
    if HR.Cast(I.UmbralMoonglaives, Settings.Fury.OffGCDasOffGCD.UmbralMoonglaives) then return ""; end
  end
  ...
  if HR.CDsON() then
    ...
    if S.Avatar:IsCastable() 
      ...
      if HR.Cast(S.Avatar, Settings.Commons.OffGCDasOffGCD.Avatar) then return ""; end
    end
    ...
    if S.BattleCry:IsCastable() 
      ...
      if HR.Cast(S.BattleCry, Settings.Commons.OffGCDasOffGCD.BattleCry) then return ""; end
    end
    ...
    if S.Bloodbath:IsCastable() and (Player:BuffP(S.BattleCry) or (Target:TimeToDie() < 14) or (S.BattleCry:CooldownRemainsP() < 2 and Player:PrevGCDP(1, S.Rampage))) then
      if HR.Cast(S.Bloodbath, Settings.Fury.OffGCDasOffGCD.Bloodbath) then return ""; end
    end
