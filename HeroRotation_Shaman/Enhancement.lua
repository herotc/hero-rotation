...
local function EvaluateCycleFlameShock(TargetUnit)
  return (TargetUnit:DebuffRefreshable(S.FlameShock))
end
...
local function Single()
  -- primordial_wave,if=!buff.primordial_wave.up TODO
  -- windfury_totem,if=runeforge.doom_winds.equipped&buff.doom_winds_debuff.down TODO
  ...
  -- crash_lightning
  if S.CrashLightning:IsCastable() and Target:IsInRange(5) then
...
local function Aoe()
  ...
  -- windfury_totem,if=runeforge.doom_winds.equipped&buff.doom_winds_debuff.down TODO
  -- flame_shock,target_if=refreshable,cycle_targets=1,if=talent.fire_nova.enabled|talent.lashing_flames.enabled|covenant.necrolord TODO covenant
  ...
  -- fire_nova,if=active_dot.flame_shock>=3 TODO
  if S.FireNova:IsCastable() and EnemiesFlameShockCount >= 3 then
    if HR.Cast(S.FireNova) then return "FireNova 2"; end
  end
  -- vesper_totem TODO
  -- lightning_bolt,if=buff.primordial_wave.up&(buff.stormkeeper.up|buff.maelstrom_weapon.stack>=5) TODO
  -- crash_lightning,if=talent.crashing_storm.enabled|buff.crash_lightning.down
  if S.CrashLightning:IsCastable() and (S.CrashingStorm:IsAvailable() or Player:BuffDown(S.CrashLightningBuff)) and Target:IsInRange(5) then
    if HR.Cast(S.CrashLightning) then return "CrashLightning 2"; end
  end
  -- lava_lash,target_if=min:debuff.lashing_flames.remains,cycle_targets=1,if=talent.lashing_flames.enabled TODO
  -- crash_lightning
  if S.CrashLightning:IsCastable() and Target:IsInRange(5) then
  ...
  -- chain_harvest,if=buff.maelstrom_weapon.stack>=5 TODO
  -- elemental_blast,if=buff.maelstrom_weapon.stack>=5
  if S.ElementalBlast:IsCastable() and Player:BuffStack(S.MaelstromWeapon) >= 5 then
  ...
  -- flame_shock,target_if=refreshable,cycle_targets=1,if=talent.fire_nova.enabled TODO
  -- sundering
  if S.Sundering:IsCastable() and Target:IsInRange(5) then
    if HR.Cast(S.Sundering) then return "Sundering 2"; end
  end
  -- lava_lash,target_if=min:debuff.lashing_flames.remains,cycle_targets=1,if=runeforge.primal_lava_actuators.equipped&buff.primal_lava_actuators.stack>6 TODO
  -- stormstrike
  if S.Stormstrike:IsCastable() then
