...
local function Aoe()
  -- fire_elemental,if=!buff.fire_elemental.up&(!talent.primal_elementalist.enabled|!buff.lesser_fire_elemental.up)
  if S.FireElemental:IsReady() and (not Shaman.FireElemental.GreaterActive and (not S.PrimalElementalist:IsAvailable() or not Shaman.FireElemental.LesserActive)) then
    if Cast(S.FireElemental, Settings.Elemental.GCDasOffGCD.FireElemental) then return "fire_elemental aoe 2"; end
  end
  -- storm_elemental,if=!buff.storm_elemental.up&(!talent.primal_elementalist.enabled|!buff.lesser_storm_elemental.up)
  if S.StormElemental:IsReady() and (not Shaman.StormElemental.GreaterActive and (not S.PrimalElementalist:IsAvailable() or not Shaman.StormElemental.LesserActive)) then
...
local function SingleTarget()
  -- fire_elemental,if=!buff.fire_elemental.up&(!talent.primal_elementalist.enabled|!buff.lesser_fire_elemental.up)
  if S.FireElemental:IsCastable() and (not Shaman.FireElemental.GreaterActive and (not S.PrimalElementalist:IsAvailable() or not Shaman.FireElemental.LesserActive)) then
    if Cast(S.FireElemental, Settings.Elemental.GCDasOffGCD.FireElemental) then return "fire_elemental single_target 2"; end
  end
  -- storm_elemental,if=!buff.storm_elemental.up&(!talent.primal_elementalist.enabled|!buff.lesser_storm_elemental.up)
  if S.StormElemental:IsCastable() and (not Shaman.StormElemental.GreaterActive and (not S.PrimalElementalist:IsAvailable() and not Shaman.StormElemental.LesserActive)) then
    if Cast(S.StormElemental, Settings.Elemental.GCDasOffGCD.StormElemental) then return "storm_elemental single_target 4"; end
  end
  -- totemic_recall,if=cooldown.liquid_magma_totem.remains>45&(active_enemies>1&(spell_targets.chain_lightning>1|spell_targets.lava_beam>1))
  if S.TotemicRecall:IsCastable() and (S.LiquidMagmaTotem:CooldownRemains() > 45 and Shaman.ClusterTargets > 1) then
    if Cast(S.TotemicRecall, Settings.CommonsOGCD.GCDasOffGCD.TotemicRecall) then return "totemic_recall single_target 6"; end
  end
  -- liquid_magma_totem,if=active_dot.flame_shock=0|dot.flame_shock.remains<6|active_enemies>1&(spell_targets.chain_lightning>1|spell_targets.lava_beam>1)
  if S.LiquidMagmaTotem:IsCastable() and (S.FlameShockDebuff:AuraActiveCount() == 0 or Target:DebuffRemains(S.FlameShockDebuff) < 6 or Shaman.ClusterTargets > 1) then
  ...
  -- primordial_wave,target_if=min:dot.flame_shock.remains
  ...
  -- flame_shock,target_if=min:dot.flame_shock.remains,if=active_enemies=1&refreshable&(dot.flame_shock.remains<cooldown.primordial_wave.remains|!talent.primordial_wave.enabled)&!buff.surge_of_power.up&(!buff.master_of_the_elements.up|(!buff.stormkeeper.up&(talent.elemental_blast.enabled&maelstrom<90-10*talent.eye_of_the_storm.enabled|maelstrom<60-5*talent.eye_of_the_storm.enabled)))
  if S.FlameShock:IsCastable() and (Shaman.ClusterTargets == 1 and Target:DebuffRefreshable(S.FlameShockDebuff) and (Target:DebuffRemains(S.FlameShockDebuff) < S.PrimordialWave:CooldownRemains() or not S.PrimordialWave:IsAvailable()) and Player:BuffDown(S.SurgeofPowerBuff) and (not Player:MotEUp() or (not Player:StormkeeperUp() and (S.ElementalBlast:IsAvailable() and Player:MaelstromP() < 90 - 10 * S.EyeoftheStorm:TalentRank() or Player:MaelstromP() < 60 - 5 * num(S.EyeoftheStorm:IsAvailable()))))) then
  ...
  -- stormkeeper,if=!buff.ascendance.up&!buff.stormkeeper.up&buff.surge_of_power.up
  if S.Stormkeeper:IsViable() and (Player:BuffDown(S.AscendanceBuff) and not Player:StormkeeperUp() and Player:BuffUp(S.SurgeofPowerBuff)) then
    if Cast(S.Stormkeeper, Settings.Elemental.GCDasOffGCD.Stormkeeper) then return "stormkeeper single_target 18"; end
  end
  -- stormkeeper,if=!buff.ascendance.up&!buff.stormkeeper.up&(!talent.surge_of_power.enabled|!talent.elemental_blast.enabled)
  if S.Stormkeeper:IsViable() and (Player:BuffDown(S.AscendanceBuff) and not Player:StormkeeperUp() and (not S.SurgeofPower:IsAvailable() or not S.ElementalBlast:IsAvailable())) then
  ...
  if S.Ascendance:IsCastable() and (not Player:StormkeeperUp()) then
  ...
  if S.LavaBeam:IsCastable() and (Shaman.ClusterTargets > 1 and Player:StormkeeperUp() and not S.SurgeofPower:IsAvailable()) then
  ...
  if S.LightningBolt:IsViable() and (Player:StormkeeperUp() and not S.SurgeofPower:IsAvailable() and Player:MotEUp()) then
  ...
  if S.LightningBolt:IsViable() and (Player:StormkeeperUp() and not S.SurgeofPower:IsAvailable() and not S.MasteroftheElements:IsAvailable()) then
  ...
  if S.LavaBeam:IsCastable() and (Shaman.ClusterTargets > 1 and Player:PotMUp() and Player:BuffRemains(S.AscendanceBuff) > S.LavaBeam:CastTime() and not Player:HasTier(31, 4)) then
    if Cast(S.LavaBeam, nil, nil, not Target:IsSpellInRange(S.LavaBeam)) then return "lava_beam single_target 42"; end
  end
  -- lava_burst,if=cooldown_react&buff.lava_surge.up&(talent.deeply_rooted_elements.enabled|!talent.master_of_the_elements.enabled|!talent.elemental_blast.enabled)
  if S.LavaBurst:IsViable() and (Player:BuffUp(S.LavaSurgeBuff) and (S.DeeplyRootedElements:IsAvailable() or not S.MasteroftheElements:IsAvailable() or not S.ElementalBlast:IsAvailable())) then
    if Cast(S.LavaBurst, nil, nil, not Target:IsSpellInRange(S.LavaBurst)) then return "lava_burst single_target 44"; end
  end
  -- lava_burst,target_if=dot.flame_shock.remains>2,if=buff.ascendance.up&!talent.elemental_blast.enabled
  if S.LavaBurst:IsViable() and (Player:BuffUp(S.AscendanceBuff) and not S.ElementalBlast:IsAvailable()) then
    if Everyone.CastCycle(S.LavaBurst, Enemies10ySplash, EvaluateCycleFlameShockRemains2, not Target:IsSpellInRange(S.LavaBurst)) then return "lava_burst single_target 46"; end
  end
  ...
  if S.LavaBurst:IsViable() and (S.MasteroftheElements:IsAvailable() and not Player:MotEUp() and not S.LightningRod:IsAvailable()) then
  ...
  -- lava_burst,if=talent.master_of_the_elements.enabled&!buff.master_of_the_elements.up&(maelstrom>=80|maelstrom>=55&!talent.elemental_blast.enabled)&talent.swelling_maelstrom.enabled&maelstrom<=130
  if S.LavaBurst:IsViable() and (S.MasteroftheElements:IsAvailable() and not Player:MotEUp() and (Player:MaelstromP() >= 80 or Player:MaelstromP() >= 55 and not S.ElementalBlast:IsAvailable()) and S.SwellingMaelstrom:IsAvailable() and Player:MaelstromP() <= 130) then
    if Cast(S.LavaBurst, nil, nil, not Target:IsSpellInRange(S.LavaBurst)) then return "lava_burst single_target 50"; end
  end
  ...
  -- Note: Buff ID for echoes_of_great_sundering_eb and echoes_of_great_sundering_es is the same.
  if S.Earthquake:IsReady() and (Player:BuffUp(S.EchoesofGreatSunderingBuff) and (not S.ElementalBlast:IsAvailable() and Shaman.ClusterTargets < 2 or Shaman.ClusterTargets > 1)) then
  ...
  if S.Earthquake:IsReady() and (Shaman.ClusterTargets > 1 and not S.EchoesofGreatSundering:IsAvailable() and not S.ElementalBlast:IsAvailable()) then
  ...
  if S.ElementalBlast:IsViable() and (Player:MotEUp() or S.LightningRod:IsAvailable()) then
  ...
  if S.EarthShock:IsReady() then
    if Cast(S.EarthShock, nil, nil, not Target:IsSpellInRange(S.EarthShock)) then return "earth_shock single_target 58"; end
  end
  ...
  if S.LavaBurst:IsViable() and (S.DeeplyRootedElements:IsAvailable()) then
    if Everyone.CastCycle(S.LavaBurst, Enemies10ySplash, EvaluateCycleFlameShockRemains2, not Target:IsSpellInRange(S.LavaBurst)) then return "lava_burst single_target 60"; end
  end
  -- frost_shock,if=buff.icefury_dmg.up&talent.flux_melting.enabled&!buff.flux_melting.up
  if S.FrostShock:IsCastable() and (Player:IcefuryUp() and S.FluxMelting:IsAvailable() and Player:BuffDown(S.FluxMeltingBuff)) then
    if Cast(S.FrostShock, nil, nil, not Target:IsSpellInRange(S.FrostShock)) then return "frost_shock single_target 62"; end
  end
  -- lava_burst,target_if=dot.flame_shock.remains>2,if=talent.echo_of_the_elements.enabled|!talent.elemental_blast.enabled|!talent.master_of_the_elements.enabled|buff.stormkeeper.up
  if S.LavaBurst:IsViable() and (S.EchooftheElements:IsAvailable() or not S.ElementalBlast:IsAvailable() or not S.MasteroftheElements:IsAvailable() or Player:StormkeeperUp()) then
    if Cast(S.LavaBurst, nil, nil, not Target:IsSpellInRange(S.LavaBurst)) then return "lava_burst single_target 64"; end
  end
  -- elemental_blast
  if S.ElementalBlast:IsViable() then
    if Cast(S.ElementalBlast, nil, nil, not Target:IsSpellInRange(S.ElementalBlast)) then return "elemental_blast single_target 66"; end
  end
  -- icefury
  if S.Icefury:IsViable() then
    if Cast(S.Icefury, nil, nil, not Target:IsSpellInRange(S.Icefury)) then return "icefury single_target 68"; end
  end
  -- chain_lightning,if=buff.power_of_the_maelstrom.up&active_enemies>1&(spell_targets.chain_lightning>1|spell_targets.lava_beam>1)
  if S.ChainLightning:IsViable() and (Player:PotMUp() and Shaman.ClusterTargets > 1) then
