...
local function CombustionPhase ()
  ...
  if S.InfernalCascade:ConduitEnabled() and SunKingsBlessingEquipped and (Player:BuffUp(S.SunKingsBlessingBuffReady) or var_extended_combustion_remains > 1.5 * Player:GCD() * (var_sun_kings_blessing_max_stack - Player:BuffStack(S.SunKingsBlessingBuff))) then
  ...
  if S.FireBlast:IsReady() and (not S.InfernalCascade:ConduitEnabled() and S.FireBlast:Charges() >= 1 and Player:BuffUp(S.CombustionBuff) and Player:BuffDown(S.FirestormBuff) and Player:BuffDown(S.HotStreakBuff) and HotStreakInFlight() < 2) then
