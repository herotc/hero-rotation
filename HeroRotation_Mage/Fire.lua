...
local var_combustion_on_use = I.CrimsonGladiatorsBadge:IsEquipped() or I.ObsidianGladiatorsBadge:IsEquipped() or I.MoonlitPrism:IsEquipped() or I.IrideusFragment:IsEquipped() or I.SpoilsofNeltharus:IsEquipped() or I.TimebreachingTalon:IsEquipped() or I.HornofValor:IsEquipped()
...
local function CombustionPhase()
  ...
  -- phoenix_flames,if=(!talent.alexstraszas_fury&travel_time<buff.combustion.remains|talent.alexstraszas_fury)&buff.heating_up.react+hot_streak_spells_in_flight<2
  if S.PhoenixFlames:IsCastable() and ((not S.AlexstraszasFury:IsAvailable() and S.PhoenixFlames:TravelTime() < CombustionRemains or S.AlexstraszasFury:IsAvailable()) and num(Player:BuffUp(S.HeatingUpBuff)) + HotStreakInFlight() < 2) then
...
local function FirestarterFireBlasts()
  ...
  if S.FireBlast:IsReady() and not FreeCastAvailable() and Player:GCDRemains() > 0 and (not var_fire_blast_pooling and num(Player:BuffUp(S.HeatingUpBuff)) + HotStreakInFlight() == 1 and (S.ShiftingPower:CooldownUp() and (not Player:HasTier(30, 2) or Target:DebuffRemains(S.CharringEmbersDebuff) > 2 * GCDMax))) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.FireBlast:IsReady() and not FreeCastAvailable() and Player:GCDRemains() > 0 and (not var_fire_blast_pooling and var_time_to_combustion > 0 and EnemiesCount8ySplash >= var_hard_cast_flamestrike and not FirestarterActive() and Player:BuffDown(S.HotStreakBuff) and (Player:BuffUp(S.HeatingUpBuff) and S.Flamestrike:ExecuteRemains() < 0.5 or S.FireBlast:ChargesFractional() >= 2)) then
      if CastLeft(S.FireBlast) then return "fire_blast main 28"; end
    end
