...
local function APL()
  ...
  Burn = function()
    ...
    -- if (bool(VarBurnPhase)) and ((Player:IsChanneling(S.Evocation) or Player:PrevGCD(1, S.Evocation)) and (Player:ManaPercentage() >= 85)) then
    --     StopBurnPhase()
    ...
    if S.PresenceofMind:IsCastableP() and HR.CDsON() and (not Player:Buff(S.PresenceofMindBuff)) and (Player:BuffRemainsP(S.RuneofPowerBuff) <= PresenceOfMindMax() * S.ArcaneBlast:ExecuteTime() or Player:BuffRemainsP(S.ArcanePowerBuff) <= PresenceOfMindMax() * S.ArcaneBlast:ExecuteTime()) then
  ...
  Conserve = function()
    ...
    if S.PresenceofMind:IsCastableP() and HR.CDsON() and (not Player:Buff(S.PresenceofMindBuff)) and (HL.Tier20_2Pc and Player:ArcaneChargesP() == 0) then
      if HR.Cast(S.PresenceofMind, Settings.Arcane.OffGCDasOffGCD.PresenceofMind) then return ""; end
    end
    -- nether_tempest,if=(refreshable|!ticking)&buff.arcane_charge.stack=buff.arcane_charge.max_stack&buff.rune_of_power.down&buff.arcane_power.down
    if S.NetherTempest:IsCastableP() and ((Target:DebuffRefreshableCP(S.NetherTempestDebuff) or not Target:DebuffP(S.NetherTempestDebuff)) and Player:ArcaneChargesP() == Player:ArcaneChargesMax() and Player:BuffDownP(S.RuneofPowerBuff) and Player:BuffDownP(S.ArcanePowerBuff)) then
  ...
  Movement = function()
    ...
    -- presence_of_mind
    if S.PresenceofMind:IsCastableP() and HR.CDsON() and (not Player:Buff(S.PresenceofMindBuff)) then
      if HR.Cast(S.PresenceofMind, Settings.Arcane.OffGCDasOffGCD.PresenceofMind) then return ""; end
    end
    -- arcane_missiles
  ...
  if Everyone.TargetIsValid() then
    ...
    if HR.CDsON() and (bool(VarBurnPhase) or Target:TimeToDie() < VarAverageBurnLength or (S.ArcanePower:CooldownRemainsP() == 0 and S.Evocation:CooldownRemainsP() <= VarAverageBurnLength and (Player:ArcaneChargesP() == Player:ArcaneChargesMax() or (S.ChargedUp:IsAvailable() and S.ChargedUp:CooldownRemainsP() == 0)))) then
