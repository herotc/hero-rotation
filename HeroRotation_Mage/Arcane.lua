...
local function Burn ()
  ...
  if S.MarkofAluneth:IsCastableP() then
    if HR.Cast(S.MarkofAluneth) then return ""; end
  end
  -- mirror_image
  if HR.CDsON() and S.MirrorImage:IsCastableP() and Player:ManaP() >= S.MirrorImage:Cost() then
    if HR.Cast(S.MirrorImage) then return ""; end
  end
  -- rune_of_power,if=mana.pct>30|(buff.arcane_power.up|cooldown.arcane_power.up)
  if HR.CDsON() and S.RuneofPower:IsCastableP() and (Player:ManaPercentage() > 30 or (Player:BuffRemainsP(S.ArcanePower) > 0 or S.ArcanePower:CooldownUpP())) and not Player:IsCasting(S.RuneofPower) then
    if HR.Cast(S.RuneofPower) then return ""; end
  end
  -- arcane_power
  if HR.CDsON() and S.ArcanePower:IsCastableP() then
    if HR.Cast(S.ArcanePower) then return ""; end
  end
  -- blood_fury
  if HR.CDsON() and S.BloodFury:IsCastableP() then
    if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return ""; end
  end
  -- berserking
  if HR.CDsON() and S.Berserking:IsCastableP() then
    if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return ""; end
  end
  -- arcane_torrent
  if HR.CDsON() and S.ArcaneTorrent:IsCastableP() then
  ...
  -- if HR.CastSuggested(I.PotionOfDeadlyGrace) then return ""; end
  -- end
  ...
  if I.TarnishedSentinel:IsEquipped() and I.TarnishedSentinel:IsReady() and (Player:BuffP(S.ArcanePower) or Target:TimeToDie() < S.ArcanePower:CooldownRemainsP()) then
    if HR.Cast(I.TarnishedSentinel) then return ""; end
  end
  if I.KiljaedensBurningWish:IsEquipped() and I.KiljaedensBurningWish:IsReady() and (Player:BuffP(S.ArcanePower) or Target:TimeToDie() < S.ArcanePower:CooldownRemainsP()) then
    if HR.Cast(I.KiljaedensBurningWish) then return ""; end
  end
  ...
  if HR.CDsON() and S.PresenceofMind:IsCastableP() and not Player:Buff(S.PresenceofMind) and (((Player:ManaPercentage() > 30 or Player:BuffRemainsP(S.ArcanePower) > 0) and HL.Tier20_2Pc) or RoPDuration () <= PresenceOfMindMax * S.ArcaneBlast:ExecuteTime() or Player:BuffRemainsP(S.ArcanePower) <= PresenceOfMindMax * S.ArcaneBlast:ExecuteTime()) then
...
local function Conserve ()
  -- HR.Print("In Conserve phase")
  -- mirror_image,if=variable.time_until_burn>recharge_time|variable.time_until_burn>target.time_to_die
  if HR.CDsON() and S.MirrorImage:IsCastableP() and Player:ManaP() >= S.MirrorImage:Cost() and (v_timeUntilBurn > S.MirrorImage:RechargeP() or v_timeUntilBurn > Target:TimeToDie()) then
    if HR.Cast(S.MirrorImage) then return ""; end
  end
  ...
  if (S.RuneofPower:IsAvailable() and HL.Tier20_4Pc and v_timeUntilBurn > 30) then
    if HR.CastQueue(S.RuneofPower, S.ArcaneBarrage, S.PresenceofMind) then return; end
  end
  -- rune_of_power,if=full_recharge_time<=execute_time|prev_gcd.1.mark_of_aluneth
  if HR.CDsON() and S.RuneofPower:IsCastableP() and (S.RuneofPower:FullRechargeTimeP() <= S.RuneofPower:ExecuteTime() or Player:PrevGCDP(1, S.MarkofAluneth)) and not Player:IsCasting(S.RuneofPower) then
    if HR.Cast(S.RuneofPower) then return ""; end
  end
  ...
  if (S.ChargedUp:IsAvailable() and S.ChargedUp:RechargeP() < v_timeUntilBurn) then
    if HR.CastQueue(S.ArcaneBarrage, S.ChargedUp) then return ""; end
  end
  -- arcane_missiles,if=variable.arcane_missiles_procs=buff.arcane_missiles.max_stack&active_enemies<3
  if S.ArcaneMissiles:IsCastableP() and Player:ManaP() >= S.ArcaneMissiles:Cost() and (Player:BuffStackP(S.ArcaneMissilesProc) == ArcaneMissilesProcMax and Cache.EnemiesCount[range] < 3) then
    if HR.Cast(S.ArcaneMissiles) then return ""; end
...
local function APL ()
  ...
  if Everyone.TargetIsValid() then    
    ...
    if HR.CDsON() and Settings.Commons.UseTimeWarp and S.TimeWarp:IsCastable() and (not Player:HasHeroism() and (HL.CombatTime() < 3 or (Player:BuffRemainsP(S.ArcanePower) > 0 and (Player:BuffRemainsP(S.PotionOfDeadlyGrace) > 0 or not I.PotionOfDeadlyGrace:IsReady())) or Target:TimeToDie() <= S.TimeWarp:BaseDuration())) then
      if HR.Cast(S.TimeWarp, Settings.Commons.OffGCDasOffGCD.TimeWarp) then return ""; end
    end
    ...
    -- if HR.Cast(S.CancelBuff) then return ""; end
    -- end
