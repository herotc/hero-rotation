...
local function single_target ()
	...
	if S.RisingSunKick:IsReadyWW() and not S.Serenity:IsAvailable() or S.Serenity:CooldownRemains() >= 5 then
		if AR.Cast(S.RisingSunKick) then return ""; end
	end
	-- actions.st+=/fists_of_fury,if=!talent.serenity.enabled&energy.time_to_max>2
	if S.FistsOfFury:IsReadyWW() and not S.Serenity:IsAvailable() and Player:EnergyTimeToMax() > 2 then
		if AR.Cast(S.FistsOfFury) then return ""; end
	end
	-- actions.st+=/whirling_dragon_punch
	if S.WhirlingDragonPunch:IsReadyWW() and S.RisingSunKick:CooldownRemains() > Player:CastRemains() and S.FistsOfFury:CooldownRemains() > Player:CastRemains() then
	...
	if S.StrikeOfTheWindlord:IsReadyWW() and not S.Serenity:IsAvailable() or S.Serenity:CooldownRemains() >= 10 then
	...
	if S.RushingJadeWind:IsReadyWW() and Player:ChiDeficit() > 1 and not Player:PrevGCD(1, S.RushingJadeWind) then
	...
	if S.BlackoutKick:IsReadyWW() and (Player:Chi() > 1 or Player:Buff(S.BlackoutKickBuff) or (S.EnergizingElixir:IsAvailable() and S.EnergizingElixir:CooldownRemains() < S.FistsOfFury:CooldownRemains())) and
	...
	-- actions.st+=/chi_wave
	if S.ChiWave:IsReadyWW() then
		if AR.Cast(S.ChiWave) then return ""; end
	end
	-- actions.st+=/chi_burst
	if S.ChiBurst:IsReadyWW() then
	...
	(S.RisingSunKick:CooldownRemains() < Player:EnergyTimeToX(50,0) or Player:PrevGCD(1,S.TigerPalm)) and
	...
	-- actions.st+=/whirling_dragon_punch
	if S.WhirlingDragonPunch:IsUsable() and S.WhirlingDragonPunch:IsAvailable() and
	(S.WhirlingDragonPunch:CooldownRemains() < Player:EnergyTimeToX(50,0) or Player:PrevGCD(1,S.TigerPalm)) and
	(S.RisingSunKick:CooldownRemains() > S.WhirlingDragonPunch:CooldownRemains() and
	S.FistsOfFury:CooldownRemains() > S.WhirlingDragonPunch:CooldownRemains()) and
	(S.ChiWave:IsAvailable() and S.WhirlingDragonPunch:CooldownRemains() < S.ChiWave:CooldownRemains() or
	S.ChiBurst:IsAvailable() and S.WhirlingDragonPunch:CooldownRemains() < S.ChiBurst:CooldownRemains()) then
		if AR.Cast(S.WhirlingDragonPunch) then return ""; end
	end
	-- actions.st+=/chi_wave
	if S.ChiWave:IsAvailable() and S.ChiWave:CooldownRemains() < Player:EnergyTimeToX(50,0) or Player:PrevGCD(1, S.TigerPalm) then
		if AR.Cast(S.ChiWave) then return ""; end
	end
	-- actions.st+=/chi_burst
	if S.ChiBurst:IsAvailable() and S.ChiBurst:CooldownRemains() < Player:EnergyTimeToX(50,0) or Player:PrevGCD(1, S.TigerPalm) then
		if AR.Cast(S.ChiBurst) then return ""; end
	end
	-- actions.st+=/tiger_palm
  ...
  local function APL ()
    ...
    Everyone.AoEToggleEnemiesUpdate();
		...
		if Everyone.TargetIsValid() then
			-- actions.st+=/chi_wave
			if S.ChiWave:IsReadyWW() and not Target:IsInRange(5) then
				if AR.Cast(S.ChiWave) then return ""; end
			end
			-- actions.st+=/chi_burst
			if S.ChiBurst:IsReadyWW() and not Target:IsInRange(5) then
				if AR.Cast(S.ChiBurst) then return ""; end
			end
			-- actions+=/touch_of_death,if=target.time_to_die<=9
			if AR.CDsON() and S.TouchOfDeath:IsReadyWW() and Target:TimeToDie() >= 9 then
			...
			if S.EnergizingElixir:IsReadyWW() and Player:Chi() <= 1 and AC.CombatTime() > 8 and Player:EnergyDeficit() >= 20 and (S.RisingSunKick:CooldownRemains() <= 0 or
			...
			if S.ArcaneTorrent:IsReadyWW() and Player:ChiDeficit() >= 1 and Player:EnergyTimeToMax() >= 0.5 then
