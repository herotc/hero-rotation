...
local function APL()
  ...
  Cooldowns = function()
    ...
    if S.PillarofFrost:CooldownUpP() and (not S.BreathofSindragosa:IsAvailable() or Player:BuffP(S.EmpowerRuneWeaponBuff)) then
  ...
  Obliteration = function()
    ...
    if S.Obliterate:IsCastableP() and (not S.Frostscythe:IsAvailable() and not Player:BuffP(S.RimeBuff) and Cache.EnemiesCount[10] >= 3) then
      if HR.Cast(S.Obliterate) then return ""; end
    end
    ...
    if S.HowlingBlast:IsCastableP() and (Player:BuffP(S.RimeBuff) and Cache.EnemiesCount[10] >= 2) then
      if HR.Cast(S.HowlingBlast) then return ""; end
    end
