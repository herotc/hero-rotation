...
local function Trinkets()
  if Settings.Commons.Enabled.Trinkets then
    ...
    -- use_item,slot=trinket1,if=!trinket.1.cast_time>0&variable.trinket_1_buffs&!variable.trinket_1_manual&(!talent.breath_of_sindragosa|buff.breath_of_sindragosa.up|cooldown.breath_of_sindragosa.remains>trinket.1.cooldown.remains%2)&(buff.breath_of_sindragosa.up|!talent.breath_of_sindragosa&buff.pillar_of_frost.remains>10)&(!trinket.2.has_cooldown|trinket.2.cooldown.remains|variable.trinket_priority=1)
    if Trinket1:IsReady() and not VarTrinket1BL and (VarTrinket1CastTime == 0 and VarTrinket1Buffs and not VarTrinket1Manual and (not S.BreathofSindragosa:IsAvailable() or Player:BuffUp(S.BreathofSindragosa) or S.BreathofSindragosa:CooldownRemains() > Trinket1:CooldownRemains() / 2) and (Player:BuffUp(S.BreathofSindragosa) or not S.BreathofSindragosa:IsAvailable() and Player:BuffRemains(S.PillarofFrostBuff) > 10) and (not Trinket2:HasCooldown() or Trinket2:CooldownDown() or VarTrinketPriority == 1)) then
    ...
    if Trinket2:IsReady() and not VarTrinket2BL and (VarTrinket1CastTime == 0 and VarTrinket2Buffs and not VarTrinket2Manual and (not S.BreathofSindragosa:IsAvailable() or Player:BuffUp(S.BreathofSindragosa) or S.BreathofSindragosa:CooldownRemains() > Trinket2:CooldownRemains() / 2) and (Player:BuffUp(S.BreathofSindragosa) or not S.BreathofSindragosa:IsAvailable() and Player:BuffRemains(S.PillarofFrostBuff) > 10) and (not Trinket1:HasCooldown() or Trinket1:CooldownDown() or VarTrinketPriority == 2)) then
...
local function Variables()
  ...
  -- variable,name=pooling_runic_power,value=talent.breath_of_sindragosa&(variable.true_breath_cooldown<variable.breath_pooling_time|fight_remains<30&!cooldown.breath_of_sindragosa.remains)|talent.obliteration&runic_power<35&cooldown.pillar_of_frost.remains<variable.oblit_pooling_time
  VarPoolingRP = S.BreathofSindragosa:IsAvailable() and (VarTrueBreathCD < VarBreathPoolingTime or FightRemains < 30 and S.BreathofSindragosa:CooldownUp()) or S.Obliteration:IsAvailable() and Player:RunicPower() < 35 and S.PillarofFrost:CooldownRemains() < VarOblitPoolingTime
