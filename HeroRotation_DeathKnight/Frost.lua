...
local function Cooldowns()
  if HR.CDsON() then
    ...
    if S.PillarOfFrost:IsCastableP() and S.EmpowerRuneWeapon:CooldownRemainsP() then
    ...
    if (S.ColdHeartTalent:IsAvailable() or I.ColdHeart:IsEquipped()) and (((Player:BuffStack(S.ColdHeartBuff) >= 10 or Player:BuffStack(S.ColdHeartItemBuff) >= 10) and Target:DebuffStack(S.RazorIce) == 5) or Target:TimeToDie() <= Player:GCD()) then
      ...
      if S.ChainsOfIce:IsCastable() and (Player:BuffStack(S.ColdHeartItemBuff) > 5 or Player:BuffStack(S.ColdHeartBuff) > 5) and Target:TimeToDie() <= Player:GCD() then
      ...
      if S.ChainsOfIce:IsCastable() and ((Player:BuffRemainsP(S.PillarOfFrost) <= Player:GCD() * (1 + S.FrostwyrmsFury:CooldownReady)) or Player:BuffRemainsP(S.PillarOfFrost < Player:RuneTimeToX(3))) and Player:BuffP(S.PillarOfFrost) then
        if HR.Cast(S.ChainsOfIce) then return ""; end
      end
...
local function Obliteration()
  ...
  if S.Obliterate:IsCastable("Melee") and not S.FrostScythe:IsAvailable() and not Player:Buff(S.Rime) and Cache.EnemiesCount[10] >= 3 then
  ...
  if S.FrostScythe:IsCastable(8, true) and (Player:Buff(S.KillingMachine) or (Player:Buff(S.KillingMachine) and (Player:PrevGCD(1, S.FrostStrike) or Player:PrevGCD(1, S.HowlingBlast) or Player:PrevGCD(1, S.GlacialAdvance)))) and (Player:RuneTimeToX(4) > Player:GCD() or Cache.EnemiesCount[8] >= 2) then
  ...
  if S.Obliterate:IsCastable("Melee") and (Player:Buff(S.KillingMachine) or (Player:Buff(S.KillingMachine) and (Player:PrevGCD(1, S.FrostStrike) or Player:PrevGCD(1, S.HowlingBlast) or Player:PrevGCD(1, S.GlacialAdvance)))) then
    if HR.Cast(S.Obliterate) then return ""; end
  end
  ...
  if S.FrostStrike:IsReady(13) and (not Player:Buff(S.Rime) or Player:RunicPowerDeficit() < 10 or Player:RuneTimeToX(2) > Player:GCD()) and Cache.EnemiesCount[10] >= 2 then
  ...
  if S.HowlingBlast:IsCastable(30, true) and Player:Buff(S.Rime) and Cache.EnemiesCount[10] >= 2 then
  ...
  if S.HowlingBlast:IsCastable(30, true) and Player:Buff(S.Rime) then
    if HR.Cast(S.HowlingBlast) then return ""; end
  end
