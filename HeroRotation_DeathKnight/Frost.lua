...
local function Trinkets()
  if Settings.Commons.Enabled.Trinkets then
    ...
    -- use_item,slot=trinket1,if=variable.trinket_1_buffs&!variable.trinket_1_manual&(!talent.breath_of_sindragosa|buff.breath_of_sindragosa.up|cooldown.breath_of_sindragosa.remains>trinket.1.cooldown.remains%2)&(buff.breath_of_sindragosa.up|buff.pillar_of_frost.remains>variable.trinket_1_duration%2&!trinket.1.cast_time>0)&(!trinket.2.has_cooldown|trinket.2.cooldown.remains|variable.trinket_priority=1)
    if Trinket1:IsReady() and not VarTrinket1BL and (VarTrinket1Buffs and not VarTrinket1Manual and (not S.BreathofSindragosa:IsAvailable() or Player:BuffUp(S.BreathofSindragosa) or S.BreathofSindragosa:CooldownRemains() > Trinket1:CooldownRemains() / 2) and (Player:BuffUp(S.BreathofSindragosa) or Player:BuffRemains(S.PillarofFrostBuff) > VarTrinket1Duration / 2 and VarTrinket1CastTime == 0) and (not Trinket2:HasCooldown() or Trinket2:CooldownDown() or VarTrinketPriority == 1)) then
