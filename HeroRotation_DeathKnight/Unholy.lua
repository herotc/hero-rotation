...
local function AoEBurst()
  ...
  if WoundSpender:IsReady() then
    if Everyone.CastTargetIf(WoundSpender, EnemiesMelee, "min", EvaluateTargetIfFilterTrollbaneSlow, EvaluateTargetIfWoundSpenderAoEBurst2, not Target:IsSpellInRange(WoundSpender)) then return "wound_spender aoe_burst 6"; end
  end
  -- festering_strike,if=buff.festering_scythe.react
  if S.FesteringScytheAction:IsReady() then
    if Cast(S.FesteringScytheAction, nil, nil, not Target:IsInMeleeRange(14)) then return "festering_scythe aoe_burst 8"; end
  end
  -- epidemic
...
local function SanTrinkets()
  ...
  if Settings.Commons.Enabled.Trinkets then
    ...
    if Trinket1:IsReady() and not VarTrinket1BL and (not VarTrinket1Buffs and (VarDamageTrinketPriority == 1 or Trinket2:CooldownDown() or not Trinket2:HasCooldown() or not S.SummonGargoyle:IsAvailable() and not S.ArmyoftheDead:IsAvailable() and not S.RaiseAbomination:IsAvailable() or not S.SummonGargoyle:IsAvailable() and S.ArmyoftheDead:IsAvailable() and (not S.RaiseAbomination:IsAvailable() and S.ArmyoftheDead:CooldownRemains() > 20 or S.RaiseAbomination:IsAvailable() and S.RaiseAbomination:CooldownRemains() > 20) or not S.SummonGargoyle:IsAvailable() and not S.ArmyoftheDead:IsAvailable() and not S.RaiseAbomination:IsAvailable() and S.DarkTransformation:CooldownRemains() > 20 or S.SummonGargoyle:IsAvailable() and S.SummonGargoyle:CooldownRemains() > 20 and not VarGargActive) or BossFightRemains < 15) then
