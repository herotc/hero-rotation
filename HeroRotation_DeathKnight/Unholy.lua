...
local function Trinkets()
  if Settings.Commons.Enabled.Trinkets then
    ...
    -- use_item,name=cursed_stone_idol,if=cooldown.apocalypse.remains<gcd|pet.apoc_ghoul.active|!talent.apocalypse&buff.dark_transformation.up|cooldown.apocalypse.ready
    if I.CursedStoneIdol:IsEquippedAndReady() and (S.Apocalypse:CooldownRemains() < Player:GCD() or VarApocGhoulActive or not S.Apocalypse:IsAvailable() and Pet:BuffUp(S.DarkTransformation) or S.Apocalypse:CooldownUp()) then
      if Cast(I.CursedStoneIdol, nil, Settings.CommonsDS.DisplayStyle.Trinkets) then return "cursed_stone_idol trinkets 4"; end
    end
