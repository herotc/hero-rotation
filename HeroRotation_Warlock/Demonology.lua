  ...
  -- In Combat
    ...
    if Everyone.TargetIsValid() then
      if Target:IsInRange(range) then
        ...
        if S.Berserking:IsCastable() and (Target:TimeToDie() <= 15) then
          if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return ""; end
        end
        -- actions+=/blood_fury,if=pet.demonic_tyrant.active|target.time_to_die<=15
        if S.BloodFury:IsCastable() and (Target:TimeToDie() <= 15) then
          if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return ""; end
        end
        -- actions+=/fireblood,if=pet.demonic_tyrant.active|target.time_to_die<=15
        if S.Fireblood:IsCastable() and (Target:TimeToDie() <= 15) then
