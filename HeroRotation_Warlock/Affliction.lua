...
local function Aoe()
  ...
  if CDsON() and Player:Covenant() == "Venthyr" and Target:DebuffTicksRemain(S.ImpendingCatastrophe) > 0 and S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
  ...
  if CDsON() and Player:Covenant() == "Night Fae" and Target:DebuffTicksRemain(S.SoulRot) > 0 and S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
    local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
  end
  ...
  if CDsON() and Player:Covenant() ~= "Venthyr" and Player:Covenant() ~= "Night Fae" and Target:DebuffTicksRemain(S.PhantomSingularityDebuff) > 0 and Target:DebuffRemains(S.PhantomSingularityDebuff) < 2 then
    local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
  end
  ...
  if CDsON() and Player:Covenant() == "Venthyr" and (S.ImpendingCatastrophe:IsReady() or Target:DebuffTicksRemain(S.ImpendingCatastrophe) > 0) and S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
    local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
  end
  ...
  if CDsON() and Player:Covenant() ~= "Venthyr" and Player:Covenant() ~= "Night Fae" and S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
    local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
  end
  ...
  if CDsON() and Player:Covenant() == "Night Fae" and (S.SoulRot:IsReady() or Target:DebuffTicksRemain(S.SoulRot) > 0) and S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
    local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
  end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
   ...
   -- call_action_list,name=darkglare_prep,if=covenant.venthyr&dot.impending_catastrophe_dot.ticking&cooldown.summon_darkglare.remains<2&(dot.phantom_singularity.remains>2|!talent.phantom_singularity.enabled)
    ...
    if CDsON() and Enemies40yCount > 2 and Player:Covenant() == "Venthyr" and (S.ImpendingCatastrophe:IsReady() or Target:DebuffTicksRemain(S.ImpendingCatastrophe) > 0) and (Target:DebuffTicksRemain(S.PhantomSingularityDebuff) > 0 or not S.PhantomSingularity:IsAvailable()) then
      local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
    end
    ...
    if CDsON() and Enemies40yCount > 2 and Player:Covenant() ~= "Venthyr" and Player:Covenant() ~= "Night Fae" and (Target:DebuffTicksRemain(S.PhantomSingularityDebuff) > 0 or not S.PhantomSingularity:IsAvailable()) then
      local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
    end
    ...
    if CDsON() and Enemies40yCount > 2 and Player:Covenant() == "Night Fae" and (S.SoulRot:IsReady() or Target:DebuffTicksRemain(S.SoulRot) > 0) and (Target:DebuffTicksRemain(S.PhantomSingularityDebuff) > 0 or not S.PhantomSingularity:IsAvailable()) then
      local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
    end
    -- seed_of_corruption,if=active_enemies>2&talent.sow_the_seeds.enabled&!dot.seed_of_corruption.ticking&!in_flight
    if S.SeedofCorruption:IsReady() and (Enemies40yCount > 2 and S.SowtheSeeds:IsAvailable() and Target:DebuffDown(S.SeedofCorruptionDebuff) and not S.SeedofCorruption:InFlight()) then
    ...
    if S.SeedofCorruption:IsReady() and (Enemies40yCount > 2 and S.SiphonLife:IsAvailable() and Target:DebuffDown(S.SeedofCorruptionDebuff) and not S.SeedofCorruption:InFlight() and Target:DebuffRemains(S.CorruptionDebuff) < 4) then
    ...
    if S.Corruption:IsReady() and Enemies40yCount < 4 - num(S.SowtheSeeds:IsAvailable() or S.SiphonLife:IsAvailable()) and Target:DebuffRemains(S.CorruptionDebuff) < 2 then
    ...
    if S.Corruption:IsReady() and Enemies40yCount < 4 - num(S.SowtheSeeds:IsAvailable() or S.SiphonLife:IsAvailable()) then
    ...
    if S.Corruption:IsReady() and (Target:DebuffRefreshable(S.CorruptionDebuff) and Enemies40yCount < 4 - num(S.SowtheSeeds:IsAvailable() or S.SiphonLife:IsAvailable())) then
