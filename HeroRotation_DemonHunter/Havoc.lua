...
local function RetargetAutoAttack(Spell, Enemies, Mode, Condition, OutofRange)
  ...
  elseif Mode == "max" then
    ...
    local BestRemains = 0
    for _, CycleUnit in pairs(Enemies) do
      if CycleUnit:DebuffRemains(S.BurningWoundDebuff) > BestRemains then
        BestTarget = CycleUnit
        BestRemains = CycleUnit:DebuffRemains(S.BurningWoundDebuff)
      end
    end
    if BestTarget and BestTarget:GUID() ~= TargetGUID then
      HR.CastLeftNameplate(BestTarget, Spell)
      LastRetargetTime = GetTime()
    end
  elseif Mode == "nonboss" then
    -- Prefer non-boss targets
    for _, CycleUnit in pairs(Enemies) do
      if CycleUnit:GUID() ~= TargetGUID and not CycleUnit:IsInBossList() then
        HR.CastLeftNameplate(CycleUnit, Spell)
        LastRetargetTime = GetTime()
        break
      end
...
local function HighestReaversMark(Enemies)
  local HighRM = 0
  local HighestUnit
  for _, CycleUnit in pairs(Enemies) do
    local CurrentRM = CycleUnit:DebuffRemains(S.ReaversMarkDebuff)
    if CurrentRM > HighRM then
      HighRM = CurrentRM
      HighestUnit = CycleUnit
    end
  end
  return HighestUnit or Target
end

local function CastSwitch(TargetUnit, Spell, OffGCD, DisplayStyle, OutofRange)
  if TargetUnit:GUID() == Target:GUID() then
    return Cast(Spell, OffGCD, DisplayStyle, OutofRange)
  else
    HR.CastLeftNameplate(TargetUnit, Spell)
  end
end
...
local function ETIFReaversMark(TargetUnit)
  -- target_if=max:debuff.reavers_mark.remains
  return TargetUnit:DebuffRemains(S.ReaversMarkDebuff)
end

local function ETIFHealthPercent(TargetUnit)
  -- target_if=max:target.health_pct
  return TargetUnit:HealthPercentage()
end

--- ===== CastTargetIf Condition Functions =====
local function ETIAnnihilationARMeta(TargetUnit)
  -- if=buff.metamorphosis.remains<gcd.max|debuff.essence_break.remains&debuff.essence_break.remains<0.5&cooldown.blade_dance.remains|buff.inner_demon.up&cooldown.essence_break.up&cooldown.metamorphosis.up
  return Player:BuffRemains(S.MetamorphosisBuff) < Player:GCD() or TargetUnit:DebuffUp(S.EssenceBreakDebuff) and TargetUnit:DebuffRemains(S.EssenceBreakDebuff) < 0.5 and S.BladeDance:CooldownDown() or Player:BuffUp(S.InnerDemonBuff) and S.EssenceBreak:CooldownUp() and S.Metamorphosis:CooldownUp()
end

local function ETIAnnihilationARMeta2(TargetUnit)
  -- if=cooldown.blade_dance.remains|fury>60|soul_fragments.total>0|buff.metamorphosis.remains<5&cooldown.felblade.up|debuff.essence_break.up
  local TotalSoulFragments = DemonHunter.Souls.AuraSouls + DemonHunter.Souls.IncomingSouls
  return S.BladeDance:CooldownDown() or Player:Fury() > 60 or TotalSoulFragments > 0 or Player:BuffRemains(S.MetamorphosisBuff) < 5 and S.Felblade:CooldownUp() or TargetUnit:DebuffUp(S.EssenceBreakDebuff)
end

local function ETIDeathSweepARMeta(TargetUnit)
  -- if=buff.metamorphosis.remains<gcd.max|debuff.essence_break.up|cooldown.metamorphosis.up&!talent.restless_hunter
  return Player:BuffRemains(S.MetamorphosisBuff) < Player:GCD() or TargetUnit:DebuffUp(S.EssenceBreakDebuff) or S.Metamorphosis:CooldownUp() and not S.RestlessHunter:IsAvailable()
end

local function ETIEssenceBreakDown(TargetUnit)
  return TargetUnit:DebuffDown(S.EssenceBreakDebuff)
end

local function ETIEssenceBreakUp(TargetUnit)
  return TargetUnit:DebuffUp(S.EssenceBreakDebuff)
end

local function ETIReaverDown(TargetUnit)
  return TargetUnit:DebuffDown(S.ReaversMarkDebuff)
end

local function ETIReaverUp(TargetUnit)
  return TargetUnit:DebuffUp(S.ReaversMarkDebuff)
end

local function ETIThrowGlaiveAR(TargetUnit)
  -- if=buff.unbound_chaos.down&recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&buff.out_of_range.down&active_enemies>1&talent.furious_throws
  -- Note: Non-target items handled before CastTargetIf.
  return TargetUnit:DebuffDown(S.EssenceBreakDebuff) and TargetUnit:IsInRange(8)
end

local function ETIThrowGlaiveARMeta(TargetUnit)
  -- if=talent.soulscar&talent.furious_throws&active_enemies=3&debuff.essence_break.down&(charges=2|full_recharge_time<cooldown.blade_dance.remains)
  return S.Soulscar:IsAvailable() and S.FuriousThrows:IsAvailable() and Enemies8yCount == 3 and TargetUnit:DebuffDown(S.EssenceBreakDebuff) and (S.ThrowGlaive:Charges() == 2 or S.ThrowGlaive:FullRechargeTime() < S.BladeDance:CooldownRemains())
end

local function ETIThrowGlaiveARMeta2(TargetUnit)
  -- if=buff.unbound_chaos.down&recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&buff.out_of_range.down&active_enemies>1&!talent.furious_throws
  return Player:BuffDown(S.UnboundChaosBuff) and S.ThrowGlaive:Recharge() < BeamAbility:CooldownRemains() and Target:DebuffDown(S.EssenceBreakDebuff) and (BeamAbility:CooldownRemains() > 8 or S.ThrowGlaive:ChargesFractional() > 1.01) and Target:IsInRange(30) and Enemies8yCount > 1 and not S.FuriousThrows:IsAvailable()
end
...
local function FSCooldown()
  ...
  if CDsON() and S.Metamorphosis:IsCastable() and ((((BeamAbility:CooldownRemains() >= 20 or S.CycleofHatred:IsAvailable() and BeamAbility:CooldownRemains() >= 13) and (not S.EssenceBreak:IsAvailable() or Target:DebuffUp(S.EssenceBreakDebuff)) and Player:BuffDown(S.FelBarrageBuff) or BossFightRemains < 30) and Player:BuffDown(S.InnerDemonBuff) and (not S.RestlessHunter:IsAvailable() and S.BladeDance:CooldownRemains() > Player:GCD() * 3 or Player:PrevGCD(1, S.DeathSweep))) and not S.Inertia:IsAvailable() and not S.EssenceBreak:IsAvailable() and CombatTime > 15) then
  ...
  if Settings.Commons.Enabled.Trinkets then
    ...
    if I.ChaoticNethergate:IsEquippedAndReady() and (VarT334P and VarDoubleOnUse or BossFightRemains < 15) then
      if Cast(I.ChaoticNethergate, nil, Settings.CommonsDS.DisplayStyle.Trinkets) then return "chaotic_nethergate fs_cooldown 21"; end
    end
    ...
    -- use_item,name=unyielding_netherprism,if=(((cooldown.eye_beam.remains<gcd.max&(active_enemies>1|talent.looks_can_kill)|!talent.chaotic_transformation&buff.metamorphosis.up)&((trinket.1.is.unyielding_netherprism&trinket.2.cooldown.duration>90&variable.trinket2_steroids|cooldown.metamorphosis.remains<=5&buff.latent_energy.stack>10)|(trinket.2.is.unyielding_netherprism&trinket.1.cooldown.duration>90&variable.trinket1_steroids|cooldown.metamorphosis.remains<=5&buff.latent_energy.stack>10))|(buff.metamorphosis.up&((trinket.1.is.unyielding_netherprism&trinket.2.cooldown.duration>90&variable.trinket2_steroids)|(trinket.2.is.unyielding_netherprism&trinket.1.cooldown.duration>90&variable.trinket1_steroids)&!equipped.improvised_seaforium_pacemaker&!equipped.soleahs_secret_technique)))&(raid_event.adds.in>105|raid_event.adds.remains>8)|fight_remains<25)&((trinket.1.is.unyielding_netherprism&(!variable.trinket2_steroids|trinket.2.cooldown.duration<120|trinket.2.cooldown.remains>20)|trinket.2.is.unyielding_netherprism&(!variable.trinket1_steroids|trinket.1.cooldown.duration<120|trinket.1.cooldown.remains>20))|equipped.improvised_seaforium_pacemaker|equipped.soleahs_secret_technique)
    if I.UnyieldingNetherprism:IsEquippedAndReady() and ((((BeamAbility:CooldownRemains() < Player:GCD() and (Enemies8yCount > 1 or S.LooksCanKill:IsAvailable()) or not S.ChaoticTransformation:IsAvailable() and Player:BuffUp(S.MetamorphosisBuff)) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and VarTrinket2CD > 90 and VarTrinket2Steroids or S.Metamorphosis:CooldownRemains() <= 5 and Player:BuffStack(S.LatentEnergyBuff) > 10) or (VarTrinket2ID == I.UnyieldingNetherprism:ID() and VarTrinket1CD > 90 and VarTrinket1Steroids or S.Metamorphosis:CooldownRemains() <= 5 and Player:BuffStack(S.LatentEnergyBuff) > 10)) or (Player:BuffUp(S.MetamorphosisBuff) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and VarTrinket2CD > 90 and VarTrinket2Steroids) or (VarTrinket2ID == I.UnyieldingNetherprism:ID() and VarTrinket1CD > 90 and VarTrinket1Steroids) and not I.ImprovisedSeaforiumPacemaker:IsEquipped() and not I.SoleahsSecretTechnique:IsEquipped()))) or BossFightRemains < 25) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and (not VarTrinket2Steroids or VarTrinket2CD < 120 or Trinket2:CooldownRemains() > 20) or VarTrinket2ID == I.UnyieldingNetherprism:ID() and (not VarTrinket1Steroids or VarTrinket1CD < 120 or Trinket1:CooldownRemains() > 20)) or I.ImprovisedSeaforiumPacemaker:IsEquipped() or I.SoleahsSecretTechnique:IsEquipped())) then
      if Cast(I.UnyieldingNetherprism, nil, Settings.CommonsDS.DisplayStyle.Trinkets) then return "unyielding_netherprism fs_cooldown 34"; end
    end
  ...
  if CDsON() and S.SigilofSpite:IsCastable() and (Target:DebuffDown(S.EssenceBreakDebuff) and S.BladeDance:CooldownDown() and CombatTime > 15) then
...
local function FSFelBarrage()
  ...
  -- sigil_of_flame,if=buff.demonsurge_hardcast.up&fury.deficit>40&buff.fel_barrage.up
  if SigilAbility:IsCastable() and (Player:Demonsurge("Hardcast") and Player:FuryDeficit() > 40 and Player:BuffUp(S.FelBarrageBuff)) then
...
local function FSMeta()
  ...
  if S.DeathSweep:IsReady() and (Player:BuffRemains(S.MetamorphosisBuff) < Player:GCD() or Target:DebuffUp(S.EssenceBreakDebuff) and ((Player:BuffDown(S.ImmolationAuraBuff) or not TWW3_2pc) and (Player:BuffDown(S.DemonSoulBuff) or not TWW3_4pc) or S.DancingWithFate:TalentRank() == 2) or Player:PrevGCD(1, S.Metamorphosis) and (not TWW3_2pc or S.DancingWithFate:TalentRank() == 2) or Player:Demonsurge("DeathSweep") and TWW3_2pc and Player:BuffRemains(S.DemonsurgeBuff) < 5 or (TWW3_2pc and S.Metamorphosis:CooldownUp() and S.Inertia:IsAvailable()) or Enemies8yCount >= 3 and Player:Demonsurge("DeathSweep") and (not S.Inertia:IsAvailable() or not InertiaTrigger() and S.VengefulRetreat:CooldownDown() or Player:BuffUp(S.InertiaBuff)) and (not S.EssenceBreak:IsAvailable() or Target:DebuffUp(S.EssenceBreakDebuff) or S.EssenceBreak:CooldownRemains() >= 5)) then
  ...
  -- sigil_of_flame,if=buff.demonsurge_hardcast.up&buff.demonsurge_sigil_of_doom.up&(buff.demonsurge.remains<5|cooldown.blade_dance.remains&buff.metamorphosis.remains<=gcd.max)&!debuff.essence_break.up
  if SigilAbility:IsCastable() and (Player:Demonsurge("Hardcast") and Player:Demonsurge("SigilofDoom") and (Player:BuffRemains(S.DemonsurgeBuff) < 5 or S.BladeDance:CooldownDown() and Player:BuffRemains(S.MetamorphosisBuff) <= Player:GCD()) and Target:DebuffDown(S.EssenceBreakDebuff)) then
    if Cast(SigilAbility, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then return "sigil_ability fs_meta 5"; end
  end
  ...
  if S.VengefulRetreat:IsCastable() and (TWW3_2pc and not InertiaTrigger() and S.Initiative:IsAvailable()) then
  ...
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and TWW3_2pc and InertiaTrigger()) then
  ...
  if S.DeathSweep:IsReady() and ((S.EssenceBreak:IsAvailable() and Player:Demonsurge("DeathSweep") and (Player:BuffUp(S.InertiaBuff) and (S.EssenceBreak:CooldownRemains() > Player:BuffRemains(S.InertiaBuff) or not S.EssenceBreak:IsAvailable()) or S.Metamorphosis:CooldownRemains() <= 5 and not InertiaTrigger() or Player:BuffUp(S.InertiaBuff) and Player:Demonsurge("AbyssalGaze")) or S.Inertia:IsAvailable() and not InertiaTrigger() and S.VengefulRetreat:CooldownRemains() >= Player:GCD() and Player:BuffDown(S.InertiaBuff)) and (not TWW3_2pc or (TWW3_2pc and (not S.Inertia:IsAvailable() or Enemies8yCount >= 3 and Target:DebuffUp(S.EssenceBreakDebuff) or S.DancingWithFate:TalentRank() == 2)))) then
  ...
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and Target:DebuffDown(S.EssenceBreakDebuff) and S.Metamorphosis:CooldownDown() and BeamAbility:CooldownDown() and (S.BladeDance:CooldownRemains() <= 5.5 and (S.EssenceBreak:IsAvailable() and S.EssenceBreak:CooldownRemains() <= 0.5 or not S.EssenceBreak:IsAvailable() or S.EssenceBreak:CooldownRemains() >= Player:BuffRemains(S.UnboundChaosBuff) and S.BladeDance:CooldownRemains() <= 4.5 and (S.BladeDance:CooldownDown() or S.BladeDance:CooldownRemains() <= 0.5)) or Player:BuffRemains(S.MetamorphosisBuff) <= 5.5 + num(S.ShatteredDestiny:IsAvailable()) * 2)) then
  ...
  if S.FelRush:IsCastable() and UseFelRush() and (InertiaTrigger() and S.Inertia:IsAvailable() and Target:DebuffDown(S.EssenceBreakDebuff) and S.Metamorphosis:CooldownDown() and BeamAbility:CooldownDown() and (S.Felblade:CooldownDown() and S.EssenceBreak:CooldownRemains() <= 0.6 or Enemies8yCount > 2)) then
  ...
  if ImmoAbility:IsCastable() and ((Enemies8yCount > 1 or S.AFireInside:IsAvailable() and (S.IsolatedPrey:IsAvailable() or TWW3_2pc)) and Target:DebuffDown(S.EssenceBreakDebuff) and (Enemies8yCount >= 3 or ImmoAbility:FullRechargeTime() < Player:GCD() * 2 or TWW3_2pc and Player:BuffRemains(S.ImmolationAuraBuff) <= Player:GCD() or TWW3_2pc and Player:BuffDown(S.ImmolationAuraBuff))) then
  ...
  if S.EssenceBreak:IsCastable() and (Player:Fury() > 20 and (S.Metamorphosis:CooldownRemains() > 10 or S.BladeDance:CooldownRemains() < Player:GCD() * 2 and not TWW3_2pc or TWW3_2pc and Player:BuffUp(S.ImmolationAuraBuff)) and (not InertiaTrigger() or Player:BuffUp(S.InertiaBuff) and Player:BuffRemains(S.InertiaBuff) >= Player:GCD() * 3 or not S.Inertia:IsAvailable() or Enemies8yCount > 1) and (not S.ShatteredDestiny:IsAvailable() or BeamAbility:CooldownRemains() > 4) and (Enemies8yCount > 1 or S.Metamorphosis:CooldownRemains() > 5 and BeamAbility:CooldownDown()) and (Player:BuffStack(S.CycleofHatredBuff) ~= 3 or Player:BuffUp(S.InitiativeBuff) or not S.Initiative:IsAvailable() or not S.CycleofHatred:IsAvailable() or S.Inertia:IsAvailable()) or BossFightRemains < 5) then
  ...
  if SigilAbility:IsCastable() and (Player:Demonsurge("Hardcast") and not Player:Demonsurge("DeathSweep") and Target:DebuffDown(S.EssenceBreakDebuff) and (BeamAbility:CooldownRemains() >= 20 or BeamAbility:CooldownRemains() <= Player:GCD()) and (not S.StudentofSuffering:IsAvailable() or Player:Demonsurge("SigilofDoom"))) then
  ...
  if ImmoAbility:IsReady() and (not TWW3_2pc and Player:BuffUp(S.DemonsurgeBuff) and Target:DebuffDown(S.EssenceBreakDebuff) and Player:Demonsurge("ConsumingFire") and S.BladeDance:CooldownRemains() >= Player:GCD() and BeamAbility:CooldownRemains() >= Player:GCD() and Player:FuryDeficit() > 10 + VarFuryGen) then
  ...
  if BeamAbility:IsReady() and (Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.InnerDemonBuff) and (Player:BuffRemains(S.MetamorphosisBuff) >= 7 or not TWW3_4pc)) then
  ...
  if BeamAbility:IsReady() and (Player:Demonsurge("Hardcast") and Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.InnerDemonBuff) and (Player:BuffStack(S.CycleofHatredBuff) < 4 or S.EssenceBreak:CooldownRemains() >= 20 - Player:GCD() * num(S.StudentofSuffering:IsAvailable()) or SigilAbility:CooldownDown() and S.StudentofSuffering:IsAvailable() or S.EssenceBreak:CooldownRemains() <= Player:GCD() or not S.EssenceBreak:IsAvailable()) and (Player:BuffRemains(S.MetamorphosisBuff) >= 7 or not TWW3_4pc)) then
    if Cast(BeamAbility, Settings.Havoc.GCDasOffGCD.EyeBeam, nil, not IsInMeleeRange(20)) then return "beam_gaze fs_meta 34"; end
  end
  -- death_sweep,if=(cooldown.essence_break.remains>=gcd.max*2+talent.student_of_suffering*gcd.max|debuff.essence_break.up|!talent.essence_break)&(buff.immolation_aura.down|!variable.fs_tier34_2piece|talent.screaming_brutality&talent.soulscar|talent.dancing_with_fate.rank=2)&(buff.demon_soul_tww3.down|!set_bonus.thewarwithin_season_3_4pc|active_enemies>=3|talent.screaming_brutality&talent.soulscar|talent.dancing_with_fate.rank=2)
  if S.DeathSweep:IsReady() and ((S.EssenceBreak:CooldownRemains() >= Player:GCD() * 2 + num(S.StudentofSuffering:IsAvailable()) * Player:GCD() or Target:DebuffUp(S.EssenceBreakDebuff) or not S.EssenceBreak:IsAvailable()) and (Player:BuffDown(S.ImmolationAuraBuff) or not TWW3_2pc or S.ScreamingBrutality:IsAvailable() and S.Soulscar:IsAvailable() or S.DancingWithFate:TalentRank() == 2) and (Player:BuffDown(S.DemonSoulBuff) or not TWW3_4pc or Enemies8yCount >= 3 or S.ScreamingBrutality:IsAvailable() and S.Soulscar:IsAvailable() or S.DancingWithFate:TalentRank() == 2)) then
  ...
  if ImmoAbility:IsReady() and (not TWW3_2pc and Target:IsInRange(8) and ImmoAbility:Recharge() < math.max(BeamAbility:CooldownRemains(), Player:BuffRemains(S.MetamorphosisBuff))) then
...
local function FSOpener()
  ...
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and TWW3_4pc and Player:BuffUp(S.MetamorphosisBuff) and Target:DebuffDown(S.EssenceBreakDebuff) and Enemies8yCount <= 2) then
  ...
  if S.FelRush:IsCastable() and UseFelRush() and (S.Inertia:IsAvailable() and InertiaTrigger() and TWW3_4pc and Player:BuffUp(S.MetamorphosisBuff) and Target:DebuffDown(S.EssenceBreakDebuff) and (Enemies8yCount >= 2 or S.Felblade:CooldownDown())) then
  ...
  if ImmoAbility:IsCastable() and (TWW3_2pc and Player:Demonsurge("Hardcast") and (Player:Demonsurge("ConsumingFire") or ImmoAbility:Charges() == 2)) then
  ...
  if S.Annihilation:IsReady() and (TWW3_2pc and Target:DebuffDown(S.EssenceBreakDebuff) and S.Metamorphosis:CooldownDown() and Player:Demonsurge("Annihilation") and BeamAbility:CooldownUp()) then
  ...
  if S.EssenceBreak:IsCastable() and (Player:BuffUp(S.MetamorphosisBuff) and (not S.Inertia:IsAvailable() or Player:BuffUp(S.InertiaBuff) and (Player:BuffDown(S.InnerDemonBuff) or not S.ChaoticTransformation:IsAvailable())) and (not Player:Demonsurge("Annihilation") or not S.ChaoticTransformation:IsAvailable()) and (not TWW3_2pc or Player:Demonsurge("Hardcast") and BeamAbility:CooldownDown() and not Player:Demonsurge("ConsumingFire"))) then
  ...
  if S.VengefulRetreat:IsCastable() and (S.Initiative:IsAvailable() and CombatTime > 4 and Player:BuffUp(S.MetamorphosisBuff) and (not S.Inertia:IsAvailable() or not InertiaTrigger()) and S.EssenceBreak:IsAvailable() and Player:BuffDown(S.InnerDemonBuff) and (Player:BuffDown(S.InitiativeBuff) or Player:GCDRemains() < 0.1) and S.BladeDance:CooldownDown()) then
  ...
  -- sigil_of_flame,if=buff.demonsurge_hardcast.up&(buff.inner_demon.down|buff.out_of_range.up)&debuff.essence_break.down&(!variable.fs_tier34_2piece|cooldown.essence_break.remains&buff.inertia.down&(!talent.inertia|buff.immolation_aura.down)|!talent.essence_break|fury<=40)
  if SigilAbility:IsCastable() and (Player:Demonsurge("Hardcast") and (Player:BuffDown(S.InnerDemonBuff) or not Target:IsInRange(8)) and Target:DebuffDown(S.EssenceBreakDebuff) and (not TWW3_2pc or S.EssenceBreak:CooldownDown() and Player:BuffDown(S.InertiaBuff) and (not S.Inertia:IsAvailable() or Player:BuffDown(S.ImmolationAuraBuff)) or not S.EssenceBreak:IsAvailable() or Player:Fury() <= 40)) then
  ...
  if S.DeathSweep:IsReady() and (Player:Demonsurge("DeathSweep") and not S.RestlessHunter:IsAvailable() and (not TWW3_2pc or not Player:Demonsurge("Hardcast"))) then
  ...
  -- eye_beam,if=buff.demonsurge_hardcast.up&debuff.essence_break.down&buff.inner_demon.down
  if BeamAbility:IsReady() and (Player:Demonsurge("Hardcast") and Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.InnerDemonBuff)) then
  ...
  if S.Annihilation:IsReady() and (TWW3_2pc and (Player:BuffUp(S.ImmolationAuraBuff) or Player:BuffUp(S.DemonSoulBuff))) then
...
local function FS()
  ...
  if S.VengefulRetreat:IsCastable() and (S.Initiative:IsAvailable() and (BeamAbility:CooldownRemains() > 15 and Player:GCDRemains() < 0.3 or Player:GCDRemains() < 0.2 and BeamAbility:CooldownRemains() <= Player:GCDRemains() and (S.Metamorphosis:CooldownRemains() > 10 or S.BladeDance:CooldownRemains() < Player:GCD() * 3)) and (not S.StudentofSuffering:IsAvailable() or SigilAbility:CooldownDown()) and (S.EssenceBreak:CooldownRemains() <= Player:GCD() * 2 and S.StudentofSuffering:IsAvailable() and SigilAbility:CooldownDown() or S.EssenceBreak:CooldownRemains() >= 18 or not S.StudentofSuffering:IsAvailable()) and S.Metamorphosis:CooldownRemains() > 10 and CombatTime > 20 and (not S.Inertia:IsAvailable() or not InertiaTrigger())) then
  ...
  if ImmoAbility:IsReady() and (
    -- immolation_aura,if=variable.fs_tier34_2piece&(full_recharge_time<gcd.max*3|buff.immolation_aura.down&(cooldown.eye_beam.remains<3&(!talent.essence_break|buff.cycle_of_hatred.stack<4)|talent.essence_break&cooldown.essence_break.remains<=5|talent.essence_break&((cooldown.eye_beam.remains<3)*cooldown.essence_break.remains)>recharge_time))&(!talent.dancing_with_fate.rank=2|cooldown.blade_dance.remains>=gcd.max|cooldown.eye_beam.remains<3)
    (TWW3_2pc and (ImmoAbility:FullRechargeTime() < Player:GCD() * 3 or Player:BuffDown(S.ImmolationAuraBuff) and (BeamAbility:CooldownRemains() < 3 and (not S.EssenceBreak:IsAvailable() or Player:BuffStack(S.CycleofHatredBuff) < 4) or S.EssenceBreak:IsAvailable() and S.EssenceBreak:CooldownRemains() <= 5 or S.EssenceBreak:IsAvailable() and (num(BeamAbility:CooldownRemains() < 3) * S.EssenceBreak:CooldownRemains()) > ImmoAbility:Recharge())) and (S.DancingWithFate:TalentRank() ~= 2 or S.BladeDance:CooldownRemains() >= Player:GCD() or S.EyeBeam:CooldownRemains() < 3)) or
    -- immolation_aura,if=variable.fs_tier34_2piece&((cooldown.eye_beam.remains+cooldown.metamorphosis.remains)<10)&(!talent.dancing_with_fate.rank=2|cooldown.blade_dance.remains>=gcd.max|cooldown.eye_beam.remains<3)
    (TWW3_2pc and ((BeamAbility:CooldownRemains() + S.Metamorphosis:CooldownRemains()) < 10) and (S.DancingWithFate:TalentRank() ~= 2 or S.BladeDance:CooldownRemains() >= Player:GCD() or S.EyeBeam:CooldownRemains() < 3)) or
  ...
  -- felblade,if=variable.fs_tier34_2piece&talent.inertia&buff.inertia_trigger.up&(buff.immolation_aura.up|buff.inertia_trigger.remains<=0.5|cooldown.the_hunt.remains<=0.5|active_enemies>1&cooldown.eye_beam.remains<=0.5)&active_enemies<=2
  -- Note: Added 0.5s extra to the checks for player latency.
  if S.Felblade:IsCastable() and (TWW3_2pc and S.Inertia:IsAvailable() and InertiaTrigger() and (Player:BuffUp(S.ImmolationAuraBuff) or Player:BuffRemains(S.UnboundChaosBuff) <= 1 or S.TheHunt:CooldownRemains() <= 1 or Enemies8yCount > 1 and BeamAbility:CooldownRemains() <= 1) and Enemies8yCount <= 2) then
    if Cast(S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade fs 16"; end
  end
  -- fel_rush,if=variable.fs_tier34_2piece&talent.inertia&buff.inertia_trigger.up&(buff.immolation_aura.up|buff.inertia_trigger.remains<=gcd.max|cooldown.the_hunt.remains<=gcd.max|active_enemies>1&cooldown.eye_beam.remains<=gcd)&(active_enemies>2|cooldown.felblade.remains)
  ...
  if S.FelRush:IsCastable() and UseFelRush() and (TWW3_2pc and S.Inertia:IsAvailable() and InertiaTrigger() and (Player:BuffUp(S.ImmolationAuraBuff) or Player:BuffRemains(S.UnboundChaosBuff) <= Player:GCD() or S.TheHunt:CooldownRemains() <= Player:GCD() or Enemies8yCount > 1 and BeamAbility:CooldownRemains() <= Player:GCD()) and (Enemies8yCount > 2 or S.Felblade:CooldownDown())) then
  ...
  -- blade_dance,if=cooldown.eye_beam.remains>=gcd.max*4&(active_enemies>3|talent.screaming_brutality&talent.soulscar|!variable.fs_tier34_2piece|variable.fs_tier34_2piece&(talent.dancing_with_fate.rank=2|buff.immolation_aura.down&!debuff.essence_break.up))
  if S.BladeDance:IsReady() and (BeamAbility:CooldownRemains() >= Player:GCD() * 4 and (Enemies8yCount > 3 or S.ScreamingBrutality:IsAvailable() and S.Soulscar:IsAvailable() or not TWW3_2pc or TWW3_2pc and (S.DancingWithFate:TalentRank() == 2 or Player:BuffDown(S.ImmolationAuraBuff) and Target:DebuffDown(S.EssenceBreakDebuff)))) then
    if Cast(S.BladeDance, nil, nil, not IsInMeleeRange(8)) then return "blade_dance fs 22"; end
  end
  -- chaos_strike,if=variable.fs_tier34_2piece&buff.immolation_aura.up&((cooldown.eye_beam.remains>=gcd.max*4|(fury>=70-30*(talent.student_of_suffering&(cooldown.sigil_of_flame.remains<=gcd.max|cooldown.sigil_of_flame.up))-buff.chaos_theory.up*20-variable.fury_gen))|talent.blind_fury)
  if S.ChaosStrike:IsReady() and (TWW3_2pc and Player:BuffUp(S.ImmolationAuraBuff) and ((BeamAbility:CooldownRemains() >= Player:GCD() * 4 or (Player:Fury() >= 70 - 30 * num(S.StudentofSuffering:IsAvailable() and (SigilAbility:CooldownRemains() <= Player:GCD() or SigilAbility:CooldownUp())) - num(Player:BuffUp(S.ChaosTheoryBuff)) * 20 - VarFuryGen)) or S.BlindFury:IsAvailable())) then
  ...
  if ImmoAbility:IsReady() and (not TWW3_2pc and BeamAbility:CooldownRemains() >= Player:GCD() * (1 + num(S.StudentofSuffering:IsAvailable() and (SigilAbility:CooldownRemains() <= Player:GCD() or SigilAbility:CooldownUp()))) or Enemies8yCount > 2) then
...
local function ARCooldown()
  ...
  if Settings.Commons.Enabled.Trinkets then
    ...
    -- use_item,name=chaotic_nethergate,if=variable.tier33_4piece&variable.double_on_use|fight_remains<15
    if I.ChaoticNethergate:IsEquippedAndReady() and (VarT334P and VarDoubleOnUse or BossFightRemains < 15) then
      if Cast(I.ChaoticNethergate, nil, Settings.CommonsDS.DisplayStyle.Trinkets) then return "chaotic_nethergate ar_cooldown 21"; end
    end
    -- use_item,name=ratfang_toxin,if=variable.tier33_4piece&variable.double_on_use|fight_remains<5
    if I.RatfangToxin:IsEquippedAndReady() and (VarT334P and VarDoubleOnUse or BossFightRemains < 5) then
    ...
    -- do_treacherous_transmitter_task,if=cooldown.eye_beam.remains>15|cooldown.eye_beam.remains<5|fight_remains<20|buff.metamorphosis.up
    -- use_item,name=unyielding_netherprism,if=((cooldown.eye_beam.remains<gcd.max&(active_enemies>1|talent.looks_can_kill)&((trinket.1.is.unyielding_netherprism&trinket.2.cooldown.duration>90&variable.trinket2_steroids|cooldown.metamorphosis.remains<=5&buff.latent_energy.stack>10)|(trinket.2.is.unyielding_netherprism&trinket.1.cooldown.duration>90&variable.trinket1_steroids|cooldown.metamorphosis.remains<=5&buff.latent_energy.stack>10))|(buff.metamorphosis.up&((trinket.1.is.unyielding_netherprism&trinket.2.cooldown.duration>90&variable.trinket2_steroids)|(trinket.2.is.unyielding_netherprism&trinket.1.cooldown.duration>90&variable.trinket1_steroids)&!equipped.improvised_seaforium_pacemaker&!equipped.soleahs_secret_technique)))&(raid_event.adds.in>105|raid_event.adds.remains>8)|fight_remains<25)&((trinket.1.is.unyielding_netherprism&(!variable.trinket2_steroids|trinket.2.cooldown.duration<120|trinket.2.cooldown.remains>20)|trinket.2.is.unyielding_netherprism&(!variable.trinket1_steroids|trinket.1.cooldown.duration<120|trinket.1.cooldown.remains>20))|equipped.improvised_seaforium_pacemaker|equipped.soleahs_secret_technique)
    if I.UnyieldingNetherprism:IsEquippedAndReady() and (((BeamAbility:CooldownRemains() < Player:GCD() and (Enemies8yCount > 1 or S.LooksCanKill:IsAvailable()) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and VarTrinket2CD > 90 and VarTrinket2Steroids or S.Metamorphosis:CooldownRemains() <= 5 and Player:BuffStack(S.LatentEnergyBuff) > 10) or (VarTrinket2ID == I.UnyieldingNetherprism:ID() and VarTrinket1CD > 90 and VarTrinket1Steroids or S.Metamorphosis:CooldownRemains() <= 5 and Player:BuffStack(S.LatentEnergyBuff) > 10)) or (Player:BuffUp(S.MetamorphosisBuff) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and VarTrinket2CD > 90 and VarTrinket2Steroids) or (VarTrinket2ID == I.UnyieldingNetherprism:ID() and VarTrinket1CD > 90 and VarTrinket1Steroids) and not I.ImprovisedSeaforiumPacemaker:IsEquipped() and not I.SoleahsSecretTechnique:IsEquipped()))) or BossFightRemains < 25) and ((VarTrinket1ID == I.UnyieldingNetherprism:ID() and (not VarTrinket2Steroids or VarTrinket2CD < 120 or Trinket2:CooldownRemains() > 20) or VarTrinket2ID == I.UnyieldingNetherprism:ID() and (not VarTrinket1Steroids or VarTrinket1CD < 120 or Trinket1:CooldownRemains() > 20)) or I.ImprovisedSeaforiumPacemaker:IsEquipped() or I.SoleahsSecretTechnique:IsEquipped())) then
  ...
  if CDsON() and S.TheHunt:IsCastable() and (Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.GlaiveFlurryBuff) and (Player:BuffRemains(S.MetamorphosisBuff) > 5 or Player:BuffDown(S.MetamorphosisBuff)) and (not S.Initiative:IsAvailable() or Player:BuffUp(S.InitiativeBuff) or CombatTime > 5) and CombatTime > 5 and (not S.Inertia:IsAvailable() and Player:BuffDown(S.UnboundChaosBuff) or not InertiaTrigger()) or BossFightRemains <= 30) then
...
local function ARFelBarrage()
  ...
  -- variable,name=gcd_drain,op=set,value=gcd.max*32
  local VarGCDDrain = Player:GCD() * 32
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=buff.inner_demon.up
  if S.Annihilation:IsReady() and (Player:BuffUp(S.InnerDemonBuff)) then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_fel_barrage 2"; end
  end
  -- eye_beam,if=(buff.fel_barrage.down|fury>45&talent.blind_fury)&(active_enemies>1&raid_event.adds.up|raid_event.adds.in>40-buff.cycle_of_hatred.stack*5)
  if BeamAbility:IsReady() and (Player:BuffDown(S.FelBarrageBuff) or Player:Fury() > 45 and S.BlindFury:IsAvailable()) then
  ...
  -- death_sweep,target_if=max:debuff.reavers_mark.remains,if=buff.fel_barrage.down
  if S.DeathSweep:IsReady() and (Player:BuffDown(S.FelBarrageBuff)) then
    if CastSwitch(HighRMTarget, S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then return "death_sweep ar_fel_barrage 8"; end
  end
  -- immolation_aura,if=(active_enemies>2|buff.fel_barrage.up)&(cooldown.eye_beam.remains>recharge_time+3)
  if ImmoAbility:IsReady() and ((Enemies8yCount > 2 or Player:BuffUp(S.FelBarrageBuff)) and (BeamAbility:CooldownRemains() > ImmoAbility:Recharge() + 3)) then
  ...
  -- blade_dance,target_if=max:debuff.reavers_mark.remains,if=buff.fel_barrage.down
  if S.BladeDance:IsReady() and (Player:BuffDown(S.FelBarrageBuff)) then
    if CastSwitch(HighRMTarget, S.BladeDance, nil, nil, not IsInMeleeRange(8)) then return "blade_dance ar_fel_barrage 14"; end
  end
  -- fel_barrage,if=fury>100&(raid_event.adds.in>90|raid_event.adds.in<gcd.max|raid_event.adds.remains>4&active_enemies>2)
  if S.FelBarrage:IsReady() and (Player:Fury() > 100) then
  ...
  if S.Felblade:IsCastable() and (InertiaTrigger() and Player:BuffUp(S.FelBarrageBuff)) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_fel_barrage 18"; end
  end
  ...
  if S.Felblade:IsCastable() and (Player:BuffUp(S.FelBarrageBuff) and Player:FuryDeficit() > 40) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade fel_barrage 22"; end
  end
  -- death_sweep,target_if=max:debuff.reavers_mark.remains,if=fury-variable.gcd_drain-35>0&(buff.fel_barrage.remains<3|variable.generator_up|fury>80|variable.fury_gen>18)
  if S.DeathSweep:IsReady() and (Player:Fury() - VarGCDDrain - 35 > 0 and (Player:BuffRemains(S.FelBarrageBuff) < 3 or VarGeneratorUp or Player:Fury() > 80 or VarFuryGen > 18)) then
    if CastSwitch(HighRMTarget, S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then return "death_sweep ar_fel_barrage 24"; end
  end
  -- glaive_tempest,if=fury-variable.gcd_drain-30>0&(buff.fel_barrage.remains<3|variable.generator_up|fury>80|variable.fury_gen>18)
  if S.GlaiveTempest:IsReady() and (Player:Fury() - VarGCDDrain - 30 > 0 and (Player:BuffRemains(S.FelBarrageBuff) < 3 or VarGeneratorUp or Player:Fury() > 80 or VarFuryGen > 18)) then
    if Cast(S.GlaiveTempest, Settings.Havoc.GCDasOffGCD.GlaiveTempest) then return "glaive_tempest ar_fel_barrage 26"; end
  end
  -- blade_dance,target_if=max:debuff.reavers_mark.remains,if=fury-variable.gcd_drain-35>0&(buff.fel_barrage.remains<3|variable.generator_up|fury>80|variable.fury_gen>18)
  if S.BladeDance:IsReady() and (Player:Fury() - VarGCDDrain - 35 > 0 and (Player:BuffRemains(S.FelBarrageBuff) < 3 or VarGeneratorUp or Player:Fury() > 80 or VarFuryGen > 18)) then
    if CastSwitch(HighRMTarget, S.BladeDance, nil, nil, not IsInMeleeRange(8)) then return "blade_dance ar_fel_barrage 28"; end
  end
  -- arcane_torrent,if=fury.deficit>40&buff.fel_barrage.up
  if CDsON() and S.ArcaneTorrent:IsCastable() and (Player:FuryDeficit() > 40 and Player:BuffUp(S.FelBarrageBuff)) then
  ...
  -- the_hunt,target_if=max:debuff.reavers_mark.remains,if=fury>40&(active_enemies>=desired_targets+raid_event.adds.count|raid_event.adds.in>80)
  if CDsON() and S.TheHunt:IsCastable() and (Player:Fury() > 40) then
    if CastSwitch(HighRMTarget, S.TheHunt, nil, Settings.CommonsDS.DisplayStyle.TheHunt, not Target:IsSpellInRange(S.TheHunt)) then return "the_hunt ar_fel_barrage 32"; end
  end
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=fury-variable.gcd_drain-40>20&(buff.fel_barrage.remains<3|variable.generator_up|fury>80|variable.fury_gen>18)
  if S.Annihilation:IsReady() and (Player:Fury() - VarGCDDrain - 40 > 20 and (Player:BuffRemains(S.FelBarrageBuff) < 3 or VarGeneratorUp or Player:Fury() > 80 or VarFuryGen > 18)) then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_fel_barrage 34"; end
  end
  -- chaos_strike,target_if=max:debuff.reavers_mark.remains,if=fury-variable.gcd_drain-40>20&(cooldown.fel_barrage.remains&cooldown.fel_barrage.remains<10&fury>100|buff.fel_barrage.up&(buff.fel_barrage.remains*variable.fury_gen-buff.fel_barrage.remains*32)>0)
  if S.ChaosStrike:IsReady() and (Player:Fury() - VarGCDDrain - 40 > 20 and (S.FelBarrage:CooldownDown() and S.FelBarrage:CooldownRemains() < 10 and Player:Fury() > 100 or Player:BuffUp(S.FelBarrageBuff) and (Player:BuffRemains(S.FelBarrageBuff) * VarFuryGen - Player:BuffRemains(S.FelBarrageBuff) * 32) > 0)) then
    if CastSwitch(HighRMTarget, S.ChaosStrike, nil, nil, not IsInMeleeRange(5)) then return "chaos_strike ar_fel_barrage 36"; end
  end
  -- demons_bite
  if S.DemonsBite:IsCastable() then
...
local function ARMeta()
  -- death_sweep,target_if=max:debuff.reavers_mark.remains,if=buff.metamorphosis.remains<gcd.max|debuff.essence_break.up|cooldown.metamorphosis.up&!talent.restless_hunter
  if S.DeathSweep:IsReady() then
    if Everyone.CastTargetIf(S.DeathSweep, Enemies8y, "max", ETIFReaversMark, ETIDeathSweepARMeta, not IsInMeleeRange(8)) then return "death_sweep ar_meta 2"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=talent.inertia&buff.inertia_trigger.up&cooldown.essence_break.remains<=1&cooldown.blade_dance.remains<=gcd.max*2&cooldown.metamorphosis.remains<=gcd.max*3
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and S.EssenceBreak:CooldownRemains() <= 1 and S.BladeDance:CooldownRemains() <= Player:GCD() * 2 and S.Metamorphosis:CooldownRemains() <= Player:GCD() * 3) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_meta 6"; end
  end
  ...
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=buff.metamorphosis.remains<gcd.max|debuff.essence_break.remains&debuff.essence_break.remains<0.5&cooldown.blade_dance.remains|buff.inner_demon.up&cooldown.essence_break.up&cooldown.metamorphosis.up
  -- Note: Adding 0.5s extra to the debuff check to account for player latency.
  if S.Annihilation:IsReady() then
    if Everyone.CastTargetIf(S.Annihilation, Enemies8y, "max", ETIFReaversMark, ETIAnnihilationARMeta, not IsInMeleeRange(5)) then return "annihilation ar_meta 10"; end
  end
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=buff.inertia_trigger.up&talent.inertia&cooldown.metamorphosis.remains&(cooldown.eye_beam.remains<=0.5|cooldown.essence_break.remains<=0.5|cooldown.blade_dance.remains<=5.5|buff.initiative.remains<gcd.remains)
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and S.Metamorphosis:CooldownDown() and (BeamAbility:CooldownRemains() <= 0.5 or S.EssenceBreak:CooldownRemains() <= 0.5 or S.BladeDance:CooldownRemains() <= 5.5 or Player:BuffRemains(S.InitiativeBuff) < Player:GCDRemains())) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_meta 12"; end
  end
  ...
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=buff.inner_demon.up&(cooldown.eye_beam.remains<gcd.max*3&cooldown.blade_dance.remains|cooldown.metamorphosis.remains<gcd.max*3)
  if S.Annihilation:IsReady() and (Player:BuffUp(S.InnerDemonBuff) and (BeamAbility:CooldownRemains() < Player:GCD() * 3 and S.BladeDance:CooldownDown() or S.Metamorphosis:CooldownRemains() < Player:GCD() * 3)) then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_meta 18"; end
  end
  ...
  -- death_sweep,target_if=max:debuff.reavers_mark.remains
  if S.DeathSweep:IsReady() then
    if CastSwitch(HighRMTarget, S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then return "death_sweep ar_meta 24"; end
  end
  ...
  -- throw_glaive,target_if=max:debuff.reavers_mark.remains,if=talent.soulscar&talent.furious_throws&active_enemies>2&debuff.essence_break.down&(charges=2|full_recharge_time<cooldown.blade_dance.remains)
  if S.ThrowGlaive:IsCastable() then
    if Everyone.CastTargetIf(S.ThrowGlaive, Enemies8y, "max", ETIFReaversMark, ETIThrowGlaiveARMeta, not Target:IsSpellInRange(S.ThrowGlaive)) then return "throw_glaive ar_meta 32"; end
  end
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=cooldown.blade_dance.remains|fury>60|soul_fragments.total>0|buff.metamorphosis.remains<5&cooldown.felblade.up|debuff.essence_break.up
  if S.Annihilation:IsReady() then
    if Everyone.CastTargetIf(S.Annihilation, Enemies8y, "max", ETIFReaversMark, ETIAnnihilationARMeta2, not IsInMeleeRange(5)) then return "annihilation ar_meta 34"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=fury.deficit>=40+variable.fury_gen*0.5&!buff.inertia_trigger.up
  if S.Felblade:IsCastable() and (Player:FuryDeficit() >= 40 + VarFuryGen * 0.5 and not InertiaTrigger()) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_meta 38"; end
  end
  ...
  -- annihilation,target_if=max:debuff.reavers_mark.remains
  if S.Annihilation:IsReady() then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_meta 44"; end
  end
  -- throw_glaive,target_if=max:debuff.reavers_mark.remains,if=buff.unbound_chaos.down&recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&buff.out_of_range.down&active_enemies>1&!talent.furious_throws
  if S.ThrowGlaive:IsReady() then
    if Everyone.CastTargetIf(S.ThrowGlaive, Enemies8y, "max", ETIFReaversMark, ETIThrowGlaiveARMeta2, not Target:IsSpellInRange(S.ThrowGlaive)) then return "throw_glaive ar_meta 46"; end
  end
  -- fel_rush,if=recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&buff.out_of_range.down&active_enemies>1
  if S.FelRush:IsCastable() and UseFelRush() and (S.FelRush:Recharge() < BeamAbility:CooldownRemains() and Target:DebuffDown(S.EssenceBreakDebuff) and (BeamAbility:CooldownRemains() > 8 or S.FelRush:ChargesFractional() > 1.01) and IsInMeleeRange(15) and Enemies8yCount > 1) then
    if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then return "fel_rush ar_meta 48"; end
  end
  -- demons_bite,target_if=max:debuff.reavers_mark.remains
  if S.DemonsBite:IsCastable() then
    if CastSwitch(HighRMTarget, S.DemonsBite, nil, nil, not IsInMeleeRange(5)) then return "demons_bite ar_meta 50"; end
  end
end
...
local function AROpener()
  ...
  -- the_hunt,target_if=max:debuff.reavers_mark.remains
  if CDsON() and S.TheHunt:IsCastable() then
    if CastSwitch(HighRMTarget, S.TheHunt, nil, Settings.CommonsDS.DisplayStyle.TheHunt, not Target:IsInRange(50)) then return "the_hunt ar_opener 4"; end
  end
  ...
  if S.VengefulRetreat:IsCastable() and (S.Initiative:IsAvailable() and CombatTime > 4 and Player:BuffUp(S.MetamorphosisBuff) and (not S.Inertia:IsAvailable() or not InertiaTrigger()) and Player:BuffDown(S.InnerDemonBuff) and S.BladeDance:CooldownDown() and Player:GCDRemains() < 0.1) then
    if Cast(S.VengefulRetreat, Settings.Havoc.OffGCDasOffGCD.VengefulRetreat) then return "vengeful_retreat ar_opener 6"; end
  end
  -- death_sweep,target_if=max:debuff.reavers_mark.remains,if=!talent.chaotic_transformation&cooldown.metamorphosis.up&buff.glaive_flurry.up
  if S.DeathSweep:IsReady() and (not S.ChaoticTransformation:IsAvailable() and S.Metamorphosis:CooldownUp() and Player:BuffUp(S.GlaiveFlurryBuff)) then
    if CastSwitch(HighRMTarget, S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then return "death_sweep ar_opener 8"; end
  end
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=buff.rending_strike.up&buff.thrill_of_the_fight_damage.down
  if S.Annihilation:IsReady() and (Player:BuffUp(S.RendingStrikeBuff) and Player:BuffDown(S.ThrilloftheFightHavocDmgBuff)) then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_opener 10"; end
  end
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=!talent.inertia&talent.unbound_chaos&buff.unbound_chaos.up&buff.initiative.up&debuff.essence_break.down&active_enemies<=2
  -- Note: Moved active_enemies check to the beginning of the line.
  if S.Felblade:IsCastable() and (Enemies8yCount <= 2 and not S.Inertia:IsAvailable() and S.UnboundChaos:IsAvailable() and Player:BuffUp(S.UnboundChaosBuff) and Player:BuffUp(S.InitiativeBuff)) then
    if Everyone.CastTargetIf(S.Felblade, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakDown, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_opener 12"; end
  end
  ...
  -- annihilation,target_if=max:debuff.reavers_mark.remains,if=talent.inner_demon&buff.inner_demon.up&(!talent.essence_break|cooldown.essence_break.up)
  if S.Annihilation:IsReady() and (S.InnerDemon:IsAvailable() and Player:BuffUp(S.InnerDemonBuff) and (not S.EssenceBreak:IsAvailable() or S.EssenceBreak:CooldownUp())) then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_opener 16"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=buff.inertia_trigger.up&talent.inertia&talent.restless_hunter&cooldown.essence_break.up&cooldown.metamorphosis.up&buff.metamorphosis.up&cooldown.blade_dance.remains<=gcd.max
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and S.RestlessHunter:IsAvailable() and S.EssenceBreak:CooldownUp() and S.Metamorphosis:CooldownUp() and Player:BuffUp(S.MetamorphosisBuff) and S.BladeDance:CooldownRemains() <= Player:GCD()) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_opener 20"; end
  end
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=talent.inertia&buff.inertia_trigger.up&(buff.inertia.down&buff.metamorphosis.up)&debuff.essence_break.down&active_enemies<=2
  -- Note: Moved active_enemies check to the beginning of the line.
  if S.Felblade:IsCastable() and (Enemies8yCount <= 2 and S.Inertia:IsAvailable() and InertiaTrigger() and (Player:BuffDown(S.InertiaBuff) and Player:BuffUp(S.MetamorphosisBuff))) then
    if Everyone.CastTargetIf(S.Felblade, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakDown, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_opener 22"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=talent.inertia&buff.inertia_trigger.up&buff.metamorphosis.up&cooldown.metamorphosis.remains&debuff.essence_break.down
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and Player:BuffUp(S.MetamorphosisBuff) and S.Metamorphosis:CooldownDown()) then
    if Everyone.CastTargetIf(S.Felblade, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakDown, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_opener 26"; end
  end
  -- the_hunt,target_if=max:debuff.reavers_mark.remains,if=(buff.metamorphosis.up&hero_tree.aldrachi_reaver&talent.shattered_destiny|!talent.shattered_destiny&hero_tree.aldrachi_reaver|hero_tree.felscarred)&(!talent.initiative|talent.inertia|buff.initiative.up|time>5)
  if CDsON() and S.TheHunt:IsCastable() and ((Player:BuffUp(S.MetamorphosisBuff) and VarHeroTree == 35 and S.ShatteredDestiny:IsAvailable() or not S.ShatteredDestiny:IsAvailable() and VarHeroTree == 35 or VarHeroTree == 34) and (not S.Initiative:IsAvailable() or S.Inertia:IsAvailable() or Player:BuffUp(S.InitiativeBuff) or CombatTime > 5)) then
    if CastSwitch(HighRMTarget, S.TheHunt, nil, Settings.CommonsDS.DisplayStyle.TheHunt, not Target:IsInRange(50)) then return "the_hunt ar_opener 28"; end
  end
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=fury<40&buff.inertia_trigger.down&debuff.essence_break.down
  if S.Felblade:IsCastable() and (Player:Fury() < 40 and not InertiaTrigger()) then
    if Everyone.CastTargetIf(S.Felblade, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakDown, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar_opener 30"; end
  end
  -- reavers_glaive,target_if=max:debuff.reavers_mark.remains,if=debuff.reavers_mark.down&debuff.essence_break.down
  if S.ReaversGlaive:IsCastable() and (Target:DebuffDown(S.ReaversMarkDebuff)) then
    if Everyone.CastTargetIf(S.ReaversGlaive, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakDown, not Target:IsInRange(50), Settings.CommonsOGCD.OffGCDasOffGCD.ReaversGlaive) then return "reavers_glaive ar_opener 32"; end
  end
  ...
  if S.ChaosStrike:IsReady() and (Player:BuffUp(S.RendingStrikeBuff) and Enemies8yCount > 2) then
    if CastSwitch(HighRMTarget, S.ChaosStrike, nil, nil, not IsInMeleeRange(5)) then return "chaos_strike ar_opener 34"; end
  end
  ...
  if S.BladeDance:IsReady() and (Player:BuffUp(S.GlaiveFlurryBuff) and Enemies8yCount > 2) then
    if CastSwitch(HighRMTarget, S.BladeDance, nil, nil, not IsInMeleeRange(8)) then return "blade_dance ar_opener 36"; end
  end
  -- immolation_aura,if=talent.a_fire_inside&talent.burning_wound&buff.metamorphosis.down
  -- Note: Added buff.immolation_aura.down check to avoid double suggestions during opener.
  if ImmoAbility:IsCastable() and Player:BuffDown(S.ImmolationAuraBuff) and (S.AFireInside:IsAvailable() and S.BurningWound:IsAvailable() and Player:BuffDown(S.MetamorphosisBuff)) then
  ...
  -- death_sweep,target_if=max:debuff.reavers_mark.remains
  if S.DeathSweep:IsReady() then
    if CastSwitch(HighRMTarget, S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then return "death_sweep ar_opener 48"; end
  end
  -- annihilation,target_if=max:debuff.reavers_mark.remains
  if S.Annihilation:IsReady() then
    if CastSwitch(HighRMTarget, S.Annihilation, nil, nil, not IsInMeleeRange(5)) then return "annihilation ar_opener 50"; end
  end
  -- demons_bite,target_if=max:debuff.reavers_mark.remains
  if S.DemonsBite:IsCastable() then
    if CastSwitch(HighRMTarget, S.DemonsBite, nil, nil, not IsInMeleeRange(5)) then return "demons_bite ar_opener 52"; end
  end
end

local function AR()
  -- Note: Most of the APL is based on the target with the highest remaining time on Reaver's Mark, so let's find that target now.
  HighRMTarget = HighestReaversMark(Enemies8y)
  ...
  -- Note: Using FightRemains in place of this variable.
  -- retarget_auto_attack,target_if=max:debuff.reavers_mark.remains
  if RetargetAutoAttack(S.DemonBlades, EnemiesMelee, "max", ETIFBurningWound, not IsInMeleeRange(5)) then return "retarget_auto_attack ar()"; end
  ...
  -- chaos_strike,target_if=max:target.health.pct,if=buff.rending_strike.up&buff.glaive_flurry.up&(variable.rg_ds=2|active_enemies>1)&time>10&!debuff.reavers_mark.up
  -- annihilation,target_if=max:target.health.pct,if=buff.rending_strike.up&buff.glaive_flurry.up&(variable.rg_ds=2|active_enemies>1)&!debuff.reavers_mark.up
  if Player:BuffUp(S.RendingStrikeBuff) and Player:BuffUp(S.GlaiveFlurryBuff) and (VarRGDS == 2 or Enemies8yCount > 1) then
    local SpellSwitch
    if S.ChaosStrike:IsReady() and CombatTime > 10 then
      SpellSwitch = S.ChaosStrike
    end
    if S.Annihilation:IsReady() then
      SpellSwitch = S.Annihilation
    end
    if SpellSwitch then
      if Everyone.CastTargetIf(SpellSwitch, Enemies8y, "max", ETIFHealthPercent, ETIReaverDown, not IsInMeleeRange(5)) then return "chaos_strike/annihilation ar 2"; end
    end
  end
  ...
  if Player:BuffUp(S.RendingStrikeBuff) and Player:BuffUp(S.GlaiveFlurryBuff) and (VarRGDS == 2 or Enemies8yCount > 1) then
    ...
    if SpellSwitch then
      if Everyone.CastTargetIf(SpellSwitch, Enemies8y, "max", ETIFReaversMark, ETIReaverUp, not IsInMeleeRange(5)) then return "chaos_strike/annihilation ar 4"; end
    end
  end
  ...
  if S.ReaversGlaive:IsReady() and (Player:BuffDown(S.GlaiveFlurryBuff) and Player:BuffDown(S.RendingStrikeBuff) and Player:BuffRemains(S.ThrilloftheFightHavocDmgBuff) < Player:GCD() * 4 + num(VarRGDS == 2) + num(S.TheHunt:CooldownRemains() < Player:GCD() * 3) * 3 + num(BeamAbility:CooldownRemains() < Player:GCD() * 3 and S.ShatteredDestiny:IsAvailable()) * 3 and (VarRGDS == 0 or VarRGDS == 1 and S.BladeDance:CooldownUp() or VarRGDS == 2 and S.BladeDance:CooldownDown()) and (Player:BuffUp(S.ThrilloftheFightHavocDmgBuff) or not Player:PrevGCD(1, S.DeathSweep) or not VarRGInc) and Enemies8yCount < 3 and S.ReaversGlaive:TimeSinceLastCast() >= 5 and Target:DebuffDown(S.EssenceBreakDebuff) and (Player:BuffRemains(S.MetamorphosisBuff) > 2 or BeamAbility:CooldownRemains() < 10 or BossFightRemains < 10) and (FightRemains >= 10 or BossFightRemains <= 10) or BossFightRemains <= 10) then
    if CastSwitch(HighRMTarget, S.ReaversGlaive, Settings.CommonsOGCD.OffGCDasOffGCD.ReaversGlaive, nil, not Target:IsInRange(50)) then return "reavers_glaive ar_opener 6"; end
  end
  -- reavers_glaive,target_if=max:debuff.reavers_mark.remains,if=buff.glaive_flurry.down&buff.rending_strike.down&buff.thrill_of_the_fight_damage.remains<4&(buff.thrill_of_the_fight_damage.up|!prev_gcd.1.death_sweep|!variable.rg_inc)&active_enemies>=2&(variable.pull_remains>=10|fight_remains<10)
  if S.ReaversGlaive:IsReady() and (Player:BuffDown(S.GlaiveFlurryBuff) and Player:BuffDown(S.RendingStrikeBuff) and Player:BuffRemains(S.ThrilloftheFightHavocDmgBuff) < 4 and (Player:BuffUp(S.ThrilloftheFightHavocDmgBuff) or not Player:PrevGCD(1, S.DeathSweep) or not VarRGInc) and Enemies8yCount >= 2 and (FightRemains >= 10 or BossFightRemains < 10)) then
    if CastSwitch(HighRMTarget, S.ReaversGlaive, Settings.CommonsOGCD.OffGCDasOffGCD.ReaversGlaive, nil, not Target:IsInRange(50)) then return "reavers_glaive ar 8"; end
  end
  ...
  if S.Felblade:IsCastable() and (
    -- felblade,target_if=max:debuff.reavers_mark.remains,if=!talent.inertia&active_enemies=1&buff.unbound_chaos.up&buff.initiative.up&debuff.essence_break.down&buff.metamorphosis.down
    ...
    -- felblade,target_if=max:debuff.reavers_mark.remains,if=buff.inertia_trigger.up&talent.inertia&cooldown.eye_beam.remains<=0.5&(cooldown.metamorphosis.remains&talent.looks_can_kill|active_enemies>1)
    ...
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar 18"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=buff.inertia_trigger.up&talent.inertia&buff.inertia.down&cooldown.blade_dance.remains<4&(cooldown.eye_beam.remains>5&cooldown.eye_beam.remains>buff.unbound_chaos.remains|cooldown.eye_beam.remains<=gcd.max&cooldown.vengeful_retreat.remains<=gcd.max+1)
  if S.Felblade:IsCastable() and (S.Inertia:IsAvailable() and InertiaTrigger() and Player:BuffDown(S.InertiaBuff) and S.BladeDance:CooldownRemains() < 4 and (BeamAbility:CooldownRemains() > 5 and BeamAbility:CooldownRemains() > Player:BuffRemains(S.UnboundChaosBuff) or BeamAbility:CooldownRemains() <= Player:GCD() and S.VengefulRetreat:CooldownRemains() <= Player:GCD() + 1)) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar 20"; end
  end
  if ImmoAbility:IsReady() and (
    -- immolation_aura,if=talent.a_fire_inside&talent.burning_wound&full_recharge_time<gcd.max*2&(raid_event.adds.in>full_recharge_time|active_enemies>desired_targets)
  ...
  -- blade_dance,target_if=max:debuff.reavers_mark.remains,if=(cooldown.eye_beam.remains>=gcd.max*2|active_enemies>=2&buff.glaive_flurry.up&(raid_event.adds.in>30-buff.cycle_of_hatred.stack*5|raid_event.adds.remains>=cooldown.eye_beam.remains&cooldown.eye_beam.remains<gcd.max*2))&buff.rending_strike.down
  if S.BladeDance:IsReady() and ((BeamAbility:CooldownRemains() >= Player:GCD() * 2 or Enemies8yCount >= 2 and Player:BuffUp(S.GlaiveFlurryBuff)) and Player:BuffDown(S.RendingStrikeBuff)) then
    if CastSwitch(HighRMTarget, S.BladeDance, nil, nil, not IsInMeleeRange(8)) then return "blade_dance ar 26"; end
  end
  -- chaos_strike,target_if=max:debuff.reavers_mark.remains,if=buff.rending_strike.up
  if S.ChaosStrike:IsReady() and (Player:BuffUp(S.RendingStrikeBuff)) then
    if CastSwitch(HighRMTarget, S.ChaosStrike, nil, nil, not IsInMeleeRange(5)) then return "chaos_strike ar 28"; end
  end
  ...
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=fury.deficit>=40+variable.fury_gen*0.5&!buff.inertia_trigger.up&(!talent.blind_fury|cooldown.eye_beam.remains>5)
  if S.Felblade:IsCastable() and (Player:FuryDeficit() >= 40 + VarFuryGen * 0.5 and not InertiaTrigger() and (not S.BlindFury:IsAvailable() or BeamAbility:CooldownRemains() > 5)) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar 32"; end
  end
  -- glaive_tempest,if=active_enemies>=desired_targets+raid_event.adds.count|raid_event.adds.in>10
  if S.GlaiveTempest:IsReady() then
    if Cast(S.GlaiveTempest, Settings.Havoc.GCDasOffGCD.GlaiveTempest) then return "glaive_tempest ar 34"; end
  end
  -- chaos_strike,target_if=max:debuff.reavers_mark.remains,if=debuff.essence_break.up
  if S.ChaosStrike:IsReady() and (Target:DebuffUp(S.EssenceBreakDebuff)) then
    if Everyone.CastTargetIf(S.ChaosStrike, Enemies8y, "max", ETIFReaversMark, ETIEssenceBreakUp, not IsInMeleeRange(5)) then return "chaos_strike ar 36"; end
  end
  -- chaos_nova,if=talent.chaos_fragments&active_enemies>4
  if S.ChaosNova:IsReady() and (S.ChaosFragments:IsAvailable() and Enemies8yCount > 4) then
    if Cast(S.ChaosNova, Settings.Havoc.GCDasOffGCD.ChaosNova, nil, not Target:IsInRange(40)) then return "chaos_nova ar 38"; end
  end
  -- throw_glaive,target_if=max:debuff.reavers_mark.remains,if=active_enemies>2&talent.furious_throws&talent.soulscar&(!talent.screaming_brutality|charges=2|full_recharge_time<cooldown.blade_dance.remains)
  if S.ThrowGlaive:IsReady() and (Enemies8yCount > 2 and S.FuriousThrows:IsAvailable() and S.Soulscar:IsAvailable() and (not S.ScreamingBrutality:IsAvailable() or S.ThrowGlaive:Charges() == 2 or S.ThrowGlaive:FullRechargeTime() < S.BladeDance:CooldownRemains())) then
    if CastSwitch(HighRMTarget, S.ThrowGlaive, nil, nil, not Target:IsSpellInRange(S.ThrowGlaive)) then return "throw_glaive ar 40"; end
  end
  ...
  if S.ChaosStrike:IsReady() and (BeamAbility:CooldownRemains() > Player:GCD() * 4 or Player:Fury() >= 70 - VarFuryGen * Player:GCD() - S.BlindFury:TalentRank() * 15) then
    if Cast(S.ChaosStrike, nil, nil, not IsInMeleeRange(5)) then return "chaos_strike ar 42"; end
  end
  -- felblade,target_if=max:debuff.reavers_mark.remains,if=!talent.a_fire_inside&fury<40
  if S.Felblade:IsCastable() and (not S.AFireInside:IsAvailable() and Player:Fury() < 40) then
    if CastSwitch(HighRMTarget, S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then return "felblade ar 44"; end
  end
  ...
  if SigilAbility:IsCastable() and (Target:IsInRange(8) and Target:DebuffDown(S.EssenceBreakDebuff) and (not S.FelBarrage:IsAvailable() or S.FelBarrage:CooldownRemains() > 25 or Enemies8yCount == 1)) then
    if Cast(SigilAbility, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then return "sigil_ability ar 48"; end
  end
  -- demons_bite,target_if=max:debuff.reavers_mark.remains
  if S.DemonsBite:IsCastable() then
    if CastSwitch(HighRMTarget, S.DemonsBite, nil, nil, not IsInMeleeRange(5)) then return "demons_bite ar 50"; end
  end
  -- throw_glaive,target_if=max:debuff.reavers_mark.remains,if=buff.unbound_chaos.down&recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&buff.out_of_range.down&active_enemies>1&!talent.furious_throws
  if S.ThrowGlaive:IsReady() and (Player:BuffDown(S.UnboundChaosBuff) and S.ThrowGlaive:Recharge() < BeamAbility:CooldownRemains() and Target:DebuffDown(S.EssenceBreakDebuff) and (BeamAbility:CooldownRemains() > 8 or S.ThrowGlaive:ChargesFractional() > 1.01) and Enemies8yCount > 1 and not S.FuriousThrows:IsAvailable()) then
    if Everyone.CastTargetIf(S.ThrowGlaive, Enemies8y, "max", ETIFReaversMark, ETIThrowGlaiveAR, not IsInMeleeRange(5)) then return "throw_glaive ar 52"; end
  end
  -- fel_rush,if=buff.unbound_chaos.down&recharge_time<cooldown.eye_beam.remains&debuff.essence_break.down&(cooldown.eye_beam.remains>8|charges_fractional>1.01)&active_enemies>1
  if S.FelRush:IsCastable() and UseFelRush() and (Player:BuffDown(S.UnboundChaosBuff) and S.FelRush:Recharge() < BeamAbility:CooldownRemains() and Target:DebuffDown(S.EssenceBreakDebuff) and (BeamAbility:CooldownRemains() > 8 or S.FelRush:ChargesFractional() > 1.01) and Enemies8yCount > 1) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.BurningWound:IsAvailable() and S.DemonBlades:IsAvailable() and GetTime() - LastRetargetTime >= 1 and S.BurningWoundDebuff:AuraActiveCount() < mathmin(EnemiesMeleeCount, 3) then
      if RetargetAutoAttack(S.DemonBlades, EnemiesMelee, "none", ETIFBurningWound, not IsInMeleeRange(5)) then return "retarget_auto_attack main 2"; end
    end
    ...
    if S.BurningWound:IsAvailable() and S.DemonBlades:IsAvailable() and GetTime() - LastRetargetTime >= 1 and S.BurningWoundDebuff:AuraActiveCount() == mathmin(EnemiesMeleeCount, 3) then
      if RetargetAutoAttack(S.DemonBlades, EnemiesMelee, "nonboss", ETIFBurningWound, not IsInMeleeRange(5)) then return "retarget_auto_attack main 3"; end
    end
