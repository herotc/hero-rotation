...
local function AR()
  ...
  -- Note: We can't check raid_event conditions, so simply checking for active_enemies>2 will make this line supercede the previous line.
  -- immolation_aura,if=active_enemies>2&talent.ragefire&(!talent.fel_barrage|cooldown.fel_barrage.remains>recharge_time)&debuff.essence_break.down&(buff.metamorphosis.down|buff.metamorphosis.remains>5)
  if ImmoAbility:IsReady() and (Enemies8yCount > 2 and S.Ragefire:IsAvailable() and (not S.FelBarrage:IsAvailable() or S.FelBarrage:CooldownRemains() > ImmoAbility:Recharge()) and Target:DebuffDown(S.EssenceBreakDebuff) and (Player:BuffDown(S.MetamorphosisBuff) or Player:BuffRemains(S.MetamorphosisBuff) > 5)) then
...
local function Init()
  ...
  HR.Print("Havoc Demon Hunter rotation has been updated for patch 11.2")
end
