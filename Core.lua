...
--- ============== UNIT CLASS ==============
  ...
  function Unit:Classification ()
    if self:GUID() then
      if not ER.Cache.UnitInfo[self:GUID()] then ER.Cache.UnitInfo[self:GUID()] = {}; end
      if ER.Cache.UnitInfo[self:GUID()].Classification == nil then
        ER.Cache.UnitInfo[self:GUID()].Classification = UnitClassification(self.UnitID);
      end
      return ER.Cache.UnitInfo[self:GUID()].Classification;
    end
    return "";
  end
  ...
  function Unit:CastPercentage (ReverseChannel)
    ...
    if self:IsChanneling() then
      ...
      return ReverseChannel and (ER.GetTime()*1000 - _T.Start)/(_T.End - _T.Start)*100 or 100-(ER.GetTime()*1000 - _T.Start)/(_T.End - _T.Start)*100;
    end
    return -1;
  end
  ...
  function Unit:IsStunned ()
    if self:GUID() then
      if not ER.Cache.UnitInfo[self:GUID()] then ER.Cache.UnitInfo[self:GUID()] = {}; end
      if ER.Cache.UnitInfo[self:GUID()].IsStunned == nil then
        ER.Cache.UnitInfo[self:GUID()].IsStunned = self:IterateStunDebuffs();
      end
      return ER.Cache.UnitInfo[self:GUID()].IsStunned;
    end
    return nil;
  end
  ...
  function Unit:IsStunnable ()
    ...
    if self:GUID() then
      if not ER.Cache.UnitInfo[self:GUID()] then ER.Cache.UnitInfo[self:GUID()] = {}; end
      if ER.Cache.UnitInfo[self:GUID()].IsStunnable == nil then
        ER.Cache.UnitInfo[self:GUID()].IsStunnable = IsStunnableClassification[self:Classification()];
      end
      return ER.Cache.UnitInfo[self:GUID()].IsStunnable;
    end
    return nil;
  end
  ...
  --- PLAYER SPECIFIC
    ...
    function Unit:RageMax ()
      if self:GUID() then
        if not ER.Cache.UnitInfo[self:GUID()] then ER.Cache.UnitInfo[self:GUID()] = {}; end
        if not ER.Cache.UnitInfo[self:GUID()].RageMax then
          ER.Cache.UnitInfo[self:GUID()].RageMax = UnitPowerMax(self.UnitID, SPELL_POWER_RAGE);
        end
        return ER.Cache.UnitInfo[self:GUID()].RageMax;
      end
    end
    -- rage
    function Unit:Rage ()
      if self:GUID() then
        if not ER.Cache.UnitInfo[self:GUID()] then ER.Cache.UnitInfo[self:GUID()] = {}; end
        if not ER.Cache.UnitInfo[self:GUID()].Rage then
          ER.Cache.UnitInfo[self:GUID()].Rage = UnitPower(self.UnitID, SPELL_POWER_RAGE);
        end
        return ER.Cache.UnitInfo[self:GUID()].Rage;
      end
    end
    -- rage.pct
...
--- ============== ITEM CLASS ==============
  ...
  function Item:IsEquipped (Slot)
    ...
    if ER.Cache.ItemInfo[self.ItemID].IsEquipped == nil then
      ER.Cache.ItemInfo[self.ItemID].IsEquipped = ER.Equipment[Slot] == self.ItemID and true or false;
    end
