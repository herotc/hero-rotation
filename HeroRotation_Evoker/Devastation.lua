...
local function Trinkets()
  ...
  if Settings.Commons.Enabled.Trinkets then
    ...
    if I.NymuesUnravelingSpindle:IsEquipped() and I.NymuesUnravelingSpindle:CooldownRemains() <= Player:GCD() and (((Player:BuffStack(S.EmeraldTranceBuff) >= 2 and VarHasExternalPI) or (S.Dragonrage:CooldownRemains() <= 3 and S.FireBreath:CooldownRemains() < 7 and S.EternitySurge:CooldownRemains() < 13 and Target:TimeToDie() >= 35 and ((not VarHasExternalPI and EnemiesCount8ySplash <= 2) or not Player:HasTier(31, 2))) or (S.Dragonrage:CooldownRemains() <= 3 and EnemiesCount8ySplash >= 3)) or FightRemains <- 20) then
