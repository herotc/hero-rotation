...
local function APL ()
  ...
  if Target:Exists() and Player:CanAttack(Target) and Target:IsInRange(5) and not Target:IsDeadOrGhost() then
    ...
    if Target:HealthPercentage() > 20 then
      ...
      if S.Slam:IsReady() and ((not Cache.EnemiesCount[5] < 1 and not Cache.EnemiesCount[5] > 1) and not S.FervorOfBattle:IsAvailable() and (Player:Rage() >= 52 or not S.Rend:IsAvailable() or not S.Ravager:IsAvailable())) then
