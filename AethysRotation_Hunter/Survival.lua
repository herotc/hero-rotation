...
--- APL Action Lists (and Variables)
  ...
  local function MokNathal ()
    ...
    if S.RaptorStrike:IsCastable() and Player:BuffStack(S.MokNathalTactics) <= 1 then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    -- actions.moknathal+=/raptor_strike,if=buff.moknathal_tactics.remains<gcd
    if S.RaptorStrike:IsCastable() and Player:BuffRemains(S.MokNathalTactics) < Player:GCD() * 2 then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if S.RaptorStrike:IsCastable() and Player:BuffStack(S.MongooseFury) >= 4 and Player:BuffRemains(S.MongooseFury) > Player:GCD() and Player:BuffStack(S.MokNathalTactics) == 3 and Player:BuffRemains(S.MokNathalTactics) < 4 and S.FuryoftheEagle:Cooldown() < Player:BuffRemains(S.MongooseFury) then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if AR.CDsON() and S.AMurderofCrows:IsCastable() and Player:Focus() > 55 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() and Player:BuffStack(S.MongooseFury) < 4 and Player:BuffDuration(S.MongooseFury) >= Player:GCD() then
    ...
    if S.FlankingStrike:IsCastable() and S.MongooseBite:Charges() <= 1 and Player:Focus() > 75 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() then
    ...
    if S.Carve:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() and Player:BuffRemains(S.MongooseFury) >= Player:GCD() then
    ...
    if S.Butchery:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() and Player:BuffRemains(S.MongooseFury) >= Player:GCD() then
    ...
    if S.Lacerate:IsCastable() and Target:DebuffRefreshable(S.Lacerate, 3.6) and ((Player:Focus() > 55 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() and Player:BuffDuration(S.MongooseFury) >= Player:GCD() and S.MongooseBite:Charges() == 0 and Player:BuffStack(S.MongooseFury) < 3 ) or 
    ...
    if S.Caltrops:IsCastable() and (Player:BuffDuration(S.MongooseFury) >= Player:GCD() and Player:BuffStack(S.MongooseFury) < 1 and not Target:Debuff(S.CaltropsDot)) and not S.SteelTrapTalent:IsAvailable() then
    ...
    if S.Carve:IsCastable() and Cache.EnemiesCount[8] > 1 and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() (not Player:Buff(S.MongooseFury) and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() or 
    ...
    if S.RaptorStrike:IsCastable() and Player:BuffStack(S.MokNathalTactics) == 2 then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if S.RaptorStrike:IsCastable() and Player:BuffRemains(S.MokNathalTactics) < 4 and Player:BuffStack(S.MongooseFury) == 6 and Player:BuffRemains(S.MongooseFury) > S.FuryoftheEagle:Cooldown() and S.FuryoftheEagle:Cooldown() <= 5 then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if S.RaptorStrike:IsCastable() and Player:Buff(S.MongooseFury) and Player:BuffRemains(S.MongooseFury) <= 3 * Player:GCD() and Player:BuffRemains(S.MokNathalTactics) < 4 + Player:GCD() and S.FuryoftheEagle:Cooldown() < Player:GCD() then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if AR.CDsON() and S.AMurderofCrows:IsCastable() and Player:Focus() > 55 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() then
      if AR.Cast(S.AMurderofCrows) then return ""; end
    end
    -- actions.moknathal+=/caltrops,if=(!dot.caltrops.ticking)
    if S.Caltrops:IsCastable() and not Target:Debuff(S.CaltropsDot) and not S.SteelTrapTalent:IsAvailable() then
      if AR.Cast(S.Caltrops) then return ""; end
    end
    ...
    if S.Carve:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() then
      if AR.Cast(S.Carve) then return ""; end
    end
    ...
    if S.Butchery:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() then
      if AR.Cast(S.Butchery) then return ""; end
    end
    -- actions.moknathal+=/lacerate,if=refreshable&focus>55-buff.moknathal_tactics.remains*focus.regen
    if S.Lacerate:IsCastable() and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 55 - Player:BuffRemains(S.MokNathalTactics) * Player:FocusRegen() then
      if AR.Cast(S.Lacerate) then return ""; end
    end
  ...
  local function NoMok ()
    ...
    if AR.CDsON() and S.AMurderofCrows:IsCastable() and S.MongooseBite:Charges() >= 0 and Player:BuffStack(S.MongooseFury) < 4 then
      if AR.Cast(S.AMurderofCrows) then return ""; end
    end
    ...
    if S.Caltrops:IsCastable() and (Player:BuffDuration(S.MongooseFury) >= Player:GCD() and Player:BuffStack(S.MongooseFury) < 4 and not Target:Debuff(S.CaltropsDot)) and not S.SteelTrapTalent:IsAvailable() then
      if AR.Cast(S.Caltrops) then return ""; end
    end
    -- actions.nomok+=/flanking_strike,if=cooldown.mongoose_bite.charges<=1&buff.aspect_of_the_eagle.remains>=gcd
    if S.FlankingStrike:IsCastable() and S.MongooseBite:Charges() <= 1 and Player:BuffRemains(S.AspectoftheEagle) >= Player:GCD() then
      if AR.Cast(S.FlankingStrike) then return ""; end
    end
    ...
    if S.Carve:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 and Player:BuffRemains(S.MongooseFury) >= Player:GCD() then
      if AR.Cast(S.Carve) then return ""; end
    end
    -- actions.nomok+=/butchery,if=equipped.frizzos_fingertrap&dot.lacerate.ticking&dot.lacerate.refreshable&focus>65&buff.mongoose_fury.remains>=gcd
     if S.Butchery:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 and Player:BuffRemains(S.MongooseFury) >= Player:GCD() then
      if AR.Cast(S.Butchery) then return ""; end
    end
    ...
    if S.Lacerate:IsCastable() and Player:BuffDuration(S.MongooseFury) >= Player:GCD() and Target:DebuffRefreshable(S.Lacerate, 3.6) and S.MongooseBite:Charges() == 0 and Player:BuffStack(S.MongooseFury) < 2 or not Player:Buff(S.MongooseFury) and S.MongooseBite:Charges() < 3 and Target:DebuffRefreshable(S.Lacerate, 3.6) then
      if AR.Cast(S.Lacerate) then return ""; end
      end
    ...
    if S.RaptorStrike:IsCastable() and S.SerpentSting:IsAvailable() and Target:DebuffRefreshable(S.SerpentSting, 4.5) and Player:BuffStack(S.MongooseFury) < 3 and S.MongooseBite:Charges() < 1 then
      if AR.Cast(S.RaptorStrike) then return ""; end
    end
    ...
    if S.FlankingStrike:IsCastable() and S.MongooseBite:Charges() <= 1 and Player:BuffRemains(S.MongooseFury) > ( 1 + S.MongooseBite:Charges() * Player:GCD()) then
      if AR.Cast(S.FlankingStrike) then return ""; end
    end
    ...
    if S.FlankingStrike:IsCastable() and S.AnimalInstincts:IsAvailable() and S.MongooseBite:Charges() < 3 then
      if AR.Cast(S.FlankingStrike) then return ""; end
    end
    ...
    if AR.CDsON() and S.AMurderofCrows:IsCastable() then
      if AR.Cast(S.AMurderofCrows) then return ""; end
    end
    ...
    if S.Carve:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 then
      if AR.Cast(S.Carve) then return ""; end
    end
    -- actions.nomok+=/butchery,if=equipped.frizzos_fingertrap&dot.lacerate.ticking&dot.lacerate.refreshable&focus>65
    if S.Butchery:IsCastable() and (I.FrizzosFinger:IsEquipped(11) or  I.FrizzosFinger:IsEquipped(12)) and Target:Debuff(S.Lacerate) and Target:DebuffRefreshable(S.Lacerate, 3.6) and Player:Focus() > 65 then
      if AR.Cast(S.Butchery) then return ""; end
    end
    -- actions.nomok+=/lacerate,if=refreshable
    if S.Lacerate:IsCastable() and Target:DebuffRefreshable(S.Lacerate, 3.6) then
      if AR.Cast(S.Lacerate) then return ""; end
    end
