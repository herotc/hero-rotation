...
local VarRegrowth, VarEasySwipe, VarMoonfireSnapshotted
...
local function Precombat()
  ...
  -- variable,name=trinket_1_buffs,value=trinket.1.has_use_buff&!trinket.1.is.imperfect_ascendancy_serum&!trinket.1.is.ovinaxs_mercurial_egg&!trinket.1.is.concoction_kiss_of_death
  -- variable,name=trinket_2_buffs,value=trinket.2.has_use_buff&!trinket.2.is.imperfect_ascendancy_serum&!trinket.2.is.ovinaxs_mercurial_egg&!trinket.2.is.concoction_kiss_of_death
...
local function Builder()
  ...
  if S.Rake:IsReady() and (((Target:DebuffRefreshable(S.RakeDebuff) and Player:PMultiplier(S.Rake) >= Target:PMultiplier(S.Rake) or Target:DebuffRemains(S.RakeDebuff) < 3.5) or Player:BuffUp(S.SuddenAmbushBuff) and Player:PMultiplier(S.Rake) > Target:PMultiplier(S.Rake)) and not (VarNeedBT and BTBuffUp(S.Rake))) then
  ...
  if S.LIMoonfire:IsReady() and (Target:DebuffRefreshable(S.LIMoonfireDebuff)) then
    if Cast(S.LIMoonfire, nil, nil, not Target:IsInRange(40)) then return "moonfire_cat builder 8"; end
  end
  ...
  if S.ThrashCat:IsCastable() and (Target:DebuffRefreshable(S.ThrashCatDebuff) and not S.ThrashingClaws:IsAvailable()) then
    if Cast(S.ThrashCat, nil, nil, not IsInAoERange) then return "thrash builder 10"; end
  end
...
local function Cooldown()
  ...
  if Settings.Commons.Enabled.Trinkets and I.ImperfectAscendancySerum:IsEquippedAndReady() and (BsInc:CooldownRemains() <= 1) then
  ...
  if S.Berserk:IsReady() and (Player:BuffUp(S.TigersFury) and (Target:TimeToDie() > 12 or Target:TimeToDie() == FightRemains)) then
    if Cast(S.Berserk, Settings.Feral.GCDasOffGCD.BsInc) then return "berserk cooldown 6"; end
  end
  -- berserking,if=buff.bs_inc.up
  if S.Berserking:IsCastable() and (Player:BuffUp(BsInc)) then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then return "berserking cooldown 8"; end
  end
  ...
  if Settings.Commons.Enabled.Potions and (Player:BuffUp(BsInc) or FightRemains < 32 or (not VarLastZerk and VarLastConvoke and S.ConvoketheSpirits:CooldownRemains() < 10)) then
    ...
    if PotionSelected and PotionSelected:IsReady() then
      if Cast(PotionSelected, nil, Settings.CommonsDS.DisplayStyle.Potions) then return "potion cooldown 10"; end
    end
  end
  -- use_items
  ...
  if Settings.Commons.Enabled.Trinkets then
    -- use_item,name=concoction_kiss_of_death,if=buff.bs_inc.up
    if I.ConcoctionKissofDeath:IsEquippedAndReady() and (Player:BuffUp(BsInc)) then
      if Cast(I.ConcoctionKissofDeath, nil, Settings.CommonsDS.DisplayStyle.Trinkets) then return "concoction_kiss_of_death cooldown 14"; end
    end
    ...
    -- use_item,slot=trinket1,if=!trinket.2.has_use_buff&(trinket.2.cooldown.remains>20|!variable.trinket_2_buffs|trinket.2.cooldown.remains&cooldown.tigers_fury.remains>20)
    if Trinket1:IsReady() and not VarTrinket1BL and (not Trinket2:HasUseBuff() and (Trinket2:CooldownRemains() > 20 or not VarTrinket2Buffs or Trinket2:CooldownDown() and S.TigersFury:CooldownRemains() > 20)) then
    ...
    -- use_item,slot=trinket2,if=!trinket.2.has_use_buff&(trinket.1.cooldown.remains>20|!variable.trinket_1_buffs|trinket.1.cooldown.remains&cooldown.tigers_fury.remains>20)
    if Trinket2:IsReady() and not VarTrinket2BL and (not Trinket1:HasUseBuff() and (Trinket1:CooldownRemains() > 20 or not VarTrinket1Buffs or Trinket1:CooldownDown() and S.TigersFury:CooldownRemains() > 20)) then
  ...
  if S.FeralFrenzy:IsReady() and (ComboPoints <= 1 or Player:BuffUp(BsInc) and ComboPoints <= 2) then
    if Cast(S.FeralFrenzy, Settings.Feral.GCDasOffGCD.FeralFrenzy, nil, not IsInMeleeRange) then return "feral_frenzy cooldown 24"; end
  end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.TigersFury:IsCastable() and ((Player:EnergyDeficit() > 35 or ComboPoints == 5 or ComboPoints >= 3 and Target:DebuffRefreshable(S.RipDebuff) and Player:BuffUp(S.BloodtalonsBuff)) and (BossFightRemains <= 15 or (BsInc:CooldownRemains() > 20 and Target:TimeToDie() > 5) or (BsInc:CooldownUp() and Target:TimeToDie() > 12 or Target:TimeToDie() == FightRemains))) then
    ...
    if S.Rip:IsReady() and (EnemiesCountMelee == 1 and Player:HeroTreeID() == 22 and not (S.RagingFury:IsAvailable() and S.Veinripper:IsAvailable()) and (Player:BuffUp(S.BloodtalonsBuff) or not S.Bloodtalons:IsAvailable()) and (Target:PMultiplier(S.Rip) < Player:PMultiplier(S.Rip) and Player:BuffRemains(S.TigersFury) > 10 and ComboPoints >= 3 or ((Player:BuffRemains(S.TigersFury) < 3 and ComboPoints == 5) or Player:BuffRemains(S.TigersFury) <= 1) and Player:BuffUp(S.TigersFury) and ComboPoints >= 3 and Target:DebuffRemains(S.RipDebuff) < S.TigersFury:CooldownRemains())) then
