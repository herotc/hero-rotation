...
--- ======= MAIN =======
  local function APL()
    ...
    if S.Renewal:IsCastable() and Player:HealthPercentage() <= Settings.Feral.RenewalHP then
      if HR.Cast(S.Renewal, Settings.Feral.OffGCDasOffGCD.Renewal) then return "Renewal"; end
    end
    if S.SurvivalInstincts:IsCastable() and (not Player:Buff(S.SurvivalInstincts)) and Player:HealthPercentage() <= Settings.Feral.SurvivalInstinctsHP then
      if HR.Cast(S.SurvivalInstincts, Settings.Feral.OffGCDasOffGCD.SurvivalInstincts) then return "Survival Instincts"; end
    end
    if S.Regrowth:IsCastable() and Player:Buff(S.PredatorySwiftness) and Player:HealthPercentage() <= Settings.Feral.RegrowthHP then
      if HR.Cast(S.Regrowth, Settings.Feral.GCDasOffGCD.RegrowthHeal) then return "Regrowth (Healing)"; end
    end
    ...
    if not Player:AffectingCombat() then
      ...
      if not InCombatLockdown() and S.Prowl:CooldownUp() and not Player:IsStealthed() and GetNumLootItems() == 0 and not UnitExists("npc") and HL.OutOfCombatTime() > 1 then
        if HR.Cast(S.Prowl, Settings.Feral.OffGCDasOffGCD.Prowl) then return "Cast"; end
      end
      -- Cat Form
      if S.CatForm:IsCastable() and not Player:Buff(S.CatForm) then
        if HR.Cast(S.CatForm, Settings.Feral.GCDasOffGCD.CatForm) then return "OOC Cat Form"; end
      end
      -- Wild Charge
      if S.WildCharge:IsCastable(S.WildCharge) and not Target:IsInRange(8) and not Target:IsInRange(MeleeRange) then
        if HR.Cast(S.WildCharge, Settings.Feral.OffGCDasOffGCD.WildCharge) then return "Cast"; end
      end
      -- Opener: Rake
      if Everyone.TargetIsValid() and S.Rake:IsCastable() then
        if HR.Cast(S.Rake) then return "Rake Opener"; end
      end
      return;
    end
    ...
    if Everyone.TargetIsValid() then
      ...
      if Player:Buff(S.CatForm) then
        ...
        if Settings.General.InterruptEnabled and S.SkullBash:IsCastable(S.SkullBash) and Target:IsInterruptible() then
          if HR.Cast(S.SkullBash) then return "Cast Kick"; end
        end
        -- Wild Charge
        if S.WildCharge:IsCastable(S.WildCharge) and not Target:IsInRange(8) and not Target:IsInRange(MeleeRange) then
          if HR.Cast(S.WildCharge, Settings.Feral.OffGCDasOffGCD.WildCharge) then return "Cast"; end
        ...
        if Target:DebuffRemainsP(S.Rip) > 0 or HL.CombatTime() > 15 then
          if Target:IsInRange(MeleeRange) then
            ...
            if S.CatForm:IsCastable(MeleeRange) and not Player:Buff(S.CatForm) then
              if HR.Cast(S.CatForm, Settings.Feral.GCDasOffGCD.CatForm) then return "Cast"; end
            end
            ...
            -- call_action_list,name=cooldowns
              ...
              if S.ProwlJungleStalker:IsCastable() and (Player:BuffRemainsP(S.Incarnation) < 0.5) and Player:Buff(S.JungleStalker) then
                if Settings.Feral.StealthMacro.JungleStalker then
                  if HR.CastQueue(S.Prowl, S.Rake) then return "Prowl to Rake as Macro"; end
                else
                  if HR.Cast(S.Prowl, Settings.Feral.OffGCDasOffGCD.Prowl) then return "Prowl to Rake"; end
                end
              end
              -- berserk,if=energy>=30&(cooldown.tigers_fury.remains>5|buff.tigers_fury.up)
              if HR.CDsON() and S.Berserk:IsCastable() and Player:EnergyPredicted() >= 30 and (S.TigersFury:CooldownRemainsP() > 5 or Player:Buff(S.TigersFury)) then
                if HR.Cast(S.Berserk, Settings.Feral.OffGCDasOffGCD.Berserk) then return "Cast"; end
              end
              -- tigers_fury,if=energy.deficit>=60
              if S.TigersFury:IsCastable() and Player:EnergyDeficitPredicted() >= 60 then
                if HR.Cast(S.TigersFury, Settings.Feral.OffGCDasOffGCD.TigersFury) then return "Cast"; end
              end
              -- actions.cooldowns+=/berserking
              if HR.CDsON() and S.Berserking:IsCastable() then
              ...
              if S.ElunesGuidance:IsCastable() and Player:ComboPoints() == 0 and Player:EnergyPredicted() >= 50 then
                if HR.Cast(S.ElunesGuidance, Settings.Feral.OffGCDasOffGCD.ElunesGuidance) then return "Cast"; end
              end
              if HR.CDsON() then
                ...
                if S.Incarnation:IsCastable() and Player:EnergyPredicted() >= 30 and (S.TigersFury:CooldownRemainsP() > 15 or Player:Buff(S.TigersFury)) then
                  if HR.Cast(S.Incarnation, Settings.Feral.OffGCDasOffGCD.Berserk) then return "Cast"; end
                end
                ...
                if S.AshamanesFrenzy:IsCastable() and Player:ComboPoints() >= 2 and (not S.Bloodtalons:IsAvailable() or Player:Buff(S.BloodtalonsBuff)) then
                  if HR.Cast(S.AshamanesFrenzy) then return "Cast"; end
                end
                ...
                if S.Shadowmeld:IsCastable() and Player:ComboPoints() < 5 and Player:Energy() >= S.Rake:Cost() and Target:PMultiplier(S.Rake) < 2.1
                  ...
                  if Settings.Feral.StealthMacro.Shadowmeld then
                    if HR.CastQueue(S.Shadowmeld, S.Rake) then return "Shadowmeld to Rake as Macro"; end
                  else
                    if HR.Cast(S.Shadowmeld, Settings.Commons.OffGCDasOffGCD.Racials) then return "Shadowmeld to Rake"; end
                  end
                end
              end
          end
          ...
          if Player:ComboPoints() > 4 and Target:IsInRange(MeleeRange) then
            ...
            if S.SavageRoar:IsCastable() and not Player:Buff(S.SavageRoar) then
              if HR.Cast(S.SavageRoar) then return "Cast"; end
            end
            ...
            if S.Maim:IsCastable() and Player:Buff(S.FieryRedMaimers) then
              if HR.Cast(S.Maim) then return "Cast"; end
            end
            ...
            if S.FerociousBite:IsCastable() then
              if Player:EnergyPredicted() < 50 and (Target:HealthPercentage() >= 25 or Player:EnergyTimeToX(50) < Target:DebuffRemainsP(S.Rip)) then
                if HR.Cast(S.PoolEnergy) then return "Pooling for Ferocious Bite"; end
              else
                if HR.Cast(S.FerociousBite) then return "Cast"; end
              end
            end
          end
          -- run_action_list,name=st_generators
            if S.Regrowth:IsCastable() then
              if S.Bloodtalons:IsAvailable() and Player:Buff(S.PredatorySwiftness) and not Player:Buff(S.BloodtalonsBuff) then
                ...
                if Player:ComboPoints() >= 2 and (S.AshamanesFrenzy:CooldownRemainsP() < Player:GCD()) then
                  if HR.Cast(S.Regrowth) then return "Cast"; end
                end
            ...
            if S.BrutalSlash:IsCastable() and Cache.EnemiesCount[AoERadius] > 1 then
              if HR.Cast(S.BrutalSlash) then return "Cast"; end
            end
            ...
            if S.LunarInspiration:IsAvailable() and S.MoonfireCat:IsCastable(RangedRange) and Target:DebuffRefreshableP(S.MoonfireCat, MoonfireThreshold) then
              if HR.Cast(S.MoonfireCat) then return "Cast"; end
            end
            if S.Thrash:IsCastable(ThrashRadius, true) and Target:FilteredTimeToDie(">=", 6, -Target:DebuffRemainsP(S.Thrash)) and Target:DebuffRefreshableP(S.Thrash, ThrashThreshold)
              ...
              -- thrash_cat,if=refreshable&variable.use_thrash=1&buff.clearcasting.react
                ...
                if HR.Cast(S.Thrash) then return "Cast"; end
            end
            -- swipe_cat,if=spell_targets.swipe_cat>1
            if S.Swipe:IsCastable() and HR.AoEON() and Cache.EnemiesCount[AoERadius] >= 2 then
              if HR.Cast(S.Swipe) then return "Cast"; end
            end
            ...
            if HR.Cast(S.PoolEnergy) then return "Pooling"; end
        end
        ...
        -- ashamanes_frenzy
        if HR.CDsON() and S.AshamanesFrenzy:IsCastable() then
          if HR.Cast(S.AshamanesFrenzy) then return "Cast"; end
        end
