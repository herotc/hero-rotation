...
local function EvaluateCyclePrimalWrath95(TargetUnit)
  return HR.AoEON() and Cache.EnemiesCount[MeleeRange] > 1 and TargetUnit:DebuffRemainsP(S.RipDebuff) < 4
end

local function EvaluateCyclePrimalWrath106(TargetUnit)
  return HR.AoEON() and Cache.EnemiesCount[MeleeRange] >= 2
end
...
local function Cooldowns()
  ...
  if S.Thorns:IsCastableP() and HR.AoEON() and (Cache.EnemiesCount[EightRange] > 1) then
  ...
  if S.FocusedAzeriteBeam:IsCastableP() and ((HR.AoEON() and Cache.EnemiesCount[EightRange] > 1) or Settings.Feral.UseFABST) then
  ...
  if S.PurifyingBlast:IsCastableP() and HR.AoEON() and (Cache.EnemiesCount[EightRange] > 1) then
...
local function Generators()
  ...
  if S.BrutalSlash:IsCastableP() and HR.AoEON() and (Cache.EnemiesCount[EightRange] > 1) then
  ...
  if S.BrutalSlash:IsCastableP() and HR.AoEON() and ((Player:BuffP(S.TigersFuryBuff) and (10000000000 > (1 + S.BrutalSlash:MaxCharges() - S.BrutalSlash:ChargesFractionalP()) * S.BrutalSlash:RechargeP())) and (Cache.EnemiesCount[EightRange] * S.BrutalSlash:Damage() % S.BrutalSlash:Cost()) > (S.Shred:Damage() % S.Shred:Cost())) then
...
local function Opener()
  ...
  if S.PrimalWrath:IsCastableP() and HR.AoEON() and (S.PrimalWrath:IsAvailable() and Target:DebuffDownP(S.RipDebuff) and Cache.EnemiesCount[EightRange] >= 2) then
