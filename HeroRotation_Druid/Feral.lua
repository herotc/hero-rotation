...
local function EvaluateTargetIfAdaptiveSwarm(TargetUnit)
  -- if=buff.cat_form.up&dot.adaptive_swarm_damage.stack<3&talent.unbridled_swarm.enabled&spell_targets.swipe_cat>1
...
local function EvaluateCycleRip2(TargetUnit)
  -- target_if=refreshable,if=(!talent.primal_wrath|spell_targets=1)&(buff.bloodtalons.up|!talent.bloodtalons)&(buff.tigers_fury.up|dot.rip.remains<cooldown.tigers_fury.remains)
  -- Note: (!talent.primal_wrath|spell_targets=1)&(buff.bloodtalons.up|!talent.bloodtalons) checked before CastCycle.
  return TargetUnit:DebuffRefreshable(S.RipDebuff) and (Player:BuffUp(S.TigersFury) or TargetUnit:DebuffRemains(S.RipDebuff) < S.TigersFury:CooldownRemains())
end
...
local function Finisher()
  ...
  if BiteFinisher:IsReady() and (Player:BuffDown(BsInc) or not S.SouloftheForest:IsAvailable()) then
...
local function Berserk()
  ...
  if S.Shadowmeld:IsReady() and not Player:StealthUp(false, true) and (not (BTBuffUp(S.Rake) and CountActiveBtTriggers() == 2) and S.Rake:IsReady() and Player:BuffDown(S.SuddenAmbushBuff) and (DebuffRefreshAny(EnemiesMelee, S.RakeDebuff) or Target:PMultiplier(S.Rake) < 1.4)) then
  ...
  if S.Rake:IsReady() and (not (BTBuffUp(S.Rake) and CountActiveBtTriggers() == 2) and (Target:DebuffRemains(S.RakeDebuff) < 3 or Player:BuffUp(S.SuddenAmbushBuff) and Player:PMultipler(S.Rake) > Target:PMultiplier(S.Rake))) then
  ...
  if S.ThrashCat:IsCastable() and (not S.ThrashingClaws:IsAvailable() and Target:DebuffRefreshable(S.ThrashCatDebuff)) then
    if Cast(S.ThrashCat, nil, nil, not IsInAoERange) then return "thrash berserk 10"; end
  end
  ...
  if S.Swipe:IsReady() and (CountActiveBtTriggers() == 2 and BTBuffDown(S.Swipe) and S.WildSlashes:IsAvailable()) then
    if Cast(S.Swipe, nil, nil, not IsInAoERange) then return "swipe berserk 16"; end
  end
  -- brutal_slash,if=cooldown.brutal_slash.charges>1&buff.bt_swipe.down
  if S.BrutalSlash:IsReady() and (S.BrustalSlash:Charges() > 1 and BTBuffDown(S.Swipe)) then
    if Cast(S.BrutalSlash, nil, nil, not IsInMeleeRange) then return "brutal_slash berserk 18"; end
  end
  ...
  if S.BrutalSlash:IsReady() and (S.BrutalSlash:Charges() > 1) then
    if Cast(S.BrutalSlash, nil, nil, not IsInMeleeRange) then return "brutal_slash berserk 22"; end
  end
  ...
  if S.Swipe:IsReady() and (BTBuffDown(S.Swipe) and S.WildSlashes:IsAvailable()) then
    if Cast(S.Swipe, nil, nil, not IsInAoERange) then return "swipe berserk 24"; end
  end
  -- shred
...
local function Cooldown()
  ...
  if Settings.Commons.Enabled.Items then
    ...
    if ItemToUse and ItemToUse:IsReady() then
      if Cast(ItemToUse, nil, Settings.CommonsDS.DisplayStyle.Items, not Target:IsInRange(ItemRange)) then return "Generic use_item for " .. ItemToUse:Name() .. " cooldown 12"; end
    end
  end
  -- use_item,name=ovinaxs_mercurial_egg,if=!in_combat
  -- Note: Can't get here while not in combat. Ignoring...
  ...
  if Settings.Commons.Enabled.Trinkets then
    ...
    if Trinket1:IsReady() and not VarTrinket1Ex and not Player:IsItemBlacklisted(Trinket1) and (VarTrinket1ID ~= I.OvinaxsMercurialEgg:ID() and not VarTrinket1Buffs and (Trinket2:CooldownRemains() > 20 or not VarTrinket2Buffs or Trinket2:CooldownDown() and S.TigersFury:CooldownRemains() > 20)) then
    ...
    if Trinket2:IsReady() and not VarTrinket2Ex and not Player:IsItemBlacklisted(Trinket2) and (VarTrinket2ID ~= I.OvinaxsMercurialEgg:ID() and not VarTrinket2Buffs and (Trinket1:CooldownRemains() > 20 or not VarTrinket1Buffs or Trinket1:CooldownDown() and S.TigersFury:CooldownRemains() > 20)) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    -- Note: variable.regrowth was removed from this condition. Leaving it in the HR profile, so the user can still toggle healing spells off, if wanted.
    ...
    if BiteFinisher:IsReady() and (Player:BuffUp(S.ApexPredatorsCravingBuff) and not (VarNeedBT and CountActiveBtTriggers() == 2)) then
      if Cast(BiteFinisher, nil, nil, not IsInMeleeRange) then return "ferocious_bite main 14"; end
    end
    ...
    if S.AdaptiveSwarm:IsReady() and (Player:BuffUp(S.CatForm) and not S.UnbridledSwarm:IsAvailable() or EnemiesCount8y == 1) then
      if Everyone.CastCycle(S.AdaptiveSwarm, Enemies8y, EvaluateCycleAdaptiveSwarm, not Target:IsSpellInRange(S.AdaptiveSwarm), nil, Settings.CommonsDS.DisplayStyle.AdaptiveSwarm) then return "adaptive_swarm main 16"; end
    end
    ...
    if S.AdaptiveSwarm:IsReady() and (Player:BuffUp(S.CatForm) and S.UnbridledSwarm:IsAvailable() and EnemiesCount8y > 1) then
      if Everyone.CastTargetIf(S.AdaptiveSwarm, Enemies8y, "max", EvaluateTargetIfFilterAdaptiveSwarm, EvaluateTargetIfAdaptiveSwarm, not Target:IsSpellInRange(S.AdaptiveSwarm), nil, Settings.CommonsDS.DisplayStyle.AdaptiveSwarm) then return "adaptive_swarm main 18"; end
    end
...
local function OnInit()
  ...
  HR.Print("Feral Druid rotation has been updated for patch 11.0.5.")
end
