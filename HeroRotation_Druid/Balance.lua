...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if Settings.Commons.Enabled.Trinkets then
      ...
      -- use_item,slot=trinket2,if=!trinket.2.is.spymasters_web&!trinket.2.is.imperfect_ascendancy_serum&!trinket.2.is.treacherous_transmitter&!trinket.2.is.soulletting_ruby&(variable.on_use_trinket!=2&trinket.1.cooldown.remains>20|fight_remains<(20+20*(trinket.1.has_use&trinket.1.cooldown.remains<25))|variable.generic_trinket_condition)
      -- Note: Initial "trinket.2.is" checks are basically OnUseExcludes items, which are already excluded via VarTrinket2Ex.
      if Trinket2 and Trinket2:IsReady() and not VarTrinket2Ex and not Player:IsItemBlacklisted(Trinket2) and (VarOnUseTrinket ~= 2 and Trinket1:CooldownRemains() > 20 or BossFightRemains < (20 + 20 * num(Trinket1:HasUseBuff() and Trinket1:CooldownRemains() < 25)) or VarGenericTrinketCondition) then
