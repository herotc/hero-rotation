...
local function Aoe()
  ...
  if S.ConvoketheSpirits:IsCastable() and (CDsON() and ((VarConvokeDesync and CaInc:CooldownDown() and (not PAPEquipped) or Player:BuffUp(CaInc) and ((not PAPEquipped) or CelestialSpiritsEquipped or FightRemains > 127 or FightRemains < CaInc:CooldownRemains() + 10 or Player:BuffRemains(CaInc) > 12)) and (((Player:AstralPowerP() <= VarConvokeAsp or EnemiesCount8ySplash > 3) and (Player:BuffRemains(S.EclipseLunar) > 10 or Player:BuffRemains(S.EclipseSolar) > 10) or Player:BuffRemains(CaInc) > 5 and Player:BuffRemains(CaInc) <= 12) and ((not S.StellarDrift:IsAvailable()) or Player:BuffRemains(S.StarfallBuff) < 1.5 - 0.5 * num(CelestialSpiritsEquipped) or Player:BuffRemains(CaInc) < Player:BuffRemains(S.StarfallBuff) + 5 and Player:BuffUp(CaInc)) and ((not TimewornEquipped) or (not CelestialSpiritsEquipped) or Player:BuffRemains(S.TimewornDreambinderBuff) > S.ConvoketheSpirits:ExecuteTime()) or BOATEquipped) and ((not BOATEquipped) or (Player:BuffStack(S.BOATNatureBuff) == 8 or Player:BuffStack(S.BOATArcaneBuff) == 8)) or FightRemains < 10 and CaInc:CooldownDown())) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    -- berserking,if=((!covenant.night_fae|!cooldown.convoke_the_spirits.up|!runeforge.balance_of_all_things)&buff.ca_inc.remains>15&!covenant.venthyr|covenant.venthyr&buff.ca_inc.up&buff.ravenous_frenzy.up&(buff.ravenous_frenzy.remains<=12-4*runeforge.sinful_hysteria|buff.ca_inc.remains<11|1%spell_haste<(2.1-talent.stellar_flare.enabled*0.1-druid.delay_berserking*0.5)))&variable.in_gcd
    if S.Berserking:IsCastable() and CDsON() and ((CovenantID ~= 3 or not S.ConvoketheSpirits:CooldownUp() or not BOATEquipped) and Player:BuffRemains(CaInc) > 15 and CovenantID ~= 2 or CovenantID == 2 and Player:BuffUp(CaInc) and Player:BuffUp(S.RavenousFrenzyBuff) and (Player:BuffRemains(S.RavenousFrenzyBuff) <= 12 - 4 * num(SinfulHysteriaEquipped) or Player:BuffRemains(CaInc) < 11 or 1 / Player:SpellHaste() < (2.1 - num(S.StellarFlare:IsAvailable()) * 0.1 - num(Settings.Balance.DelayBerserking) * 0.5))) then
