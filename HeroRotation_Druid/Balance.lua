...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if (Settings.Commons.Enabled.Trinkets) then
      ...
      if I.ShadowedOrbofTorment:IsEquippedAndReady() and ((CovenantID ~= 2 or (CaInc:CooldownUp() and (S.Berserking:CooldownRemains() < 2 or not Player:Race() == "Troll")) and and VarThrillSeekerWait) and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownRemains() < 20) and Target:DebuffUp(S.SunfireDebuff) and (Target:DebuffUp(S.StellarFlareDebuff) or (not S.StellarFlare:IsAvailable()) or EnemiesCount8ySplash > 3) and Target:DebuffUp(S.MoonfireDebuff) and (not I.InscrutableQuantumDevice:IsEquipped()) or I.InscrutableQuantumDevice:IsEquipped() and I.InscrutableQuantumDevice:CooldownRemains() > 30 and Player:BuffDown(CaInc) or FightRemains < 40) then
