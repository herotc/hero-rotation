...
local function St()
  ...
  VarDotRequirements = ((Player:BuffRemains(S.RavenousFrenzyBuff) > 5 or Player:BuffDown(S.RavenousFrenzyBuff)) and not (Player:BuffUp(S.RavenousFrenzyBuff) and SinfulHysteriaEquipped) and (Player:BuffRemains(S.KindredEmpowermentEnergizeBuff) < GCDMax) and (Player:BuffRemains(S.EclipseSolar) > GCDMax or Player:BuffRemains(S.EclipseLunar) > GCDMax or (Player:BuffDown(S.EclipseLunar) or Player:BuffDown(S.EclipseSolar)) and not S.Solstice:IsAvailable()))
  ...
  VarCDConditionST = (CDsON() and VarCDCondition and ((I.EmpyrealOrdinance:CooldownRemains() < 167 + (11 * num(SinfulHysteriaEquipped)) and I.EmpyrealOrdinance:IsEquipped()) or Player:AstralPowerP() >= 90 and ((not S.FuryofElune:IsAvailable()) or Player:BuffDuration(CaInc) > S.FuryofElune:CooldownRemains() + 8) and Target:DebuffRemains(S.SunfireDebuff) > 8 and Target:DebuffRemains(S.MoonfireDebuff) > 9 and (Target:DebuffRemains(S.StellarFlareDebuff) > 10 or not S.StellarFlare:IsAvailable()) and VarThrillSeekerWait and (Player:BuffUp(S.KindredEmpowermentEnergizeBuff) or CovenantID ~= 1) and Target:TimeToDie() > Player:BuffDuration(CaInc) * 0.7 or fightRemains < Player:BuffDuration(CaInc) or CovenantID == 3 or Player:BloodlustUp() and Player:BloodlustRemains() < Player:BuffDuration(CaInc) + (9 * num(PAPEquipped))) and Player:BuffDown(CaInc) and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownUp() or fightRemains < S.ConvoketheSpirits:CooldownRemains() + 6 or fightRemains % 180 < Player:BuffDuration(CaInc)))
  -- ravenous_frenzy,if=buff.ca_inc.remains>15|buff.ca_inc.duration<32&variable.cd_condition_st
  if S.RavenousFrenzy:IsCastable() and (Player:BuffRemains(CaInc) > 15 or Player:BuffDuration(CaInc) < 32 and VarCDConditionST) then
  ...
  if S.FuryofElune:IsCastable() and (EclipseInAny and AP_Check(S.FuryofElune) and PAPValue < 240 and (Target:DebuffUp(S.AdaptiveSwarmDebuff) or CovenantID ~= 4) and VarSaveForCAInc and (Player:BuffRemains(S.RavenousFrenzyBuff) < 9 - (5 * num(SinfulHysteriaEquipped) + num(not I.InstructorsDivineBell:IsEquipped())) and Player:BuffUp(S.RavenousFrenzyBuff) or Player:BuffDown(S.RavenousFrenzyBuff))) then
  ...
  if S.Starsurge:IsReady() and (Player:BuffUp(S.OnethsClearVisionBuff) or Player:BuffUp(S.KindredEmpowermentEnergizeBuff) or Player:BuffUp(CaInc) and (Player:BuffRemains(S.RavenousFrenzyBuff) < (GCDMax * ceil(Player:AstralPowerP() / 30) + 3 * num(S.NewMoon:IsAvailable())) and (not SinfulHysteriaEquipped) and Player:BuffUp(S.RavenousFrenzyBuff) or Player:BuffRemains(CaInc) < (GCDMax * ceil(Player:AstralPowerP() / 30)) or (Player:BuffUp(S.RavenousFrenzyBuff) and SinfulHysteriaEquipped) or (Player:BuffRemains(S.RavenousFrenzyBuff) < S.Starfire:ExecuteTime() + 0.5 and Player:SpellHaste() < 0.4 or Player:BuffRemains(S.RavenousFrenzyBuff) < S.Wrath:ExecuteTime() + 0.5 or VarIQDCondition and I.InscrutableQuantumDevice:IsEquippedAndReady()) and Player:BuffUp(S.RavenousFrenzyBuff) or Player:BuffUp(S.RavenousFrenzyBuff) and S.RavenousFrenzy:CooldownDown() or CovenantID ~= 2) or Player:AstralPowerP() > 90 and EclipseInAny) then
...
local function Aoe()
  ...
  VarCDConditionAoE = (CDsON() and VarCDCondition and ((Player:BuffUp(S.StarfallBuff) or S.StellarDrift:IsAvailable() or CovenantID == 3) and (Player:BuffDown(S.SolsticeBuff) and Player:BuffDown(CaInc) and ((not S.FuryofElune:IsAvailable()) or Player:BuffDuration(CaInc) > S.FuryofElune:CooldownRemains() + 8) and VarThrillSeekerWait and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownUp() and (Player:BuffRemains(S.StarfallBuff) < 2 or not S.StellarDrift:IsAvailable())) and Target:TimeToDie() > Player:BuffDuration(CaInc) * 0.7) or fightRemains < Player:BuffDuration(CaInc)))
  -- ravenous_frenzy,if=buff.ca_inc.remains>15|buff.ca_inc.duration<30&variable.cd_condition_aoe
  if S.RavenousFrenzy:IsCastable() and (Player:BuffRemains(CaInc) > 15 or Player:BuffDuration(CaInc) < 30 and VarCDConditionAoE) then
  ...
  if S.CelestialAlignment:IsCastable() and (VarCDConditionAoE and (Player:BuffDuration(CaInc) >= 30 or CovenantID ~= 2) or Player:BuffUp(S.RavenousFrenzyBuff) and Player:BuffRemains(S.RavenousFrenzyBuff) < 9 + PATime + num(Player:BloodlustDown() and not S.Starlord:IsAvailable())) then
  ...
  if S.Starsurge:IsReady() and (Player:BuffUp(S.OnethsClearVisionBuff) or ((not AP_Check(S.Starfire)) and (not VarIgnoreStarsurge) or (Player:BuffRemains(CaInc) < 5 and Player:BuffUp(CaInc) or ((Player:BuffRemains(CaInc) < GCDMax * ceil(Player:AstralPowerP() / 30) and Player:BuffUp(CaInc) or (SinfulHysteriaEquipped and Player:BuffRemains(S.RavenousFrenzyBuff) < GCDMax * ceil(Player:AstralPowerP() / 30) and Player:BuffUp(S.RavenousFrenzyBuff))) and CovenantID == 2)) and (EnemiesCount40y < 3 or VarStarfallWontFallOff)) and (not VarIgnoreStarsurge) and ((not TimewornEquipped) or EnemiesCount40y < 3)) then
...
local function Boat()
  ...
  if S.FuryofElune:IsCastable() and (CaInc:CooldownUp() and VarCDCondition and (Player:AstralPowerP() > 90 and CovenantID ~= 3 or CovenantID == 3 and Player:AstralPowerP() < 40) and CovenantID ~= 2 and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownUp()) and CDsON()) then
  ...
  VarCDConditionBOAT = (CDsON() and VarCDCondition and ((Player:AstralPowerP() > 90 and (Player:BuffUp(S.KindredEmpowermentEnergizeBuff) or CovenantID ~= 1) or Player:BloodlustUp() and Player:BloodlustRemains() < Player:BuffDuration(CaInc)) or fightRemains < Player:BuffDuration(CaInc) or CovenantID == 3) and (CovenantID ~= 3 or (Player:AstralPowerP() < 40 or Target:DebuffUp(S.FuryofElune)) and (VarConvokeDesync or S.ConvoketheSpirits:CooldownUp())))
