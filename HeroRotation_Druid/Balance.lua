...
local function St()
  ...
  VarCDConditionST = (CDsON() and VarCDCondition and ((I.EmpyrealOrdinance:CooldownRemains() < 167 + (11 * num(SinfulHysteriaEquipped)) and I.EmpyrealOrdinance:IsEquipped()) or Player:AstralPowerP() >= 90 and ((not S.FuryofElune:IsAvailable()) or Player:BaseDuration(CaInc) > S.FuryofElune:CooldownRemains() + 8) and Target:DebuffRemains(S.SunfireDebuff) > 8 and Target:DebuffRemains(S.MoonfireDebuff) > 9 and (Target:DebuffRemains(S.StellarFlareDebuff) > 10 or not S.StellarFlare:IsAvailable()) and VarThrillSeekerWait and (Player:BuffUp(S.KindredEmpowermentEnergizeBuff) or CovenantID ~= 1) and Target:TimeToDie() > Player:BaseDuration(CaInc) * 0.7 or fightRemains < Player:BaseDuration(CaInc) or CovenantID == 3 or Player:BloodlustUp() and Player:BloodlustRemains() < Player:BaseDuration(CaInc) + (9 * num(PAPEquipped))) and Player:BuffDown(CaInc) and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownUp() or fightRemains < S.ConvoketheSpirits:CooldownRemains() + 6 or fightRemains % 180 < Player:BaseDuration(CaInc)))
  -- ravenous_frenzy,if=buff.ca_inc.remains>15|buff.ca_inc.duration<32&variable.cd_condition_st
  if S.RavenousFrenzy:IsCastable() and (Player:BuffRemains(CaInc) > 15 or Player:BaseDuration(CaInc) < 32 and VarCDConditionST) then
  ...
  if (CDsON()) then
    ...
    if S.CelestialAlignment:IsCastable() and (VarCDConditionST and (Player:BaseDuration(CaInc) >= 32 or CovenantID ~= 2) or Player:BuffUp(S.RavenousFrenzyBuff) and Player:BuffRemains(S.RavenousFrenzyBuff) < 9 + PATime + num(Player:BloodlustDown() and not S.Starlord:IsAvailable())) then
...
local function Aoe()
  ...
  VarCDConditionAoE = (CDsON() and VarCDCondition and ((Player:BuffUp(S.StarfallBuff) or S.StellarDrift:IsAvailable() or CovenantID == 3) and (Player:BuffDown(S.SolsticeBuff) and Player:BuffDown(CaInc) and ((not S.FuryofElune:IsAvailable()) or Player:BaseDuration(CaInc) > S.FuryofElune:CooldownRemains() + 8) and VarThrillSeekerWait and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownUp() and (Player:BuffRemains(S.StarfallBuff) < 2 or not S.StellarDrift:IsAvailable())) and Target:TimeToDie() > Player:BaseDuration(CaInc) * 0.7) or fightRemains < Player:BaseDuration(CaInc)))
  -- ravenous_frenzy,if=buff.ca_inc.remains>15|buff.ca_inc.duration<30&variable.cd_condition_aoe
  if S.RavenousFrenzy:IsCastable() and (Player:BuffRemains(CaInc) > 15 or Player:BaseDuration(CaInc) < 30 and VarCDConditionAoE) then
  ...
  if (CDsON()) then
    ...
    if S.CelestialAlignment:IsCastable() and (VarCDConditionAoE and (Player:BaseDuration(CaInc) >= 30 or CovenantID ~= 2) or Player:BuffUp(S.RavenousFrenzyBuff) and Player:BuffRemains(S.RavenousFrenzyBuff) < 9 + PATime + num(Player:BloodlustDown() and not S.Starlord:IsAvailable())) then
...
local function Boat()
  ...
  VarCDConditionBOAT = (CDsON() and VarCDCondition and ((Player:AstralPowerP() > 90 and (Player:BuffUp(S.KindredEmpowermentEnergizeBuff) or CovenantID ~= 1) or Player:BloodlustUp() and Player:BloodlustRemains() < Player:BaseDuration(CaInc)) or fightRemains < Player:BaseDuration(CaInc) or CovenantID == 3) and (CovenantID ~= 3 or (Player:AstralPowerP() < 40 or Target:DebuffUp(S.FuryofElune)) and (VarConvokeDesync or S.ConvoketheSpirits:CooldownUp())))
  if (CDsON()) then
