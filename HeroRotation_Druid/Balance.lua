...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    VarCDCondition = (FightRemains > 15 and ((I.EmpyrealOrdinance:IsEquipped() and (CovenantID ~= 2 or I.EmpyrealOrdinance:CooldownRemains() < 167 + (5 * num(SinfulHysteriaEquipped))) and I.EmpyrealOrdinance:CooldownDown()) or I.SoullettingRuby:IsEquipped() and (CovenantID ~= 3 or I.SoullettingRuby:CooldownRemains() < 114) or (S.Berserking:CooldownUp() or Player:Race() ~= "Troll" or CovenantID == 3) and ((not S.FuryofElune:IsAvailable()) or CovenantID == 2 and S.FuryofElune:CooldownRemains() < 11 + 5 * num(SinfulHysteriaEquipped) or CovenantID ~= 2 and S.FuryofElune:CooldownRemains() < CaInc:BaseDuration() + 12 * num(PAPEquipped) - 12) and (I.InscrutableQuantumDevice:IsEquipped() and I.InscrutableQuantumDevice:CooldownRemains() < 5) or ((I.ShadowedOrbofTorment:IsEquipped() and I.ShadowedOrbofTorment:CooldownRemains() > 0) or ((VarOnUseTrinket == 1 or VarOnUseTrinket == 3) and (trinket1:IsReady() or trinket1:CooldownRemains() > FightRemains - 10) or VarOnUseTrinket == 2 and (trinket2:IsReady() or trinket2:CooldownRemains() > FightRemains - 10) or VarOnUseTrinket == 0))))
