  ...
  if IsInMeleeRange() then
    ...
    if S.SymbolsofDeath:IsCastable() then
      if not S.DeathfromAbove:IsAvailable() then
        ...
        if AR.AoEON() and S.DeathfromAbove:CooldownRemains() <= 3 and (Target:DebuffRemains(S.Nightblade) >= S.DeathfromAbove:CooldownRemains() + 3 or Target:FilteredTimeToDie("<=", 6) or not Target:TimeToDieIsNotValid()) then
          if AR.Cast(S.SymbolsofDeath, Settings.Subtlety.OffGCDasOffGCD.SymbolsofDeath) then return ""; end
        end
      end
    end
    ...
    if AR.AoEON() and (not S.DeathfromAbove:IsAvailable() or AC.Tier19_2Pc) and (not S.DarkShadow:IsAvailable() or not Player:Buff(S.ShadowDanceBuff)) and Rogue.MantleDuration() == 0 then
      ...
      for _, Unit in pairs(Cache.Enemies[5]) do
        if Everyone.UnitIsCycleValid(Unit, BestUnitTTD, -Unit:DebuffRemains(S.Nightblade))
          ...
          and ((Unit:DebuffRefreshable(S.Nightblade, NightbladeThreshold) and (not AC.Finality(Unit) or Player:Buff(S.FinalityNightblade)))
            or Unit:DebuffRemains(S.Nightblade) < 4) then
...
local function APL ()
  ...
  -- In Combat
    ...
    if Everyone.TargetIsValid() then
      ...
      if Player:ComboPoints() >= 5 or (Player:ComboPoints() >= 4 and Cache.EnemiesCount[8] >= 3 and Cache.EnemiesCount[8] <= 4)
