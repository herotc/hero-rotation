...
--- ===== Rotation Variables =====
local VarTotMMaxStacks = S.KnowledgeoftheBrokenTemple:IsAvailable() and 8 or 4
...
local function Precombat()
  ...
  -- variable,name=invoke_xuen_count,op=set,value=0
  -- Note: Reset in PLAYER_REGEN_ENABLED event registration.
...
local function Cooldowns()
  ...
  local VarXuenDungeonsliceCondition = EnemiesCount8y == 1 and (CombatTime < 10 or S.XuensBond:IsAvailable() and S.CelestialConduit:IsAvailable() and Target:TimeToDie() > 14) or EnemiesCount8y > 1 and S.StormEarthAndFire:CooldownUp() and Target:TimeToDie() > 14 and (EnemiesCount8y > 2 or Target:DebuffUp(S.AcclamationDebuff) or not S.OrderedElements:IsAvailable() and CombatTime < 5) and ((Chi > 2 and not S.OrderedElements:IsAvailable() or S.OrderedElements:IsAvailable() or not S.OrderedElements:IsAvailable() and Energy < 50) or S.SequencedStrikes:IsAvailable() and S.EnergyBurst:IsAvailable() and S.RevolvingWhirl:IsAvailable()) or BossFightRemains < 30 or EnemiesCount8y > 3 and Target:TimeToDie() > 5 or DungeonSlice and CombatTime > 50 and Target:TimeToDie() > 1 and S.XuensBond:IsAvailable()
  ...
  local VarXuenCondition = (DungeonSlice and EnemiesCount8y == 1 and (CombatTime < 10 or S.XuensBond:IsAvailable() and S.CelestialConduit:IsAvailable()) or not DungeonSlice or EnemiesCount8y > 1) and S.StormEarthAndFire:CooldownUp() and (Target:TimeToDie() > 14 and not DungeonSlice or Target:TimeToDie() > 22) and (EnemiesCount8y > 2 or Target:DebuffUp(S.AcclamationDebuff) or not S.OrderedElements:IsAvailable() and CombatTime < 5) and (Chi > 2 and S.OrderedElements:IsAvailable() or Chi > 5 or Chi > 3 and Energy < 50 or Energy < 50 and EnemiesCount8y == 1 or Player:PrevGCD(1, S.TigerPalm) and not S.OrderedElements:IsAvailable() and CombatTime < 5) or BossFightRemains < 30 or DungeonSlice and S.CelestialConduit:IsAvailable() and Target:TimeToDie() > 14
  ...
  local VarXuenDungeonrouteCondition = S.StormEarthAndFire:CooldownUp() and (EnemiesCount8y > 1 and S.StormEarthAndFire:CooldownUp() and Target:TimeToDie() > 22 and (EnemiesCount8y > 2 or Target:DebuffUp(S.AcclamationDebuff) or not S.OrderedElements:IsAvailable() and CombatTime < 5) and ((Chi > 2 and not S.OrderedElements:IsAvailable() or S.OrderedElements:IsAvailable() or not S.OrderedElements:IsAvailable() and Energy < 50) or S.SequencedStrikes:IsAvailable() and S.EnergyBurst:IsAvailable() and S.RevolvingWhirl:IsAvailable()) or BossFightRemains < 30 or EnemiesCount8y > 3 and Target:TimeToDie() > 15 or CombatTime > 50 and (Target:TimeToDie() > 10 and S.XuensBond:IsAvailable() or Target:TimeToDie() > 20)) or Player:BuffRemains(S.StormEarthAndFireBuff) > 5
  ...
  if S.TigerPalm:IsReady() and ((Target:TimeToDie() > 14 and not DungeonSlice or Target:TimeToDie() > 22) and S.InvokeXuenTheWhiteTiger:CooldownUp() and (Chi < 5 and not S.OrderedElements:IsAvailable() or Chi < 3) and (ComboStrike(S.TigerPalm) or not S.HitCombo:IsAvailable())) then
...
local function NormalOpener()
  ...
  if S.TigerPalm:IsReady() and (Chi < 6 and ComboStrike(S.TigerPalm)) then
...
local function DefaultAoE()
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.GaleForce:IsAvailable() and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10 and TWW3_2pc and not S.FlurryStrikes:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_aoe 8"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffStack(S.ChiEnergyBuff) > 29 and S.FistsofFury:CooldownRemains() < 5) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_aoe 14"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffStack(S.DanceofChijiBuff) < 2) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_aoe 16"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffRemains(S.StormEarthAndFireBuff) > 2 and S.FlurryStrikes:IsAvailable() and EnergyTTMCheck and S.FistsofFury:CooldownRemains() and (not S.XuensBattlegear:IsAvailable() or Chi < 6) and TWW3_4pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 18"; end
  end
  ...
  if S.CelestialConduit:IsReady() and (Player:BuffUp(S.StormEarthAndFireBuff) and S.StrikeoftheWindlord:CooldownDown() and (not VarSmallHotjsActive or Target:DebuffRemains(S.GaleForceDebuff) < 5) and (S.XuensBond:IsAvailable() or not S.XuensBond:IsAvailable() and Player:BuffUp(S.InvokersDelightBuff)) or BossFightRemains < 15 or DungeonSlice and Player:BuffUp(S.InvokersDelightBuff) and S.StrikeoftheWindlord:CooldownDown() and Player:BuffRemains(S.StormEarthAndFireBuff) < 8) then
    if Cast(S.CelestialConduit, Settings.Windwalker.GCDasOffGCD.CelestialConduit, nil, not Target:IsInMeleeRange(15)) then return "celestial_conduit default_aoe 22"; end
  end
  -- fists_of_fury,target_if=max:target.time_to_die,if=combo_strike&buff.heart_of_the_jade_serpent_cdr_celestial.up|variable.small_hotjs_active
  if S.FistsofFury:IsReady() and (ComboStrike(S.FistsofFury) and Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff) or VarSmallHotjsActive) then
  ...
  if S.RisingSunKick:IsReady() and (S.WhirlingDragonPunch:CooldownRemains() < 2 and S.FistsofFury:CooldownRemains() > 1 and Player:BuffStack(S.DanceofChijiBuff) < 2 or Player:BuffDown(S.StormEarthAndFireBuff) and Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_aoe 26"; end
  end
  -- tiger_palm,if=combo_strike&buff.storm_earth_and_fire.remains>2&talent.flurry_strikes&energy.time_to_max<=gcd.max*3&set_bonus.tww3_4pc
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffRemains(S.StormEarthAndFireBuff) > 2 and S.FlurryStrikes:IsAvailable() and EnergyTTMCheck and TWW3_4pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 28"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (not S.RevolvingWhirl:IsAvailable() or S.RevolvingWhirl:IsAvailable() and Player:BuffStack(S.DanceofChijiBuff) < 2 and EnemiesCount8y > 2) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_aoe 30"; end
  end
  -- blackout_kick,if=combo_strike&buff.bok_proc.up&chi<2&talent.energy_burst&energy<55
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutKickBuff) and Chi < 2 and S.EnergyBurst:IsAvailable() and Energy < 55) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 32"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and ((CombatTime > 5 or Player:BuffUp(S.InvokersDelightBuff) and Player:BuffUp(S.StormEarthAndFireBuff)) and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 15 or S.FlurryStrikes:IsAvailable())) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord), Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord) then return "strike_of_the_windlord default_aoe 34"; end
  end
  -- slicing_winds
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 8 and S.ShadowboxingTreads:IsAvailable()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 38"; end
  end
  ...
  if S.FistsofFury:IsReady() and (S.FlurryStrikes:IsAvailable() or S.XuensBattlegear:IsAvailable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 5 and not DungeonSlice or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 9) or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsInMeleeRange(8)) then return "fists_of_fury default_aoe 42"; end
  end
  -- tiger_palm,if=combo_strike&energy.time_to_max<=gcd.max*3&talent.flurry_strikes&buff.wisdom_of_the_wall_flurry.up&chi<6
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and Player:BuffUp(S.WisdomoftheWallFlurryBuff) and Chi < 6) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 44"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=combo_strike&chi>5
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Chi > 5) then
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffStack(S.ChiEnergyBuff) > 29 and S.FistsofFury:CooldownRemains() < 5) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_aoe 48"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and S.FistsofFury:CooldownRemains() > 2) then
    if Cast(S.RisingSunKick, nil, nil, not IsInMeleeRange) then return "rising_sun_kick default_aoe 50"; end
  end
  -- tiger_palm,if=combo_strike&buff.storm_earth_and_fire.remains>2&talent.flurry_strikes&active_enemies>5&set_bonus.tww3_4pc
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffRemains(S.StormEarthAndFireBuff) > 2 and S.FlurryStrikes:IsAvailable() and EnemiesCount8y > 5 and TWW3_4pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 52"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and S.CourageousImpulse:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffStack(S.BlackoutKickBuff) == 2) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 54"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_aoe 56"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and Player:BuffUp(S.OrderedElementsBuff)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 60"; end
  end
  -- tiger_palm,if=combo_strike&chi.deficit>=2&(!buff.ordered_elements.up|energy.time_to_max<=gcd.max*3)
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and ChiDeficit >= 2 and (Player:BuffDown(S.OrderedElementsBuff) or EnergyTTMCheck)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_aoe 62"; end
  end
  ...
  if S.JadefireStomp:IsCastable() and (S.SingularlyFocusedJade:IsAvailable() or S.JadefireHarmony:IsAvailable()) then
    if Everyone.CastTargetIf(S.JadefireStomp, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_aoe 64"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.OrderedElementsBuff) and S.CraneVortex:IsAvailable() and EnemiesCount8y > 2 and Chi > 4) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_aoe 66"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) > 3 or Player:BuffUp(S.OrderedElementsBuff)) and (S.ShadowboxingTreads:IsAvailable() or Player:BuffUp(S.BlackoutKickBuff))) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 68"; end
  end
  -- blackout_kick,if=combo_strike&!cooldown.fists_of_fury.remains&chi<3
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownUp() and Chi < 3) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 70"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and S.CourageousImpulse:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutKickBuff)) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 72"; end
  end
  -- spinning_crane_kick,if=combo_strike&(chi>3|energy>55)
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and (Chi > 3 or Energy > 55)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_aoe 74"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and (Player:BuffUp(S.OrderedElementsBuff) or (Player:BuffUp(S.BlackoutKickBuff) and ChiDeficit >= 1 and S.EnergyBurst:IsAvailable())) and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_aoe 76"; end
  end
  -- blackout_kick,if=combo_strike&cooldown.fists_of_fury.remains&(chi>2|energy>60|buff.bok_proc.up)
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown() and (Chi > 2 or Energy > 60 or Player:BuffUp(S.BlackoutKickBuff))) then
  ...
  if S.JadefireStomp:IsCastable() then
    if Everyone.CastTargetIf(S.JadefireStomp, Enemies8y, "max", EvaluateTargetIfFilterAcclamation, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_aoe 80"; end
  end
  -- tiger_palm,if=combo_strike&buff.ordered_elements.up&chi.deficit>=1
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffUp(S.OrderedElementsBuff) and ChiDeficit >= 1) then
  ...
  if S.ChiBurst:IsCastable() and (Player:BuffDown(S.OrderedElementsBuff)) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then return "chi_burst default_aoe 84"; end
  end
  -- chi_burst
  if S.ChiBurst:IsCastable() then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then return "chi_burst default_aoe 86"; end
  end
...
local function DefaultCleave()
  ...
  -- strike_of_the_windlord,if=talent.gale_force&cooldown.invoke_xuen_the_white_tiger.remains>10&set_bonus.tww3_2pc&!talent.flurry_strikes
  if S.StrikeoftheWindlord:IsReady() and (S.GaleForce:IsAvailable() and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10 and TWW3_2pc and not S.FlurryStrikes:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_cleave 4"; end
  end
  -- slicing_winds,if=set_bonus.tww3_2pc&talent.celestial_conduit&variable.small_hotjs_active&!talent.flurry_strikes
  if S.SlicingWinds:IsReady() and (TWW3_2pc and S.CelestialConduit:IsAvailable() and VarSmallHotjsActive and not S.FlurryStrikes:IsAvailable()) then
  ...
  -- whirling_dragon_punch,target_if=max:target.time_to_die,if=variable.small_hotjs_active&buff.dance_of_chiji.stack<2
  if S.WhirlingDragonPunch:IsReady() and (VarSmallHotjsActive and Player:BuffStack(S.DanceofChijiBuff) < 2) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_cleave 10"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and EnemiesCount8y < 4 and S.FistsofFury:CooldownRemains() > 4) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_cleave 12"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (S.WhirlingDragonPunch:CooldownRemains() < 2 and S.FistsofFury:CooldownRemains() > 1 and Player:BuffStack(S.DanceofChijiBuff) < 2) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_cleave 14"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffStack(S.DanceofChijiBuff) == 2 and EnemiesCount8y > 3) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_cleave 16"; end
  end
  -- tiger_palm,if=combo_strike&(energy>55&talent.inner_peace|energy>60&!talent.inner_peace)&combo_strike&chi.max-chi>=2&buff.teachings_of_the_monastery.stack<buff.teachings_of_the_monastery.max_stack&(talent.energy_burst&!buff.bok_proc.up|!talent.energy_burst)&!buff.ordered_elements.up|(talent.energy_burst&!buff.bok_proc.up|!talent.energy_burst)&!buff.ordered_elements.up&!cooldown.fists_of_fury.remains&chi<3|(prev.strike_of_the_windlord|cooldown.strike_of_the_windlord.remains)&cooldown.celestial_conduit.remains<2&buff.ordered_elements.up&chi<5&combo_strike|(!variable.small_hotjs_active|!buff.heart_of_the_jade_serpent_cdr_celestial.up)&combo_strike&chi.deficit>=2&!buff.ordered_elements.up
  -- Note: Initial combo_strike condition is redundant, so it's being omitted.
  ...
  if S.TouchofDeath:CooldownUp() and (not VarSmallHotjsActive and Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff)) then
    ...
    if ToDTar then
      if ToDTar:GUID() == Target:GUID() then
        if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not IsInMeleeRange) then return "touch_of_death default_cleave 20"; end
      else
        if HR.CastLeftNameplate(ToDTar, S.TouchofDeath) then return "touch_of_death default_cleave 22"; end
      end
    end
  end
  -- whirling_dragon_punch,target_if=max:target.time_to_die,if=variable.small_hotjs_active&buff.dance_of_chiji.stack<2
  -- whirling_dragon_punch,target_if=max:target.time_to_die,if=buff.dance_of_chiji.stack<2
  -- Note: Second line supersedes the first line.
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffStack(S.DanceofChijiBuff) < 2) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_cleave 24"; end
  end
  -- slicing_winds,if=variable.small_hotjs_active|buff.heart_of_the_jade_serpent_cdr_celestial.up
  if S.SlicingWinds:IsReady() and (VarSmallHotjsActive or Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff)) then
  ...
  if S.CelestialConduit:IsReady() and (Player:BuffUp(S.StormEarthAndFireBuff) and Target:DebuffRemains(S.GaleForceDebuff) < 5 and S.StrikeoftheWindlord:CooldownDown() and (Player:BuffDown(S.HeartoftheJadeSerpentCDRBuff) or Target:DebuffRemains(S.GaleForceDebuff) < 5) and (S.XuensBond:IsAvailable() or not S.XuensBond:IsAvailable() and Player:BuffUp(S.InvokersDelightBuff)) or BossFightRemains < 15 or DungeonSlice and Player:BuffUp(S.InvokersDelightBuff) and S.StrikeoftheWindlord:CooldownDown() and Player:BuffRemains(S.StormEarthAndFireBuff) < 8 or BossFightRemains < 10) then
    if Cast(S.CelestialConduit, Settings.Windwalker.GCDasOffGCD.CelestialConduit, nil, not Target:IsInMeleeRange(15)) then return "celestial_conduit default_cleave 28"; end
  end
  -- rising_sun_kick,target_if=max:target.time_to_die,if=!pet.xuen_the_white_tiger.active&prev.tiger_palm&time<5|variable.small_hotjs_active&buff.pressure_point.up&cooldown.fists_of_fury.remains&(talent.glory_of_the_dawn|active_enemies<3)
  if S.RisingSunKick:IsReady() and (not Monk.Xuen.Active and Player:PrevGCD(1, S.TigerPalm) and CombatTime < 5 or VarSmallHotjsActive and Player:BuffUp(S.PressurePointBuff) and S.FistsofFury:CooldownDown() and (S.GloryoftheDawn:IsAvailable() or EnemiesCount8y < 3)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_cleave 30"; end
  end
  -- fists_of_fury,target_if=max:target.time_to_die,if=combo_strike&buff.heart_of_the_jade_serpent_cdr_celestial.up|variable.small_hotjs_active
  if S.FistsofFury:IsReady() and (ComboStrike(S.FistsofFury) and Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff) or VarSmallHotjsActive) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsInMeleeRange(8)) then return "fists_of_fury default_cleave 32"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff)) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_cleave 34"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.GaleForce:IsAvailable() and Player:BuffUp(S.InvokersDelightBuff) and (Player:BloodlustUp() or Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff))) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord), Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord) then return "strike_of_the_windlord default_cleave 36"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:PowerInfusionUp() and Player:BloodlustUp() and EnemiesCount8y < 3) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_cleave 40"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 8 and (EnemiesCount8y < 3 or S.ShadowboxingTreads:IsAvailable())) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 42"; end
  end
  -- tiger_palm,if=combo_strike&buff.storm_earth_and_fire.remains>2&talent.flurry_strikes&!talent.xuens_battlegear&set_bonus.tww3_4pc
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffRemains(S.StormEarthAndFireBuff) > 2 and S.FlurryStrikes:IsAvailable() and not S.XuensBattlegear:IsAvailable() and TWW3_4pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 44"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (not S.RevolvingWhirl:IsAvailable() or S.RevolvingWhirl:IsAvailable() and Player:BuffStack(S.DanceofChijiBuff) < 2 and EnemiesCount8y > 2 or EnemiesCount8y < 3) then
    if Everyone.CastTargetIf(S.WhirlingDragonPunch, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch) then return "whirling_dragon_punch default_cleave 46"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (CombatTime > 5 and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 15 or S.FlurryStrikes:IsAvailable()) and (S.FistsofFury:CooldownRemains() < 2 or S.CelestialConduit:CooldownRemains() < 10)) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_cleave 48"; end
  end
  -- slicing_winds
  if S.SlicingWinds:IsReady() then
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffStack(S.DanceofChijiBuff) == 2) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_cleave 54"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and EnemiesCount8y < 5 and Player:BuffUp(S.WisdomoftheWallFlurryBuff) and EnemiesCount8y < 4) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 56"; end
  end
  ...
  if S.FistsofFury:IsReady() and ((S.FlurryStrikes:IsAvailable() or S.XuensBattlegear:IsAvailable() or not S.XuensBattlegear:IsAvailable() and (S.StrikeoftheWindlord:CooldownRemains() > 1 or VarSmallHotjsActive or Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff))) and (S.FlurryStrikes:IsAvailable() or S.XuensBattlegear:IsAvailable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 5 and not DungeonSlice or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 9) or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterTTD, nil, not Target:IsInMeleeRange(8)) then return "fists_of_fury default_cleave 58"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and EnemiesCount8y < 5 and Player:BuffUp(S.WisdomoftheWallFlurryBuff)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 60"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffStack(S.ChiEnergyBuff) > 29) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_cleave 62"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Chi > 4 and (EnemiesCount8y < 3 or S.GloryoftheDawn:IsAvailable()) or Chi > 2 and Energy > 50 and (EnemiesCount8y < 3 or S.GloryoftheDawn:IsAvailable()) or S.FistsofFury:CooldownRemains() > 2 and (EnemiesCount8y < 3 or S.GloryoftheDawn:IsAvailable())) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterTTD, nil, not IsInMeleeRange) then return "rising_sun_kick default_cleave 64"; end
  end
  -- blackout_kick,if=talent.shadowboxing_treads&talent.courageous_impulse&combo_strike&buff.bok_proc.stack=2
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and S.CourageousImpulse:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffStack(S.BlackoutKickBuff) == 2) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 66"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and EnemiesCount8y < 5) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 74"; end
  end
  -- tiger_palm,if=combo_strike&chi.deficit>=2&(!buff.ordered_elements.up|energy.time_to_max<=gcd.max*3)
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and ChiDeficit >= 2 and (Player:BuffDown(S.OrderedElementsBuff) or EnergyTTMCheck)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 76"; end
  end
  ...
  if S.JadefireStomp:IsCastable() and (S.SingularlyFocusedJade:IsAvailable() or S.JadefireHarmony:IsAvailable()) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_cleave 80"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) > 3 or Player:BuffUp(S.OrderedElementsBuff)) and (S.ShadowboxingTreads:IsAvailable() or Player:BuffUp(S.BlackoutKickBuff) or Player:BuffUp(S.OrderedElementsBuff))) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 82"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=combo_strike&!buff.ordered_elements.up&talent.crane_vortex&active_enemies>2&chi>4
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.OrderedElementsBuff) and S.CraneVortex:IsAvailable() and EnemiesCount8y > 2 and Chi > 4) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_cleave 84"; end
  end
  -- chi_burst,if=!buff.ordered_elements.up
  if S.ChiBurst:IsCastable() and (Player:BuffDown(S.OrderedElementsBuff)) then
  ...
  -- blackout_kick,if=combo_strike&(buff.ordered_elements.up|buff.bok_proc.up&chi.deficit>=1&talent.energy_burst)&cooldown.fists_of_fury.remains
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and (Player:BuffUp(S.OrderedElementsBuff) or Player:BuffUp(S.BlackoutKickBuff) and ChiDeficit >= 1 and S.EnergyBurst:IsAvailable()) and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 88"; end
  end
  -- blackout_kick,if=combo_strike&cooldown.fists_of_fury.remains&(chi>2|energy>60|buff.bok_proc.up)
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown() and (Chi > 2 or Energy > 60 or Player:BuffUp(S.BlackoutKickBuff))) then
  ...
  -- jadefire_stomp,target_if=max:debuff.acclamation.stack
  if S.JadefireStomp:IsCastable() then
    if Everyone.CastTargetIf(S.JadefireStomp, Enemies8y, "max", EvaluateTargetIfFilterAcclamation, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_cleave 92"; end
  end
  -- tiger_palm,if=combo_strike&buff.ordered_elements.up&chi.deficit>=1
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffUp(S.OrderedElementsBuff) and ChiDeficit >= 1) then
  ...
  -- chi_burst
  if S.ChiBurst:IsCastable() then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then return "chi_burst default_cleave 96"; end
  end
  -- spinning_crane_kick,if=combo_strike&buff.ordered_elements.up&talent.hit_combo
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.OrderedElementsBuff) and S.HitCombo:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_cleave 98"; end
  end
  -- blackout_kick,if=buff.ordered_elements.up&!talent.hit_combo&cooldown.fists_of_fury.remains
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.OrderedElementsBuff) and not S.HitCombo:IsAvailable() and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 100"; end
  end
  -- blackout_kick,if=combo_strike&!cooldown.fists_of_fury.remains&prev.tiger_palm
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownUp() and Player:PrevGCD(1, S.TigerPalm)) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_cleave 102"; end
  end
  -- tiger_palm,if=prev.tiger_palm&chi<3&!cooldown.fists_of_fury.remains
  if S.TigerPalm:IsReady() and (Player:PrevGCD(1, S.TigerPalm) and Chi < 3 and S.FistsofFury:CooldownUp()) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 104"; end
  end
  -- Manually added: tiger_palm,if=chi=0 (avoids a potential profile stall)
  if S.TigerPalm:IsReady() and (Chi == 0) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_cleave 106"; end
  end
end

local function DefaultST()
  -- rising_sun_kick,if=combo_strike&buff.pressure_point.up&variable.small_hotjs_active
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and Player:BuffUp(S.PressurePointBuff) and VarSmallHotjsActive) then
    if Cast(S.RisingSunKick, nil, nil, not IsInMeleeRange) then return "rising_sun_kick default_st 2"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and S.CelestialConduit:CooldownUp() and Player:BuffUp(S.PressurePointBuff) and Chi < 5 and CombatTime < 10) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 6"; end
  end
  -- rising_sun_kick,if=combo_strike&(buff.pressure_point.up&!variable.small_hotjs_active&buff.heart_of_the_jade_serpent_cdr_celestial.up|buff.invokers_delight.up|buff.bloodlust.up|buff.pressure_point.up&cooldown.fists_of_fury.remains)
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and (Player:BuffUp(S.PressurePointBuff) and not VarSmallHotjsActive and Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff) or Player:BuffUp(S.InvokersDelightBuff) or Player:BloodlustUp() or Player:BuffUp(S.PressurePointBuff) and S.FistsofFury:CooldownDown())) then
    if Cast(S.RisingSunKick, nil, nil, not IsInMeleeRange) then return "rising_sun_kick default_st 8"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=chi<5&combo_strike&!variable.small_hotjs_active&!buff.heart_of_the_jade_serpent_cdr_celestial.up&energy.time_to_max<=gcd.max*3
  if S.TigerPalm:IsReady() and (Chi < 5 and ComboStrike(S.TigerPalm) and not VarSmallHotjsActive and Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and EnergyTTMCheck) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 10"; end
  end
  -- tiger_palm,if=combo_strike&buff.storm_earth_and_fire.remains>2&talent.flurry_strikes&set_bonus.tww3_4pc
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffRemains(S.StormEarthAndFireBuff) > 2 and S.FlurryStrikes:IsAvailable() and TWW3_4pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 12"; end
  end
  -- tiger_palm,if=combo_strike&buff.storm_earth_and_fire.remains&talent.flurry_strikes&set_bonus.tww3_4pc&!buff.bloodlust.up
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffUp(S.StormEarthAndFireBuff) and S.FlurryStrikes:IsAvailable() and TWW3_4pc and not Player:BloodlustUp()) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 14"; end
  end
  -- whirling_dragon_punch,if=!buff.heart_of_the_jade_serpent_cdr_celestial.up&!buff.dance_of_chiji.stack=2&!set_bonus.tww3_2pc
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and Player:BuffStack(S.DanceofChijiBuff) ~= 2 and not TWW3_2pc) then
    if Cast(S.WhirlingDragonPunch, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch, nil, not IsInMeleeRange) then return "whirling_dragon_punch default_st 16"; end
  end
  ...
  if S.CelestialConduit:IsReady() and (Player:BuffUp(S.StormEarthAndFireBuff) and (Player:BuffDown(S.HeartoftheJadeSerpentCDRBuff) or Target:DebuffRemains(S.GaleForceDebuff) < 5) and S.StrikeoftheWindlord:CooldownDown() and (S.XuensBond:IsAvailable() or not S.XuensBond:IsAvailable() and Player:BuffUp(S.InvokersDelightBuff)) or BossFightRemains < 15 or DungeonSlice and Player:BuffUp(S.InvokersDelightBuff) and S.StrikeoftheWindlord:CooldownDown() and Player:BuffRemains(S.StormEarthAndFireBuff) < 8 or BossFightRemains < 10) then
    if Cast(S.CelestialConduit, Settings.Windwalker.GCDasOffGCD.CelestialConduit, nil, not Target:IsInMeleeRange(15)) then return "celestial_conduit default_st 18"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=chi<5&combo_strike&variable.small_hotjs_active&!buff.heart_of_the_jade_serpent_cdr_celestial.up&energy.time_to_max<=gcd.max*3&!buff.invokers_delight.up&!buff.bloodlust.up&!buff.storm_earth_and_fire.up
  if S.TigerPalm:IsReady() and (Chi < 5 and ComboStrike(S.TigerPalm) and VarSmallHotjsActive and Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and EnergyTTMCheck and Player:BuffDown(S.InvokersDelightBuff) and not Player:BloodlustUp() and Player:BuffDown(S.StormEarthAndFireBuff)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 20"; end
  end
  -- fists_of_fury,if=combo_strike&buff.heart_of_the_jade_serpent_cdr_celestial.up|variable.small_hotjs_active
  if S.FistsofFury:IsReady() and (ComboStrike(S.FistsofFury) and Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff) or VarSmallHotjsActive) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then return "fists_of_fury default_st 22"; end
  end
  -- spinning_crane_kick,if=buff.dance_of_chiji.stack=2&combo_strike&(!set_bonus.tww3_2pc|!buff.bloodlust.up)&!talent.flurry_strikes
  if S.SpinningCraneKick:IsReady() and (Player:BuffStack(S.DanceofChijiBuff) == 2 and ComboStrike(S.SpinningCraneKick) and (not TWW3_2pc or not Player:BloodlustUp()) and not S.FlurryStrikes:IsAvailable()) then
  ...
  if S.TigerPalm:IsReady() and ((Energy > 55 and S.InnerPeace:IsAvailable() or Energy > 60 and not S.InnerPeace:IsAvailable()) and ComboStrike(S.TigerPalm) and ChiDeficit >= 2 and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < VarTotMMaxStacks and (S.EnergyBurst:IsAvailable() and Player:BuffDown(S.BlackoutKickBuff) or not S.EnergyBurst:IsAvailable()) and Player:BuffDown(S.OrderedElementsBuff) or (S.EnergyBurst:IsAvailable() and Player:BuffDown(S.BlackoutKickBuff) or not S.EnergyBurst:IsAvailable()) and Player:BuffDown(S.OrderedElementsBuff) and S.FistsofFury:CooldownUp() and Chi < 3 or (Player:PrevGCD(1, S.StrikeoftheWindlord) or Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff)) and ComboStrike(S.TigerPalm) and ChiDeficit >= 2 and Player:BuffDown(S.OrderedElementsBuff)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 26"; end
  end
  -- touch_of_death
  if S.TouchofDeath:IsReady() then
    if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not IsInMeleeRange) then return "touch_of_death default_st 28"; end
  end
  -- rising_sun_kick,if=combo_strike&(!pet.xuen_the_white_tiger.active&prev.tiger_palm&time<5|buff.storm_earth_and_fire.up&talent.ordered_elements)
  -- Note: Re-ordered conditions so the condition set can pass or fail with less checks.
  ...
  if S.StrikeoftheWindlord:IsReady() and (Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and S.CelestialConduit:IsAvailable() and Player:BuffDown(S.InvokersDelightBuff) and Player:BuffUp(S.HeartoftheJadeSerpentCDRBuff) and S.FistsofFury:CooldownRemains() < 5 and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 15 and (S.SlicingWinds:CooldownRemains() < 23 or not TWW3_2pc) or BossFightRemains < 12) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_st 32"; end
  end
  -- strike_of_the_windlord,if=talent.gale_force&cooldown.invoke_xuen_the_white_tiger.remains>10
  if S.StrikeoftheWindlord:IsReady() and (S.GaleForce:IsAvailable() and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_st 34"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (CombatTime > 5 and S.FlurryStrikes:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_st 36"; end
  end
  -- strike_of_the_windlord,if=time>15&set_bonus.tww3_2pc&cooldown.invoke_xuen_the_white_tiger.remains>15&(cooldown.slicing_winds.remains<23|!set_bonus.tww3_2pc)
  if S.StrikeoftheWindlord:IsReady() and (CombatTime > 15 and TWW3_2pc and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 15 and (S.SlicingWinds:CooldownRemains() < 23 or not TWW3_2pc)) then
    if Cast(S.StrikeoftheWindlord, Settings.Windwalker.GCDasOffGCD.StrikeOfTheWindlord, nil, not Target:IsSpellInRange(S.StrikeoftheWindlord)) then return "strike_of_the_windlord default_st 38"; end
  end
  -- spinning_crane_kick,if=buff.dance_of_chiji.stack=2&combo_strike&!talent.flurry_strikes
  if S.SpinningCraneKick:IsReady() and (Player:BuffStack(S.DanceofChijiBuff) == 2 and ComboStrike(S.SpinningCraneKick) and not S.FlurryStrikes:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_st 40"; end
  end
  -- blackout_kick,if=buff.teachings_of_the_monastery.stack>3&buff.ordered_elements.up&cooldown.rising_sun_kick.remains>1&cooldown.fists_of_fury.remains>2&!set_bonus.tww3_2pc
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) > 3 and Player:BuffUp(S.OrderedElementsBuff) and S.RisingSunKick:CooldownRemains() > 1 and S.FistsofFury:CooldownRemains() > 2 and not TWW3_2pc) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 42"; end
  end
  -- blackout_kick,if=buff.teachings_of_the_monastery.stack>4&cooldown.rising_sun_kick.remains>1&cooldown.fists_of_fury.remains>2&!set_bonus.tww3_2pc
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) > 4 and S.RisingSunKick:CooldownRemains() > 1 and S.FistsofFury:CooldownRemains() > 2 and not TWW3_2pc) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 44"; end
  end
  -- whirling_dragon_punch,if=set_bonus.tww3_2pc
  if S.WhirlingDragonPunch:IsReady() and (TWW3_2pc) then
    if Cast(S.WhirlingDragonPunch, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch, nil, not IsInMeleeRange) then return "whirling_dragon_punch default_st 46"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and Player:BuffStack(S.DanceofChijiBuff) ~= 2 or Player:BuffUp(S.OrderedElementsBuff) or S.KnowledgeoftheBrokenTemple:IsAvailable()) then
    if Cast(S.WhirlingDragonPunch, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch, nil, not IsInMeleeRange) then return "whirling_dragon_punch default_st 48"; end
  end
  ...
  if S.CracklingJadeLightning:IsReady() and (Player:BuffStack(S.TheEmperorsCapacitorBuff) > 19 and not VarSmallHotjsActive and Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and ComboStrike(S.CracklingJadeLightning) and (not DungeonSlice or Target:TimeToDie() > 20) and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 15 or Player:BuffStack(S.TheEmperorsCapacitorBuff) > 20 and not VarSmallHotjsActive and Player:BuffDown(S.HeartoftheJadeSerpentCDRCelestialBuff) and ComboStrike(S.CracklingJadeLightning) and (not DungeonSlice or Target:TimeToDie() > 20) and S.InvokeXuenTheWhiteTiger:CooldownRemains() < 20 and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 2 or Player:BuffStack(S.TheEmperorsCapacitorBuff) > 10 and BossFightRemains < 5 or Player:BuffRemains(S.StormEarthAndFireBuff) < 2 and Player:BuffStack(S.TheEmperorsCapacitorBuff) > 15 and Player:BuffUp(S.StormEarthAndFireBuff)) then
    if Cast(S.CracklingJadeLightning, Settings.Windwalker.GCDasOffGCD.CracklingJadeLightning, nil, not Target:IsSpellInRange(S.CracklingJadeLightning)) then return "crackling_jade_lightning default_st 50"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and (Chi > 4 or Chi > 2 and Energy > 50 or S.FistsofFury:CooldownRemains() > 2)) then
    if Cast(S.RisingSunKick, nil, nil, not IsInMeleeRange) then return "rising_sun_kick default_st 54"; end
  end
  -- fists_of_fury,if=(talent.xuens_battlegear|!talent.xuens_battlegear&(cooldown.strike_of_the_windlord.remains>1|variable.small_hotjs_active|buff.heart_of_the_jade_serpent_cdr_celestial.up))&(talent.xuens_battlegear&cooldown.invoke_xuen_the_white_tiger.remains>5|cooldown.invoke_xuen_the_white_tiger.remains>10)&(!buff.invokers_delight.up|buff.invokers_delight.up&cooldown.strike_of_the_windlord.remains>4&cooldown.celestial_conduit.remains)|fight_remains<5|talent.flurry_strikes
  if S.FistsofFury:IsReady() and ((S.XuensBattlegear:IsAvailable() or not S.XuensBattlegear:IsAvailable() and (S.StrikeoftheWindlord:CooldownRemains() > 1 or VarSmallHotjsActive or Player:BuffUp(S.HeartoftheJadeSerpentCDRCelestialBuff))) and (S.XuensBattlegear:IsAvailable() and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 5 or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 10) and (Player:BuffDown(S.InvokersDelightBuff) or Player:BuffUp(S.InvokersDelightBuff) and S.StrikeoftheWindlord:CooldownRemains() > 4 and S.CelestialConduit:CooldownDown()) or BossFightRemains < 5 or S.FlurryStrikes:IsAvailable()) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then return "fists_of_fury default_st 56"; end
  end
  -- tiger_palm,if=combo_strike&energy.time_to_max<=gcd.max*3&talent.flurry_strikes&buff.wisdom_of_the_wall_flurry.up
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable() and Player:BuffUp(S.WisdomoftheWallFlurryBuff)) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 58"; end
  end
  -- tiger_palm,if=combo_strike&chi<5&set_bonus.tww3_2pc
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Chi < 5 and TWW3_2pc) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 60"; end
  end
  -- spinning_crane_kick,if=buff.dance_of_chiji.stack=2&combo_strike
  if S.SpinningCraneKick:IsReady() and (Player:BuffStack(S.DanceofChijiBuff) == 2 and ComboStrike(S.SpinningCraneKick)) then
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BloodlustUp() and VarSmallHotjsActive and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_st 66"; end
  end
  -- tiger_palm,if=combo_strike&chi.deficit>=2&energy.time_to_max<=gcd.max*3
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and ChiDeficit >= 2 and EnergyTTMCheck) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 68"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) > 7 and S.MemoryoftheMonastery:IsAvailable() and Player:BuffDown(S.MemoryoftheMonasteryBuff) and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 70"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, Settings.Windwalker.GCDasOffGCD.WhirlingDragonPunch, nil, not IsInMeleeRange) then return "whirling_dragon_punch default_st 74"; end
  end
  -- spinning_crane_kick,if=buff.dance_of_chiji.stack=2&combo_strike
  -- Note: Duplicate from a few lines above.
  ...
  -- spinning_crane_kick,if=buff.dance_of_chiji.up&set_bonus.tww3_2pc&combo_strike
  if S.SpinningCraneKick:IsReady() and (Player:BuffUp(S.DanceofChijiBuff) and TWW3_2pc and ComboStrike(S.SpinningCraneKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_st 78"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.OrderedElementsBuff) and S.RisingSunKick:CooldownRemains() > 1 and S.FistsofFury:CooldownRemains() > 2) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 80"; end
  end
  -- tiger_palm,if=combo_strike&energy.time_to_max<=gcd.max*3&talent.flurry_strikes
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnergyTTMCheck and S.FlurryStrikes:IsAvailable()) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 82"; end
  end
  ...
  -- tiger_palm,if=combo_strike&energy.time_to_max<=gcd.max*3&talent.flurry_strikes
  -- Note: Duplicate from 2 lines above.
  -- jadefire_stomp,if=talent.Singularly_Focused_Jade|talent.jadefire_harmony
  if S.JadefireStomp:IsCastable() and (S.SingularlyFocusedJade:IsAvailable() or S.JadefireHarmony:IsAvailable()) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_st 86"; end
  end
  -- chi_burst,if=!buff.ordered_elements.up
  if S.ChiBurst:IsCastable() and (Player:BuffDown(S.OrderedElementsBuff)) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then return "chi_burst default_st 88"; end
  end
  -- blackout_kick,if=combo_strike&(buff.ordered_elements.up|buff.bok_proc.up&chi.deficit>=1&talent.energy_burst)&cooldown.fists_of_fury.remains
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and (Player:BuffUp(S.OrderedElementsBuff) or Player:BuffUp(S.BlackoutKickBuff) and ChiDeficit >= 1 and S.EnergyBurst:IsAvailable()) and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 90"; end
  end
  -- blackout_kick,if=combo_strike&cooldown.fists_of_fury.remains&(chi>2|energy>60|buff.bok_proc.up)
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown() and (Chi > 2 or Energy > 60 or Player:BuffUp(S.BlackoutKickBuff))) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 92"; end
  end
  -- jadefire_stomp
  if S.JadefireStomp:IsCastable() then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then return "jadefire_stomp default_st 94"; end
  end
  -- tiger_palm,if=combo_strike&buff.ordered_elements.up&chi.deficit>=1
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:BuffUp(S.OrderedElementsBuff) and ChiDeficit >= 1) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 96"; end
  end
  -- chi_burst
  if S.ChiBurst:IsCastable() then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then return "chi_burst default_st 98"; end
  end
  -- spinning_crane_kick,if=combo_strike&buff.ordered_elements.up&talent.hit_combo
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.OrderedElementsBuff) and S.HitCombo:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then return "spinning_crane_kick default_st 100"; end
  end
  -- blackout_kick,if=buff.ordered_elements.up&!talent.hit_combo&cooldown.fists_of_fury.remains
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.OrderedElementsBuff) and not S.HitCombo:IsAvailable() and S.FistsofFury:CooldownDown()) then
    if Cast(S.BlackoutKick, nil, nil, not IsInMeleeRange) then return "blackout_kick default_st 102"; end
  end
  -- tiger_palm,if=prev.tiger_palm&chi<3&!cooldown.fists_of_fury.remains
  if S.TigerPalm:IsReady() and (Player:PrevGCD(1, S.TigerPalm) and Chi < 3 and S.FistsofFury:CooldownUp()) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 104"; end
  end
  -- Manually added: tiger_palm,if=chi=0 (avoids a potential profile stall)
  if S.TigerPalm:IsReady() and (Chi == 0) then
    if Cast(S.TigerPalm, nil, nil, not IsInMeleeRange) then return "tiger_palm default_st 106"; end
  end
end

local function Fallback()
  -- spinning_crane_kick,if=chi>5&combo_strike
  if S.SpinningCraneKick:IsReady() and (Chi > 5 and ComboStrike(S.SpinningCraneKick)) then
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Chi > 3) then
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Chi > 5) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if not Settings.Windwalker.IgnoreFSK and S.FlyingSerpentKickLand:IsCastable() and Player:PrevGCD(1, S.FlyingSerpentKick) then
    ...
    -- variable,name=small_hotjs_active,value=buff.heart_of_the_jade_serpent_cdr.up|buff.heart_of_the_jade_serpent_tww3_tier.up
    VarSmallHotjsActive = Player:BuffUp(S.HeartoftheJadeSerpentCDRBuff) or Player:BuffUp(S.HeartoftheJadeSerpentTWW3TierBuff)
    -- variable,name=invoke_xuen_count,op=add,value=1,if=prev.invoke_xuen_the_white_tiger
    ...
    if CDsON() then
      ...
      if S.ArcaneTorrent:IsCastable() and (Chi < Player:ChiMax() and Energy < 55) then
...
local function Init()
  HR.Print("Windwalker Monk rotation has been updated for patch 11.2.0.")
end
