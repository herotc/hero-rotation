...
local function EvaluateTargetIfBlackoutKick430(TargetUnit)
  return (ComboStrike(S.BlackoutKick) and (Player:BuffP(S.BlackoutKickBuff) or (S.HitComboBuff:IsAvailable() and Player:PrevGCD(1, S.TigerPalm) and Player:Chi() < 4)))
end
...
local function Serenity()
  ...
  -- Manual add to avoid main target icon problems
  if S.BlackoutKick:IsReadyP() and (ComboStrike(S.BlackoutKick) or not S.HitComboBuff:IsAvailable()) then
...
local function St()
  ...
  if S.SpinningCraneKick:IsReadyP() and (ComboStrike(S.SpinningCraneKick) and Player:BuffP(S.DanceofChijiAzeriteBuff)) then
...
local function Aoe()
  ...
  if S.SpinningCraneKick:IsReadyP() and (ComboStrike(S.SpinningCraneKick) and (((Player:Chi() > 3 or S.FistsofFury:CooldownRemainsP() > 6) and (Player:Chi() >= 5 or S.FistsofFury:CooldownRemainsP() > 2)) or Player:EnergyTimeToMaxPredicted() <= 3 or Player:BuffP(S.DanceofChijiAzeriteBuff))) then
  ...
  -- Manual add to avoid main target icon problems
  if S.TigerPalm:IsReadyP() and (Player:ChiDeficit() >= 2 and (not S.HitComboBuff:IsAvailable() or ComboStrike(S.TigerPalm))) then
  ...
  -- Manual add to avoid main target icon problems
  if S.BlackoutKick:IsReadyP() and (ComboStrike(S.BlackoutKick) and (Player:BuffP(S.BlackoutKickBuff) or (S.HitComboBuff:IsAvailable() and Player:PrevGCD(1, S.TigerPalm) and Player:Chi() < 4))) then
