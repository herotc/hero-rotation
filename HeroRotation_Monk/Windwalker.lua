...
local function APL ()
  ...
  Precombat = function()
    ...
    if I.BattlePotionofAgi:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.BattlePotionofAgi) then return "Pre-potion"; end
    end
  ...
  Cooldowns = function()
    if HR.CDsON() then
      ...
      if (I.LustrousGoldenPlumage:IsEquipped(13) or I.LustrousGoldenPlumage:IsEquipped(14)) and I.LustrousGoldenPlumage:IsReadyP() then
        if HR.Cast(I.LustrousGoldenPlumage, Settings.Windwalker.OffGCDasOffGCD.LustrousGoldenPlumage) then 
          return "Use Lustrous Golden Plumage trinket"; end
      end
      ...
      if S.LightsJudgment:IsReadyP() then
        if HR.CastSuggested(S.LightsJudgment, Settings.Commons.OffGCDasOffGCD.Racials) then
          return "Cast Light's Judgment"; end
      end
      ...
      if S.StormEarthAndFire:IsReadyP() and not Player:BuffP(S.StormEarthAndFire) and (S.StormEarthAndFire:ChargesP() == 2 or S.FistsOfFury:CooldownRemainsP() <= 6) and Player:Chi() >= 3 and (S.RisingSunKick:CooldownRemainsP() <= 1 or Target:TimeToDie() <= 15) then
      ...
      if S.Serenity:IsReadyP() and not Player:BuffP(S.Serenity) and 
  ...
  Serenity = function()
    ...
    if S.FistsOfFury:IsReadyP() and 
      (
        (Player:HasHeroismP() and Player:PrevGCD(1,S.RisingSunKick)) or 
  ...
  Aoe = function()
    ...
    if S.TigerPalm:IsReadyP() and Player:ChiDeficit() >= 2 and 
      (not S.HitCombo:IsAvailable() or not Player:PrevGCD(1, S.TigerPalm)) then
    ...
    if S.BlackoutKick:IsReadyP() and not Player:PrevGCD(1, S.BlackoutKick) and
      (Player:BuffP(S.BlackoutKickBuff) or
        (S.HitCombo:IsAvailable() and Player:PrevGCD(1, S.TigerPalm) and Player:Chi() < 4)) then
  ...
  SingleTarget = function()
    ...
    if S.SpinningCraneKick:IsReadyP() and not Player:PrevGCD(1, S.SpinningCraneKick) and Player:BuffRemainsP(S.DanceofChijiBuff) > 0 then
      if HR.Cast(S.SpinningCraneKick) then return "Cast Single Target Spinning Crane Kick"; end
    end
    ...
    if S.BlackoutKick:IsReadyP() and (not Player:PrevGCD(1, S.BlackoutKick) and
      (S.RisingSunKick:CooldownRemainsP() > 3 or Player:Chi() >= 3) and
      (S.FistsOfFury:CooldownRemainsP() > 4 or Player:Chi() >= 4 or 
        (Player:Chi() == 2 and Player:PrevGCD(1, S.TigerPalm))) and 
	...
	-- In Combat
      ...
      if Settings.General.InterruptEnabled and Target:IsInterruptible()  then
        if S.SpearHandStrike:IsReadyP("Melee") then
          if HR.Cast(S.SpearHandStrike, Settings.Windwalker.OffGCDasOffGCD.SpearHandStrike) then return "Spear Hand Strike - Interrupt"; end
        end
      end
      -- actions+=/touch_of_karma,interval=90,pct_health=0.5
      if S.TouchOfKarma:IsReadyP() then
        if HR.Cast(S.TouchOfKarma, Settings.Windwalker.OffGCDasOffGCD.TouchOfKarma) then return "Cast Touch of Karma"; end
      end
      ...
      if S.TigerPalm:IsReadyP() and (Player:EnergyTimeToMaxPredicted() < 1 or
