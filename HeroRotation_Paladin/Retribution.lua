...
function finishers()
	...
	if (HR.AoEON() and Player:HolyPower() >= 3 and (not S.Crusade:IsAvailable() or S.Crusade:CooldownRemains() > Player:GCD() * 2) or Player:Buff(S.EmpyreanPowerBuff) and not Target:Debuff(S.Judgment) and not Player:Buff(S.DivinePurposeBuff) ) then
		if HR.Cast(S.DivineStorm) then return "Cast Divine Storm"; end
	end
