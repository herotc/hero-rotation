...
local function Precombat()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.KillShot:IsCastable() then
      if Cast(S.KillShot, nil, nil, not Target:IsSpellInRange(S.KillShot)) then return "Kill Shot (PreCombat)"; end
    end
    -- REMOVE coordinated_assault
    if S.CoordinatedAssault:IsCastable() then
      if Cast(S.CoordinatedAssault, Settings.Survival.GCDasOffGCD.CoordinatedAssault, nil, not Target:IsSpellInRange(S.CoordinatedAssault)) then return "coordinated_assault 6"; end
    end
    ...
    if S.SteelTrap:IsCastable() and Player:DebuffDown(S.SteelTrapDebuff) then
      if Cast(S.SteelTrap, nil, nil, not Target:IsInRange(40)) then return "steel_trap 10"; end
    end
    -- REMOVE harpoon
    if S.Harpoon:IsCastable() then
      if Cast(S.Harpoon, Settings.Survival.GCDasOffGCD.Harpoon, nil, not Target:IsInRange(30)) then return "harpoon 12"; end
    end
  end
end
...
local function Cds()
  ...
  if S.BloodFury:IsCastable() and (S.CoordinatedAssault:CooldownRemains() > 30) then
    if Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return "[CDs] blood_fury 286"; end
  end
  ...
  if S.AncestralCall:IsCastable() and (S.CoordinatedAssault:CooldownRemains() > 30) then
    if Cast(S.AncestralCall, Settings.Commons.OffGCDasOffGCD.Racials) then return "[CDs] ancestral_call 288"; end
  end
  ...
  if S.Fireblood:IsCastable() and (S.CoordinatedAssault:CooldownRemains() > 30) then
    if Cast(S.Fireblood, Settings.Commons.OffGCDasOffGCD.Racials) then return "[CDs] fireblood 292"; end
  end
  -- lights_judgment
  if S.LightsJudgment:IsCastable() then
    if Cast(S.LightsJudgment, Settings.Commons.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(40)) then return "[CDs] lights_judgment 296"; end
  end
  ...
  if S.Berserking:IsCastable() and (S.CoordinatedAssault:CooldownRemains() > 60 or Target:TimeToDie() < 13) then
    if Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "[CDs] berserking 298"; end
  end
  -- Kill Command
  -- if S.KillCommand:IsCastable() and Target:IsInRange(40) then
  --   if Cast(S.KillCommand) then return "[CDs] Kill Shot (2)"; end
  -- end
  ...
  if S.KillShot:IsCastable() and (EnemyCount8y == 1 and Target:TimeToDie() < Player:Focus() / (S.KillShot:Cost() - Player:FocusCastRegen(S.KillShot:ExecuteTime())) * Player:GCD()) then
    if Cast(S.KillShot, nil, nil, not Target:IsSpellInRange(S.KillShot)) then return "[CDs] KillShot CD"; end
  end
  ...
  if S.MongooseBite:IsCastable() and (EnemyCount8y == 1 and Target:TimeToDie() < Player:Focus() / (S.MongooseBite:Cost() - Player:FocusCastRegen(S.MongooseBite:ExecuteTime())) * Player:GCD()) then
    if Cast(S.MongooseBite, nil, nil, not Target:IsSpellInRange(S.MongooseBite)) then return "[CDs] MongooseBite CD"; end
  end
  ...
  if S.RaptorStrike:IsReady() and (EnemyCount8y == 1 and Target:TimeToDie() < Player:Focus() / (S.MongooseBite:Cost() - Player:FocusCastRegen(S.MongooseBite:ExecuteTime())) * Player:GCD()) then
    if Cast(S.RaptorStrike, nil, nil, not Target:IsSpellInRange(S.RaptorStrike)) then return "[CDs] RaptorStrike CD"; end
  end
  -- aspect_of_the_eagle,if=target.distance>=6
  if S.AspectoftheEagle:IsCastable() and not Target:IsInRange(6) then
    if Cast(S.AspectoftheEagle) then return "[CDs] AspectoftheEagle"; end
  end
end
...
local function apst()
  ...
  if S.SerpentSting:IsReady() then
    ...
    --     if Cast(S.SerpentSting, nil, nil, not Target:IsInRange(40)) then return "[APST] SerpentSting 1@Target"; end
    -- end
  end
  -- flayed_shot
  -- resonating_arrow
  ...
  if CDsON() and S.WildSpirits:IsCastable() then
    if Cast(S.WildSpirits, nil, Settings.Commons.DisplayStyle.Covenant) then return "[APST] Wild Spirits"; end
  end
  -- coordinated_assault
  if CDsON() and S.CoordinatedAssault:IsCastable() then
    if Cast(S.CoordinatedAssault, Settings.Survival.GCDasOffGCD.CoordinatedAssault) then return "[APST] CoordinatedAssault"; end
  end
  -- kill_shot,if=target.health.pct<=20
  if S.KillShot:IsCastable() and Target:HealthPercentage() <= 20 then
    if Cast(S.KillShot, nil, nil, not Target:IsSpellInRange(S.KillShot)) then return "[APST] Kill Shot"; end
  end
  -- flanking_strike,if=focus+cast_regen<focus.max
  -- a_murder_of_crows
  if S.AMurderofCrows:IsCastable() then
    if Cast(S.AMurderofCrows, Settings.Survival.GCDasOffGCD.AMurderofCrows, nil, not Target:IsSpellInRange(S.AMurderofCrows)) then return "[APST] AMurderofCrows"; end
  end
  ...
  --         if Cast(S.WildfireBomb, nil, nil, not Target:IsInRange(40)) then return "[APST] wildfire_bomb 64"; end
  --     end
  -- end
  ...
  if S.PheromoneBomb:IsReady() then
    ...
    if S.PheromoneBomb:FullRechargeTime() < Player:GCD() or Player:Focus() + Player:FocusCastRegen(S.PheromoneBomb:ExecuteTime()) < Player:FocusMax() and (not Player:BuffUp(S.MongooseFuryBuff) and Player:Focus() + Player:FocusCastRegen(S.PheromoneBomb:ExecuteTime()) < Player:FocusMax() - Player:FocusCastRegen(S.KillCommand:ExecuteTime())*3) or Target:TimeToDie() < 10 then
      if Cast(S.PheromoneBomb, nil, nil, not Target:IsSpellInRange(S.PheromoneBomb)) then return "[APST] PheromoneBomb 64"; end
    end
  end
  ...
  if S.VolatileBomb:IsReady() then
    ...
    if S.VolatileBomb:FullRechargeTime() < Player:GCD() or Player:Focus() + Player:FocusCastRegen(S.VolatileBomb:ExecuteTime()) < Player:FocusMax() and (Target:DebuffUp(S.SerpentStingDebuff) and Target:DebuffRefreshable(S.SerpentStingDebuff)) or Target:TimeToDie() < 10 then
      if Cast(S.VolatileBomb, nil, nil, not Target:IsSpellInRange(S.VolatileBomb)) then return "[APST] VolatileBomb 64"; end
    end
  end
  ...
  --     if Cast(S.PheromoneBomb, nil, nil, not Target:IsInRange(40)) then return "[APST] PheromoneBomb"; end
  -- end
  ...
  --     if Cast(S.VolatileBomb, nil, nil, not Target:IsInRange(40)) then return "[APST] PheromoneBomb"; end
  -- end
  ...
  if S.ShrapnelBomb:IsReady() then
    ...
    if (Player:Focus() > S.MongooseBite:Cost()*2 and Target:DebuffRemains(S.SerpentStingDebuff) > 5 * Player:GCD()) or (Player:Focus() + Player:FocusCastRegen(S.ShrapnelBomb:ExecuteTime()) < Player:FocusMax()) then
      if Cast(S.ShrapnelBomb, nil, nil, not Target:IsSpellInRange(S.ShrapnelBomb)) then return "[APST] WildfireBomb shrapnel burst"; end
    end
  end
  -- carve,if=active_enemies>1&!runeforge.rylakstalkers_confounding_strikes.equipped
  if S.Carve:IsReady() and (EnemyCount8y > 1) then
    if Cast(S.Carve, nil, nil, not Target:IsInRange(8)) then return "[APST] carve 379"; end
  end
  ...
  if S.KillCommand:IsCastable() then
    ...
    --     if Cast(S.KillCommand, nil, nil, not Player:GetEnemiesInRange(50)) then return "[APST] Kill Command 2"; end
    -- end
  end
  ...
  if S.RaptorStrike:IsReady() then
    ...
    --     if Cast(S.RaptorStrike, nil, nil, not Player:GetEnemiesInMeleeRange(8)) then return "[APST] RaptorStrike 2"; end
    -- end
  end
  -- mongoose_bite,if=dot.shrapnel_bomb.ticking
  if S.MongooseBite:IsReady() and (Target:DebuffUp(S.ShrapnelBombDebuff)) then
    if Cast(S.MongooseBite, nil, nil, not Target:IsSpellInRange(S.MongooseBite)) then return "[APST] MongooseBite 2"; end
  end
  ...
  if S.KillCommand:IsCastable() then
    ...
    -- if EvaluateKillCommandCycleTargetIfCondition(Target) then
    --     if Cast(S.KillCommand, nil, nil, not Player:GetEnemiesInRange(50)) then return "[APST] Kill Command 4"; end
    -- end
  end
  -- Kill_Command
  if S.KillCommand:IsCastable() and Player:Focus() < 45 then
    if Cast(S.KillCommand, nil, nil, not Target:IsSpellInRange(S.KillCommand)) then return "[APST] Kill Command"; end
  end
  ...
  if S.MongooseBite:IsReady() then
    ...
    if EvaluateRaptorStrikeTargetIfCondition(Target) then
      if Cast(S.MongooseBite, nil, nil, not Target:IsSpellInRange(S.MongooseBite)) then return "[APST] MongooseBite 4"; end
    end
  end
  ...
  if S.RaptorStrike:IsReady() then
    ...
    if EvaluateRaptorStrikeTargetIfCondition(Target) then
      if Cast(S.RaptorStrike, nil, nil, not Target:IsSpellInRange(S.RaptorStrike)) then return "[APST] RaptorStrike 4"; end
    end
  end
  ...
  if S.WildfireBomb:IsCastable() then
    if (S.VolatileBomb.IsCastable() and Target:DebuffUp(S.SerpentStingDebuff)) or (S.PheromoneBomb.IsCastable() or S.ShrapnelBomb.IsCastable()) and Player:Focus() > 50 then
      if Cast(S.WildfireBomb, nil, nil, not Target:IsSpellInRange(S.WildfireBomb)) then return "[APST] Wildfire Bomb"; end
    end
  end
end
...
local function cleave()
  ...
  -- wild_spirits
  if CDsON() and S.WildSpirits:IsCastable() then
    if Cast(S.WildSpirits, nil, Settings.Commons.DisplayStyle.Covenant) then return "[Cleave] Wild Spirits"; end
  end
  -- resonating_arrow
  -- wildfire_bomb,if=full_recharge_time<gcd
  if S.WildfireBomb:IsReady() then
    if S.WildfireBomb:FullRechargeTime() < Player:GCD() then
      if Cast(S.WildfireBomb, nil, nil, not Target:IsSpellInRange(S.WildfireBomb)) then return "[Cleave] Wildfire Bomb"; end
    end
  end
  -- chakrams
  ...
  if S.Carve:IsReady() then
    if Target:DebuffUp(S.ShrapnelBombDebuff) then
      if Cast(S.Carve, nil, nil, not Target:IsInRange(8)) then return "[Cleave] Carve 1"; end
    end
  end
  -- death_chakram,if=focus+cast_regen<focus.max
  -- coordinated_assault
  ...
  if S.Carve:IsReady() then
    if S.Carve:FullRechargeTime() > (EnemyCount8y / 2) and S.AlphaPredator:IsLearned() then
      if Cast(S.Carve, nil, nil, not Target:IsInRange(8)) then return "[Cleave] Carve 2"; end
    end
  end
  ...
  if S.Carve:IsReady() then
    if S.WildfireBomb:FullRechargeTime() > (EnemyCount8y / 2) then
      if Cast(S.Carve, nil, nil, not Target:IsInRange(8)) then return "[Cleave] Carve 3"; end
    end
  end
  -- kill_shot
  if S.KillShot:IsCastable() then
    if Cast(S.KillShot, nil, nil, not Target:IsSpellInRange(S.KillShot)) then return "[Cleave] Kill Shot"; end
  end
  -- flayed_shot
  -- a_murder_of_crows
  -- steel_trap
  ...
  -- carve
  if S.Carve:IsReady() then
    if Cast(S.Carve, nil, nil, not Target:IsInRange(8)) then return "[Cleave] Carve 4"; end
  end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    -- kill_shot,if=target.health.pct<=20
    if S.KillShot:IsCastable() and Target:HealthPercentage() <= 20 then
      if Cast(S.KillShot, nil, nil, not Target:IsSpellInRange(S.KillShot)) then return "[APL] Kill Shot"; end
    end
    -- use_items
