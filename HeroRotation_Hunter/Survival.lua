...
--- APL Main
  local function APL ()
    ...
    if Everyone.TargetIsValid() then
      ...
      if Target:IsInRange(40) and S.WildfireBomb:IsCastable() and not S.WildfireInfusion:IsAvailable() and ((Player:Focus() + Player:FocusCastRegen (Player:GCD()) < Player:FocusMax() or Cache.EnemiesCount[5] > 1) and (not Target:Debuff(S.WildfireBombDot) and not Player:Buff(S.MongooseFury) or S.WildfireBomb:FullRechargeTime() < Player:GCD())) then
      ...
      if S.Butchery:IsAvailable() and S.Butchery:IsCastable() and Player:FocusPredicted(0.2) > 30 and ((not S.WildfireInfusion:IsAvailable() or S.Butchery:FullRechargeTime() < Player:GCD()) and Cache.EnemiesCount[8] > 3 or (Target:Debuff(S.ShrapnelBombDot) and Target:DebuffStack(S.InternalBleeding) < 3)) then
      ...
      if S.SerpentSting:IsCastable() and Target:IsInRange(40) and Player:FocusPredicted(0.2) > 20 and ((Cache.EnemiesCount[8] < 2 and Target:DebuffRefreshable(S.SerpentSting, 2.7) and (not Player:Buff(S.MongooseFury) or (CanGCD() and not S.VipersVenom:IsAvailable()))) or Player:Buff(S.VipersVenomBuff)) then
      ...
      if S.MongooseBite:IsCastable() and S.MongooseBite:IsAvailable() and Player:FocusPredicted(0.2) > 30 and (Player:Buff(S.MongooseFury) or Player:Focus() > 60) then
        if HR.Cast(S.MongooseBite) then return ""; end
      end
      -- actions+=/butchery
      if S.Butchery:IsCastable() and S.Butchery:IsAvailable() and Player:FocusPredicted(0.2) > 30 then
        if HR.Cast(S.Butchery) then return ""; end
      end
