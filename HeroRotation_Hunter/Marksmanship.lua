...
--- APL Action Lists (and Variables)
  ...
  -- actions.cooldowns+=/potion,if=(buff.trueshot.react&buff.bloodlust.react)|buff.bullseye.react>=23|((consumable.prolonged_power&target.time_to_die<62)|target.time_to_die<31)
    ...
    if HR.CDsON() and S.TrueShot:IsCastable() and (TrueshotCooldown == 0 or Player:HasHeroism() or (TrueshotCooldown > 0 and Target:TimeToDie() > (TrueshotCooldown + 15)) or Player:BuffStack(S.BullsEye) > 25 or Target:TimeToDie() < 16) then
      if HR.Cast(S.TrueShot, Settings.Marksmanship.OffGCDasOffGCD.TrueShot) then return ""; end
    end
    return false;
  end
  ...
  local function Non_Patient_Sniper ()
    ...
    if HR.CDsON() and IsCastableP(S.ExplosiveShot) then
      if HR.Cast(S.ExplosiveShot) then return ""; end
    end
    -- actions.non_patient_sniper+=/piercing_shot,if=lowest_vuln_within.5>0&focus>100
    if HR.CDsON() and IsCastableP(S.PiercingShot) and TargetDebuffRemainsP(S.Vulnerability) > 0 and PlayerFocusPredicted() > 100 then
    ...
    if IsCastableP(S.MarkedShot) and Hunter.GetSplashCount(Target,8) > 1 then
      if HR.Cast(S.MarkedShot) then return ""; end
    end
    ...
    if HR.AoEON() and IsCastableP(S.Sentinel) and not TargetDebuffP(S.HuntersMark) then
      HR.CastSuggested(S.Sentinel);
    end
    ...
    if IsCastableP(S.BlackArrow) and (S.Sidewinders:IsAvailable() or Hunter.GetSplashCount(Target,8) < 6) then
      if HR.Cast(S.BlackArrow) then return ""; end
    end
    -- actions.non_patient_sniper+=/a_murder_of_crows,if=target.time_to_die>=cooldown+duration|target.health.pct<20
    if HR.CDsON() and S.AMurderofCrows:IsCastableP() and PlayerFocusPredicted() > S.AMurderofCrows:Cost() and (Target:TimeToDie() >= 60 + 15 or Target:HealthPercentage() < 20) then
    ...
    if IsCastableP(S.Windburst) then
      if not IsCastableM(S.Windburst) then HR.CastSuggested(S.Windburst) elseif HR.Cast(S.Windburst) then return ""; end
    end
    ...
    ((Player:Buff(S.MarkingTargets) or Player:Buff(S.TrueShot) or S.Sidewinders:ChargesFractional() > 1.8) and (PlayerFocusDeficitPredicted() > Player:FocusRegen()))) then
      if HR.Cast(S.Sidewinders) then return ""; end
    end
    ...
    if HR.Cast(S.PoolingSpell) then return ""; end
    return false;
  end
  ...
  local function Patient_Sniper ()
    ...
    if HR.CDsON() and S.PiercingShot:IsAvailable() and S.PiercingShot:CooldownUp() and Hunter.GetSplashCount(Target,8) == 1 and TargetDebuffRemainsP(S.Vulnerability) > 0 and TargetDebuffRemainsP(S.Vulnerability) < 1 then
      if HR.Cast(S.PiercingShot) then return ""; end
    end
    ...
    if HR.CDsON() and IsCastableP(S.AMurderofCrows) and PlayerFocusPredicted() > S.AMurderofCrows:Cost() and (not PoolingforPiercing() or TargetDebuffRemainsP(S.Vulnerability) > Player:GCD()) and (Target:TimeToDie() >= 60 + 15 or (Target:HealthPercentage() < 20 or Target:TimeToDie() < 16)) and Vuln_Aim_Casts == 0 then
      if HR.Cast(S.AMurderofCrows, Settings.Marksmanship.GCDasOffGCD.AMurderofCrows) then return ""; end
    end
  ...
  local function TargetDie () 
    ...
    if HR.CDsON() and IsCastableP(S.PiercingShot) and TargetDebuffP(S.Vulnerability) then
      if HR.Cast(S.PiercingShot) then return ""; end
    end
  ...
  local function APL ()
    ...
    if not Player:AffectingCombat() and not Player:IsCasting() then
      ...
      if IsCastableP(S.Volley) and not Player:Buff(S.Volley) then
        if HR.Cast(S.Volley, Settings.Marksmanship.GCDasOffGCD.Volley) then return; end
      end
      -- Opener
      if Everyone.TargetIsValid() and Target:IsInRange(40) then
        ...
        if IsCastableP(S.Windburst) then
          if HR.Cast(S.Windburst) then return; end
        end
      end
      return;
    end
