...
local function SimulationcraftAPL()
  ...
  if Settings.Blood.ArcaneTorrent and S.ArcaneTorrent:IsCastable() and Player:RunicPowerDeficit() > 20 then
  ...
  if Settings.Blood.DancingRuneWeapon and S.DancingRuneWeapon:IsCastable() and not S.BloodDrinker:IsReady() and not S.DeathandDecay:IsReady() then
  ...
  if S.DeathStrike:IsUsable() and Player:RunicPowerDeficit() <= 15 then
  ...
  if S.DeathStrike:IsUsable() and Player:RunicPowerDeficit() <= 40 and (Player:Buff(S.BloodShield) or not Player:Buff(S.DancingRuneWeaponBuff)) then
    if AR.Cast(S.DeathStrike) then return ""; end
  end
  -- actions.standard+=/consumption
  if Settings.Blood.Consumption and S.Consumption:IsCastable() then
...
local function APL ()
  -- Unit Update
  AC.GetEnemies("Melee");
  AC.GetEnemies(8, true); -- Death and Decay
